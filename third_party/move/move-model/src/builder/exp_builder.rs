            ...
            Apply(_, type_name, args) => {
                ...
                match &type_name.value {
                    Builtin(builtin_type_name) => match &builtin_type_name.value {
                        ...
                        Auth => Type::new_prim(PrimitiveType::Auth),
            ...
            Apply(access, args) => {
                if let EA::ModuleAccess_::Name(n) = &access.value {
                    ...
                    match n.value.as_str() {
                        ...
                        "auth" => {
                            return check_zero_args(self, Type::new_prim(PrimitiveType::Auth));
                        },
                        "vector" => {
                            if args.len() != 1 {
                                self.error(
...
impl ExpTranslator<'_, '_, '_> {
    ...
    fn get_field_decls_for_pack_unpack<'a>(
        ...
        struct_name: &QualifiedSymbol,
        struct_name_loc: &Loc,
    ) -> Option<&'a BTreeMap<Symbol, (Loc, usize, Type)>> {
    ...
    pub fn translate_from_move_value(&mut self, loc: &Loc, ty: &Type, value: &MoveValue) -> Value {
        match (ty, value) {
            ...
            (_, MoveValue::Auth(a)) => Value::Address(Address::Numerical(*a)),
