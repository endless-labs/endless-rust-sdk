...
pub struct ModuleView<'a, T> {
    module: &'a T,
    name_to_function_definition_view: BTreeMap<Identifier, FunctionDefinitionView<'a, T>>,
    name_to_struct_definition_view: BTreeMap<Identifier, StructDefinitionView<'a, T>>,
}
...
impl<'a, T: ModuleAccess> ModuleView<'a, T> {
    pub fn new(module: &'a T) -> Self {
        ...
        for function_def in module.function_defs() {
            ...
            name_to_function_definition_view.insert(view.name().to_owned(), view);
        }
        let mut name_to_struct_definition_view = BTreeMap::new();
        for struct_def in module.struct_defs() {
            let view = StructDefinitionView::new(module, struct_def);
            name_to_struct_definition_view.insert(view.name().to_owned(), view);
        }
        Self {
            module,
...
impl<'a, T: ModuleAccess> LocalsSignatureView<'a, T> {
    ...
    fn parameters(&self) -> &[SignatureToken] {
        &self.function_def_view.parameters().0
    }

    fn additional_locals(&self) -> &[SignatureToken] {
        &self
            .function_def_view
            .module
    ...
    pub fn token_at(&self, index: LocalIndex) -> SignatureTokenView<'_, T> {
...
impl<'a, T: ModuleAccess> FunctionDefinitionView<'a, T> {
    ...
    pub fn locals_signature(&self) -> Option<LocalsSignatureView<'_, T>> {
    ...
    pub fn return_tokens(&self) -> impl DoubleEndedIterator<Item = SignatureTokenView<'_, T>> + '_ {
        self.function_handle_view.return_tokens()
    }

    pub fn arg_tokens(&self) -> impl DoubleEndedIterator<Item = SignatureTokenView<'_, T>> + '_ {
