    ...
    let mut read_next = || {
        if let Ok(byte) = cursor.read_u8() {
            ...
            Ok(match S::from_u8(byte)? {
                ...
                S::AUTH => T::Saturated(SignatureToken::Auth),
                S::VECTOR => T::Vector,
...
fn load_function_def(cursor: &mut VersionedCursor) -> BinaryLoaderResult<FunctionDefinition> {
    ...
    let (visibility, is_entry, sponsored, mut extra_flags) = if cursor.version() == VERSION_1 {
        ...
        (vis, is_entry, false, extra_flags)
    } else {
        let vis = flags.try_into().map_err(|_| {
            PartialVMError::new(StatusCode::MALFORMED)
                .with_message("Invalid visibility byte".to_string())
        ...
        let sponsored = (extra_flags & FunctionDefinition::SPONSORED) != 0;
        if sponsored {
            extra_flags ^= FunctionDefinition::SPONSORED;
        }
        (vis, is_entry, sponsored, extra_flags)
    };
