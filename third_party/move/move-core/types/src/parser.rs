...
pub fn parse_string_list(s: &str) -> Result<Vec<String>> {
    ...
    for c in s.chars() {
        ...
        if !(0..8).contains(&depth) {
            bail!("mismatched parentheses in type tag: {}", s)
        }
    }
...
pub fn parse_type_tags(s: &str) -> Result<Vec<TypeTag>> {
    ...
    let parse_type_list: Vec<String> = parse_string_list(s)?;

    let mut type_tags = vec![];

    for type_str in parse_type_list {
        type_tags.push(parse_single_type(&type_str)?);
    }
...
pub fn parse_type_tag(s: &str) -> Result<TypeTag> {
    let type_params = parse_type_tags(s)?;

    if type_params.is_empty() {
        bail!("No type parameters found");
    }
...
pub fn parse_struct_tag(s: &str) -> Result<StructTag> {
    parse_struct_tag_helper(&remove_white_space(s))
}
...
fn parse_struct_tag_helper(s: &str) -> Result<StructTag> {
    let (address_str, rest) = if let Some((address, rest)) = s.split_once("::") {
        (address, rest)
    } else {
        bail!("Invalid struct tag: missing address in '{}'", s);
    };

    let (module_str, rest) = if let Some((module, rest)) = rest.split_once("::") {
        (module, rest)
    } else {
        bail!("Invalid struct tag: missing module in '{}'", s);
    };

    let (name_str, type_params_str) = if let Some((name, params)) = rest.split_once('<') {
        (name, &params[..params.len() - 1])
    } else {
        (rest, "")
    };
    ...
    Ok(StructTag {
        ...
        module: Identifier::new(module_str)?,
        name: Identifier::new(name_str)?,
        type_params: parse_type_tags(type_params_str)?,
    })
}

fn parse_single_type(s: &str) -> Result<TypeTag> {
    match s.trim() {
        ...
        _ if s.starts_with("vector<") && s.ends_with('>') => {
            ...
            Ok(TypeTag::Vector(Box::new(parse_single_type(inner)?)))
        },
        _ if s.contains("::") => parse_struct_tag_helper(s).map(|st| TypeTag::Struct(Box::new(st))),
        _ => bail!("invalid type tag: {}", s),
    }
}
