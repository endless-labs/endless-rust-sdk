...
impl AccountAddress {
    /// Core resource account: 11111111111111111111111111111Root
    /// Only exists in tests.
    pub const CORE_RESOURCE: Self = Self::get_core_resource_address();
    /// Hex address: 0x4
    ...
    const fn get_core_resource_address() -> Self {
        //0x49DAD7
        let mut addr = [0u8; AccountAddress::LENGTH];
        addr[AccountAddress::LENGTH - 1] = 0xD7;
        addr[AccountAddress::LENGTH - 2] = 0xDA;
        addr[AccountAddress::LENGTH - 3] = 0x49;
        Self(addr)
    }
    ...
    /// if system addr, format to Binary Canonical String
    /// else format to Bs58 encoded
    pub fn to_bs58_string_or_bcs(&self) -> String {
        if self.is_special() {
            hex::encode(self.0)
        } else {
...
impl FromStr for AccountAddress {
    ...
    fn from_str(s: &str) -> Result<Self, AccountAddressParseError> {
        if !s.starts_with("0x") {
            ...
            if let Ok(acc) = AccountAddress::from_bs58_str(s) {
                Ok(acc)
            } else {
...
impl AccountAddress {
    fn from_bs58_str(s: &str) -> Result<Self, AccountAddressParseError> {
        if let Ok(decoded) = bs58::decode(s).into_vec() {
            AccountAddress::from_bytes(decoded)
        } else {
            Err(AccountAddressParseError::Base58DecodeError)
        }
    }
}
...
pub enum AccountAddressParseError {
    ...
    #[error("Base58 decode error when handling input Base58 Encode AccountAddress")]
    Base58DecodeError,
}
