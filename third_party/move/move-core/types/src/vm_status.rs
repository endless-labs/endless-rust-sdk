...
impl VMStatus {
    ...
    pub fn keep_or_discard(self) -> Result<KeptVMStatus, DiscardedVMStatus> {
        match self {
            ...
            VMStatus::ExecutionFailure {
                status_code: StatusCode::PAYMENT_CHECKSUM_MISMATCH,
                ..
            }
            | VMStatus::Error {
                status_code: StatusCode::PAYMENT_CHECKSUM_MISMATCH,
                ..
            } => Ok(KeptVMStatus::PaymentChecksumMismatch),
...
impl fmt::Display for KeptVMStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            KeptVMStatus::PaymentChecksumMismatch => write!(f, "PAY_CHECKSUM_MISMATCH"),
        }
    }
}
...
impl fmt::Display for AbortLocation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AbortLocation::Module(id) => write!(f, "{}", id.short_str_lossless()),
        }
    }
}
...
pub enum StatusCode {
    ...
    INVALID_SPONSORED_DECL_WITHOUT_ENTRY = 1124,
    NUMBER_OF_AUTH_ARGUMENTS_MISMATCH = 1125,
    ...
    // Fungible asset withdraw events are different from simulated.
    PAYMENT_CHECKSUM_MISMATCH = 4036,
    // We tried to create resource with more than currently allowed number of DelayedFields
    TOO_MANY_DELAYED_FIELDS = 4037,
