...
pub fn benchmark(args: &[String]) {
    let cmd_line_parser = Command::new("benchmark")
        ...
        .arg(
            Arg::new("endless-natives")
                .short('a')
                .long("endless")
                .num_args(0)
                .action(Set)
                .help("whether the endless-natives should be included."),
        )
        .arg(
    ...
    let use_endless_natives = matches.contains_id("endless-natives");
