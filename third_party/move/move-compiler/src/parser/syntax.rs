...
fn parse_function_decl(
    ...
    // [ "sponsored" ] "fun" <FunctionDefName>
    let sponsored = if context.tokens.peek() == Tok::Sponsored {
        if entry.is_none() {
            let vloc = current_token_loc(context.tokens);
            let msg = format!(
                "'{}' can only be used with a '{}' function",
                Tok::Sponsored,
                ENTRY_MODIFIER
            );
            return Err(Box::new(diag!(Syntax::UnexpectedToken, (vloc, msg,))));
        }
        context.tokens.advance()?;
        true
    } else {
            ...
            match context.tokens.peek() {
                ...
                _ => {
                    ...
                    match context.tokens.peek() {
                        ...
                        Tok::Fun | Tok::Inline | Tok::Sponsored => ModuleMember::Function(
