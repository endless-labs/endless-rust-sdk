...
// This is a hack to recognize ENDLESS StdLib, Framework, and Token libs to avoid warnings on some old errors.
// This will be removed after library attributes are cleaned up.
// (See https://github.com/endless-labs/endless-core/issues/9410)
fn module_is_in_endless_libs(module_address: Option<Spanned<Address>>) -> bool {
    const ENDLESS_STDLIB_NAME: &str = "endless_std";
    static ENDLESS_STDLIB_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const ENDLESS_FRAMEWORK_NAME: &str = "endless_framework";
    static ENDLESS_FRAMEWORK_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const ENDLESS_TOKEN_NAME: &str = "endless_token";
    static ENDLESS_TOKEN_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x3").unwrap());
    const ENDLESS_TOKEN_OBJECTS_NAME: &str = "endless_token_objects";
    static ENDLESS_TOKEN_OBJECTS_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
    ...
    match &module_address {
        Some(spanned_address) => {
            ...
            match address {
                ...
                {
                    Some(spanned_symbol) => {
                        ((&spanned_symbol.value as &str) == ENDLESS_STDLIB_NAME
                            && (spanned_numerical_address.value
                                == *ENDLESS_STDLIB_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == ENDLESS_FRAMEWORK_NAME
                                && (spanned_numerical_address.value
                                    == *ENDLESS_FRAMEWORK_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == ENDLESS_TOKEN_NAME
                                && (spanned_numerical_address.value
                                    == *ENDLESS_TOKEN_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == ENDLESS_TOKEN_OBJECTS_NAME
                                && (spanned_numerical_address.value
                                    == *ENDLESS_TOKEN_OBJECTS_NUMERICAL_ADDRESS))
                    },
                    None => false,
                },
...
fn module_(
    ...
    {
        context.in_endless_libs = false;
    } else {
        context.in_endless_libs = module_is_in_endless_libs(module_address);
    }
...
fn check_for_deprecated_module_use(context: &mut Context, mident: &ModuleIdent) -> bool {
    ...
    if !warn_deprecation || context.in_deprecated_code || context.in_endless_libs {
        return false;
    }
