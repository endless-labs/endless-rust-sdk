...
impl Value {
    ...
    pub fn auth(x: AccountAddress) -> Self {
        Self(ValueImpl::Container(Container::auth(x)))
    }

    pub fn auth_reference(x: AccountAddress) -> Self {
        Self(ValueImpl::ContainerRef(ContainerRef::Local(
            Container::auth(x),
        )))
    }
...
fn check_elem_layout(ty: &Type, v: &Container) -> PartialVMResult<()> {
    match (ty, v) {
        ...
        | (Type::Signer, Container::Struct(_))
        | (Type::Auth, Container::Struct(_)) => Ok(()),
        ...
        | (Type::Auth, _) => Err(PartialVMError::new(
            StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
        )
        .with_message(format!(
...
impl Vector {
    pub fn pack(type_param: &Type, elements: Vec<Value>) -> PartialVMResult<Value> {
        let container = match type_param {
            ...
            | Type::Auth => Value(ValueImpl::Container(Container::Vec(Rc::new(RefCell::new(
    ...
    for SerializationReadyValue<'_, '_, '_, MoveTypeLayout, ValueImpl, C>
{
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        match (self.layout, self.value) {
            ...
            // Auth
            (L::Auth, ValueImpl::Container(Container::Struct(r))) => {
                let v = r.borrow();
                if v.len() != 1 {
                    return Err(invariant_violation::<S>(format!(
                        "cannot serialize container as a auth -- expected 1 field got {}",
                        v.len()
                    )));
                }
                (SerializationReadyValue {
    ...
    for DeserializationSeed<'_, &MoveTypeLayout, C>
{
    ...
    fn deserialize<D: serde::de::Deserializer<'d>>(
        ...
        match self.layout {
            ...
            L::Auth => AccountAddress::deserialize(deserializer).map(Value::auth),
...
pub mod prop {
    ...
    pub fn value_strategy_with_layout(layout: &MoveTypeLayout) -> impl Strategy<Value = Value> {
        ...
        match layout {
            ...
            L::Auth => any::<AccountAddress>().prop_map(Value::auth).boxed(),
