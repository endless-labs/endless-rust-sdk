...
### Naming
- **Module names**: are camel case e.g., `EndlessAccount`, `Endless`
...
- Generic types should be descriptive, or anti-descriptive where appropriate (e.g. `T` for the Vector generic type parameter, `EndlessAccount` for the core `EndlessAccount` resource, `deposit<CoinType>(t: CoinType)` for depositing a token in the `Endless` module). Most of the time the "main" type in a module should be the same name as the module e.g., `Endless::Endless`, `EndlessAccount::EndlessAccount`.
- **Module file names**: are the same as the module name e.g., `EndlessAccount.move`
