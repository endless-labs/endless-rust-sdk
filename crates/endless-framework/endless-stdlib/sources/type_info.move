module endless_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `endless_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `endless_framework::chain_id::get()` will
    /// return whichever ID was passed to `endless_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::endless_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(account_address(&type_info) == @endless_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags(&fx, vector[features::get_endless_stdlib_chain_id_feature()], vector[]);

        // The testing environment chain ID is 4u8.
        assert!(chain_id() == 223u8, 1);
    }
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @endless_std;
