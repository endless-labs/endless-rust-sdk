...
module endless_std::ed25519 {
    ...
    /// The identifier of the Ed25519 signature scheme, which is used when deriving Endless authentication keys by hashing
    ...
    #[test_only]
    /// Generates an Ed25519 key pair.
    public fun generate_known_keys(sk: vector<u8>, pk: vector<u8>): (SecretKey, ValidatedPublicKey) {
        let sk = SecretKey {
            bytes: sk
        };
        let pk = ValidatedPublicKey {
    ...
    #[test_only]
    /// Generates an Ed25519 key pair.
    public fun generate_keys_internal_test(): (vector<u8>, vector<u8>) {
