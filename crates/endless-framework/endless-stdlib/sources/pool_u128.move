...
module endless_std::pool_u128 {
    ...
    public fun create_with_scaling_factor(shareholders_limit: u64, scaling_factor: u128): Pool {
        Pool {
            shareholders_limit,
            total_coins: 0,
            total_shares: 0,
    ...
    public fun total_coins(pool: &Pool): u128 {
        pool.total_coins
    }
    ...
    public fun balance(pool: &Pool, shareholder: address): u128 {
    ...
    public fun update_total_coins(pool: &mut Pool, new_total_coins: u128) {
    ...
    public fun buy_in(pool: &mut Pool, shareholder: address, coins_amount: u128): u128 {
        if (coins_amount == 0) return 0;

        let new_shares = amount_to_shares(pool, coins_amount);
        assert!(MAX_U128 - pool.total_coins >= coins_amount, error::invalid_argument(EPOOL_TOTAL_COINS_OVERFLOW));
        assert!(MAX_U128 - pool.total_shares >= new_shares, error::invalid_argument(EPOOL_TOTAL_COINS_OVERFLOW));
    ...
    public fun redeem_shares(pool: &mut Pool, shareholder: address, shares_to_redeem: u128): u128 {
    ...
    public fun amount_to_shares(pool: &Pool, coins_amount: u128): u128 {
    ...
    /// `amount` needs to big enough to avoid rounding number.
    public fun amount_to_shares_with_total_coins(pool: &Pool, coins_amount: u128, total_coins: u128): u128 {
    ...
    public fun shares_to_amount(pool: &Pool, shares: u128): u128 {
    ...
    /// `shares` needs to big enough to avoid rounding number.
    public fun shares_to_amount_with_total_coins(pool: &Pool, shares: u128, total_coins: u128): u128 {
    ...
    public entry fun test_buy_in_and_redeem_large_numbers() {
        let pool = new(2);
        let half_max_u128 = MAX_U128 / 2;
        let shares_1 = buy_in(&mut pool, @1, half_max_u128);
        assert!(shares_1 == half_max_u128, 0);
        let shares_2 = buy_in(&mut pool, @2, half_max_u128 + 1);
        assert!(shares_2 == half_max_u128 + 1, 1);
        assert!(total_shares(&pool) == MAX_U128, 2);
        assert!(total_coins(&pool) == MAX_U128, 3);
        assert!(redeem_shares(&mut pool, @1, shares_1) == half_max_u128, 4);
        assert!(redeem_shares(&mut pool, @2, shares_2) == half_max_u128 + 1, 5);
        destroy_empty(pool);
    }
