...
module endless_std::smart_table {
    use std::error;
    use std::vector;
    ...
    public inline fun for_each_ref<K, V>(table: &SmartTable<K, V>, f: |&K, &V|) {
        let i = 0;
        while (i < endless_std::smart_table::num_buckets(table)) {
            vector::for_each_ref(
                endless_std::table_with_length::borrow(endless_std::smart_table::borrow_buckets(table), i),
                |elem| {
                    let (key, value) = endless_std::smart_table::borrow_kv(elem);
                    f(key, value)
                }
            );
            i = i + 1;
    ...
    public inline fun for_each_mut<K, V>(table: &mut SmartTable<K, V>, f: |&K, &mut V|) {
        ...
        while (i < endless_std::smart_table::num_buckets(table)) {
            vector::for_each_mut(
                table_with_length::borrow_mut(endless_std::smart_table::borrow_buckets_mut(table), i),
                |elem| {
                    let (key, value) = endless_std::smart_table::borrow_kv_mut(elem);
                    f(key, value)
                }
            );
            i = i + 1;
    ...
    public inline fun any<K, V>(
        ...
        let i = 0;
        while (i < endless_std::smart_table::num_buckets(table)) {
            found = vector::any(table_with_length::borrow(endless_std::smart_table::borrow_buckets(table), i), |elem| {
                let (key, value) = endless_std::smart_table::borrow_kv(elem);
                p(key, value)
            });
            if (found) break;
