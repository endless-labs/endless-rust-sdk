...
module endless_token::nft {
    use std::error;
    use std::option::{Self, Option};
    use std::string::String;
    use std::signer;
    use endless_framework::object::{Self, ConstructorRef, Object};
    use endless_token::collection;
    use endless_token::property_map;
    use endless_token::royalty;
    use endless_token::token;
    ...
    struct EndlessCollection has key {
    ...
    public fun create_collection_object(
        ...
        let endless_collection = EndlessCollection {
            mutator_ref,
            royalty_mutator_ref,
            mutable_description,
            mutable_uri,
        ...
        move_to(&object_signer, endless_collection);
    ...
    ) acquires EndlessCollection, EndlessToken {
    ...
    public fun mint_token_object(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): Object<EndlessToken> acquires EndlessCollection, EndlessToken {
        ...
        if (freezable_by_creator) {
            let endless_token_addr = object::address_from_constructor_ref(&constructor_ref);
            let endless_token = borrow_global_mut<EndlessToken>(endless_token_addr);
            let transfer_ref = object::generate_transfer_ref(&constructor_ref);
            option::fill(&mut endless_token.transfer_ref, transfer_ref);
        };
    ...
    public fun mint_soul_bound_token_object(
        ...
        property_values: vector<vector<u8>>,
        soul_bound_to: address,
    ): Object<EndlessToken> acquires EndlessCollection {
        let constructor_ref = mint_internal(
            creator,
            collection,
            description,
    ...
    fun mint_internal(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): ConstructorRef acquires EndlessCollection {
    ...
    inline fun borrow<T: key>(token: &Object<T>): &EndlessToken {
        let token_address = object::object_address(token);
        assert!(
            exists<EndlessToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
        );
        borrow_global<EndlessToken>(token_address)
    }

    #[view]
    public fun are_properties_mutable<T: key>(token: Object<T>): bool acquires EndlessCollection {
        let collection = token::collection_object(token);
        borrow_collection(&collection).mutable_token_properties
    }

    #[view]
    public fun is_burnable<T: key>(token: Object<T>): bool acquires EndlessToken {
        option::is_some(&borrow(&token).burn_ref)
    }

    #[view]
    public fun is_freezable_by_creator<T: key>(token: Object<T>): bool acquires EndlessCollection {
        are_collection_tokens_freezable(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_description<T: key>(token: Object<T>): bool acquires EndlessCollection {
        is_mutable_collection_token_description(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_name<T: key>(token: Object<T>): bool acquires EndlessCollection {
        is_mutable_collection_token_name(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_uri<T: key>(token: Object<T>): bool acquires EndlessCollection {
        is_mutable_collection_token_uri(token::collection_object(token))
    }

    // Token mutators

    inline fun authorized_borrow<T: key>(token: &Object<T>, creator: &signer): &EndlessToken {
        let token_address = object::object_address(token);
        assert!(
            exists<EndlessToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
    ...
    public entry fun burn<T: key>(creator: &signer, token: Object<T>) acquires EndlessToken {
        let endless_token = authorized_borrow(&token, creator);
        assert!(
            option::is_some(&endless_token.burn_ref),
            error::permission_denied(ETOKEN_NOT_BURNABLE),
        );
        move endless_token;
        let endless_token = move_from<EndlessToken>(object::object_address(&token));
        let EndlessToken {
            burn_ref,
            transfer_ref: _,
            mutator_ref: _,
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires EndlessCollection, EndlessToken {
        let endless_token = authorized_borrow(&token, creator);
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && option::is_some(&endless_token.transfer_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        object::disable_ungated_transfer(option::borrow(&endless_token.transfer_ref));
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        object::enable_ungated_transfer(option::borrow(&endless_token.transfer_ref));
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let endless_token = authorized_borrow(&token, creator);
        token::set_description(option::borrow(&endless_token.mutator_ref), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let endless_token = authorized_borrow(&token, creator);
        token::set_name(option::borrow(&endless_token.mutator_ref), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let endless_token = authorized_borrow(&token, creator);
        token::set_uri(option::borrow(&endless_token.mutator_ref), uri);
    }
    ...
    public entry fun add_property<T: key>(
        ...
        property_map::add(&endless_token.property_mutator_ref, key, type, value);
    }
    ...
    public entry fun add_typed_property<T: key, V: drop>(
        ...
        property_map::add_typed(&endless_token.property_mutator_ref, key, value);
    }
    ...
    public entry fun remove_property<T: key>(
        ...
        property_map::remove(&endless_token.property_mutator_ref, &key);
    }
    ...
    public entry fun update_property<T: key>(
        ...
        property_map::update(&endless_token.property_mutator_ref, &key, type, value);
    }
    ...
    public entry fun update_typed_property<T: key, V: drop>(
        ...
        property_map::update_typed(&endless_token.property_mutator_ref, &key, value);
    }

    // Collection accessors

    inline fun collection_object(creator: &signer, name: &String): Object<EndlessCollection> {
        let collection_addr = collection::create_collection_address(&signer::address_of(creator), name);
        object::address_to_object<EndlessCollection>(collection_addr)
    }

    inline fun borrow_collection<T: key>(token: &Object<T>): &EndlessCollection {
        let collection_address = object::object_address(token);
        assert!(
            exists<EndlessCollection>(collection_address),
            error::not_found(ECOLLECTION_DOES_NOT_EXIST),
        );
        borrow_global<EndlessCollection>(collection_address)
    }

    public fun is_mutable_collection_description<T: key>(
        collection: Object<T>,
    ): bool acquires EndlessCollection {
    ...
    inline fun authorized_borrow_collection<T: key>(collection: &Object<T>, creator: &signer): &EndlessCollection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        let endless_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            endless_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        collection::set_description(option::borrow(&endless_collection.mutator_ref), description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        let endless_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            option::is_some(&endless_collection.royalty_mutator_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        royalty::update(option::borrow(&endless_collection.royalty_mutator_ref), royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        collection::set_uri(option::borrow(&endless_collection.mutator_ref), uri);
    }
    ...
    #[test(creator = @0x123)]
    fun test_create_and_transfer(creator: &signer) acquires EndlessCollection, EndlessToken {
    ...
    fun test_mint_soul_bound(creator: &signer, bob: &signer) acquires EndlessCollection {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = object)]
    fun test_frozen_transfer(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_unfrozen_transfer(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_noncreator_freeze(creator: &signer, another: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123, another = @0x456)]
    #[expected_failure(abort_code = 0x50003, location = Self)]
    fun test_noncreator_unfreeze(creator: &signer, another: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_description(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_set_immutable_description(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_name(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_name(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_uri(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_uri(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_burnable(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        ...
        assert!(exists<EndlessToken>(token_addr), 0);
        burn(creator, token);
        assert!(!exists<EndlessToken>(token_addr), 1);
    }

    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    fun test_not_burnable(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_description(creator: &signer) acquires EndlessCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_description(creator: &signer) acquires EndlessCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_uri(creator: &signer) acquires EndlessCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, true);
        let value = string::utf8(b"not");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_uri(creator: &signer) acquires EndlessCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, false);
    ...
    #[test(creator = @0x123)]
    fun test_property_add(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_typed_add(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"u8");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        add_typed_property<EndlessToken, u8>(creator, token, property_name, 0x8);

        assert!(property_map::read_u8(&token, &property_name) == 0x8, 0);
    }

    #[test(creator = @0x123)]
    fun test_property_update(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_update_typed(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        update_typed_property<EndlessToken, bool>(creator, token, property_name, false);

        assert!(!property_map::read_bool(&token, &property_name), 0);
    }

    #[test(creator = @0x123)]
    fun test_property_remove(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
    ...
    #[test(creator = @0x123)]
    fun test_royalties(creator: &signer) acquires EndlessCollection, EndlessToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
