...
impl ExtendedChecker<'_> {
    ...
    fn check_transaction_input_type(&self, loc: &Loc, ty: &Type) {
        ...
        match ty {
            ...
            Reference(ReferenceKind::Immutable, bt)
                if matches!(
                    bt.as_ref(),
                    Primitive(PrimitiveType::Signer | PrimitiveType::Auth)
                ) =>
            {
                // Reference to signer or auth allowed
            },
            Vector(ety) => {
    ...
    fn is_allowed_input_struct(&self, qid: QualifiedId<StructId>) -> bool {
        ...
        // TODO(gerben) find a nice way to keep this in sync with allowed_structs in endless-vm
        matches!(
            name.as_str(),
            "0x1::string::String"
                | "0x1::object::Object"
