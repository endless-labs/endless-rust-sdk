...
impl ReleasePackage {
    ...
    fn generate_script_proposal_impl(
        ...
        emitln!(writer, "use endless_framework::endless_governance;");
        emitln!(writer, "use endless_framework::code;\n");
        ...
        if is_testnet && !is_multi_step {
            ...
            emitln!(
                writer,
                "let framework_signer = endless_governance::get_signer_testnet_only(core_resources, @{});",
                for_address
            );
            ...
            emitln!(
                writer,
                "let framework_signer = endless_governance::resolve(proposal_id, @{});",
                for_address
            );
        } else {
            emitln!(writer, "fun main(proposal_id: u64){");
    ...
    fn generate_next_execution_hash_blob(
        ...
        if next_execution_hash == "vector::empty<u8>()".as_bytes() {
            emitln!(
                writer,
                "let framework_signer = endless_governance::resolve_multi_step_proposal(proposal_id, @{}, {});\n",
                for_address,
                "vector::empty<u8>()",
            );
        } else {
            emitln!(
                writer,
                "let framework_signer = endless_governance::resolve_multi_step_proposal("
            );
            writer.indent();
