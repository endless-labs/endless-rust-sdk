...
impl ReleaseTarget {
    ...
    pub fn packages(self) -> Vec<(&'static str, Option<&'static str>)> {
        let result = vec![
            ...
            (
                "endless-framework",
                Some("cached-packages/src/endless_framework_sdk_builder.rs"),
            ),
            (
                "endless-token",
                Some("cached-packages/src/endless_token_objects_sdk_builder.rs"),
            ),
        ];
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    let resources = NumericalAddress::parse_str("0x49DAD7").unwrap();
    result.insert("std".to_owned(), one);
    result.insert("endless_std".to_owned(), one);
    result.insert("endless_framework".to_owned(), one);
    result.insert("endless_token".to_owned(), four);
