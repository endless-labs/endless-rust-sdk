...
use endless_aggregator::{
    ...
    types::{code_invariant_error, DelayedFieldCompare},
};
use endless_gas_algebra::NumBytes;
use endless_gas_schedule::gas_params::natives::endless_framework::*;
use endless_native_interface::{
    safely_pop_arg, RawSafeNative, SafeNativeBuilder, SafeNativeContext, SafeNativeError,
    SafeNativeResult,
};
use endless_types::delayed_fields::{
...
fn native_try_sub(
    ...
    debug_assert_eq!(args.len(), 2);
    debug_assert_eq!(ty_args.len(), 1);
    context.charge(AGGREGATOR_V2_COMPARE_BASE)?;
    ...
    let res = if let Some((resolver, mut delayed_field_data)) = get_context_data(context) {
        let id = get_aggregator_value_as_id(&aggregator, aggregator_value_ty, resolver)?;
...
fn compare_impl(
    ...
    let res = if let Some((resolver, delayed_field_data)) = get_context_data(context) {
        let id = get_aggregator_value_as_id(&aggregator, aggregator_value_ty, resolver)?;
        delayed_field_data.compare(id, max_value, method, rhs, resolver)?
    } else {
        let lhs = get_aggregator_value(&aggregator, aggregator_value_ty)?;
...
fn native_less_than(
    ...
    compare_impl(context, ty_args, args, DelayedFieldCompare::LessThan)
}

/********************************************************************************************************
 * public native fun less_than_or_equal<IntElement>(aggregator: &Aggregator<IntElement>, value: IntElement): bool;
 *******************************************************************************************************/
fn native_less_than_or_equal(
    context: &mut SafeNativeContext,
    ty_args: Vec<Type>,
    args: VecDeque<Value>,
) -> SafeNativeResult<SmallVec<[Value; 1]>> {
    compare_impl(context, ty_args, args, DelayedFieldCompare::LessThanOrEqual)
}
...
fn native_greater_than_or_equal(
    ...
    compare_impl(
        ...
        ty_args,
        args,
        DelayedFieldCompare::GreaterThanOrEqual,
    )
}
...
pub fn make_all(
    ...
    let natives = [
        ...
        ("less_than", native_less_than),
        ("less_than_or_equal", native_less_than_or_equal),
        ("greater_than", native_greater_than),
        ("greater_than_or_equal", native_greater_than_or_equal),
        ("read", native_read),
