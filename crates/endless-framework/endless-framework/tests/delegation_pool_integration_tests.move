#[test_only]
module endless_framework::delegation_pool_integration_tests {
    use std::features;
    use std::signer;

    use endless_std::bls12381;
    use endless_std::stake;
    use endless_std::vector;

    use endless_framework::account;
    use endless_framework::endless_coin;
    use endless_framework::reconfiguration;
    use endless_framework::delegation_pool as dp;
    ...
    public fun initialize_for_test_custom(
        ...
        features::change_feature_flags(endless_framework, vector[DELEGATION_POOLS, MODULE_EVENT], vector[]);
    }
    ...
    #[expected_failure(abort_code = 0x10007, location = endless_framework::stake)]
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_EDS, false, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator, 9900 * ONE_EDS + 1);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_EDS, true, true);
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, true, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator_2, 9900 * ONE_EDS + 1);
    }
    ...
    public entry fun test_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_EDS, true, true);
    ...
    public entry fun test_active_validator_with_pending_inactive_stake_cannot_add_stake_if_exceeding_max_allowed(
        ...
        dp::unlock(validator, pool_address, 50 * ONE_EDS);
        stake::assert_validator_state(pool_address, 50 * ONE_EDS, 0, 0, 50 * ONE_EDS, 0);

        // Add 9900 EDS + 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 EDS + 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator, 9900 * ONE_EDS + 1);
    }
    ...
    public entry fun test_pending_inactive_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_EDS, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, true, true);
        ...
        // Add 9900 EDS + 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 EDS + 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator_1, 9900 * ONE_EDS + 1);
    }
    ...
    public entry fun test_end_to_end(
        ...
        stake::mint(validator, 900 * ONE_EDS);
        dp::add_stake(validator, pool_address, 100 * ONE_EDS);
        assert!(endless_coin::balance(validator_address) == 800 * ONE_EDS, 2);
        stake::assert_validator_state(pool_address, 100 * ONE_EDS, 0, 100 * ONE_EDS, 0, 0);
        ...
        stake::assert_validator_state(pool_address, 201 * ONE_EDS, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_EDS);
        ...
        dp::withdraw(validator, pool_address, 50 * ONE_EDS);
        assert!(endless_coin::balance(validator_address) == 84999999999, 6);
        stake::assert_validator_state(pool_address, 10201000001, 5099999999, 0, 0, 0);
        dp::withdraw(validator, pool_address, 51 * ONE_EDS);
        assert!(endless_coin::balance(validator_address) == 90099999998, 7);
    ...
    #[test(endless_framework = @endless_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x1000D, location = endless_framework::stake)]
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, false, false);
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_EDS, false, true);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, false, false);
        ...
        mint_and_add_stake(validator_2, 100 * ONE_EDS);
        stake::assert_validator_state(validator_2_address, 200 * ONE_EDS, 0, 0, 0, 0);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator_2, ONE_EDS);
    }
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_EDS, true, false);
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit_in_multiple_epochs(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(validator_address, 100 * ONE_EDS, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 110 * ONE_EDS, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 121 * ONE_EDS, 0, 0, 0, 0);
        // Add more than 50% limit. The following line should fail.
        mint_and_add_stake(validator, 99 * ONE_EDS);
    }
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator, 50 * ONE_EDS + 1);
    }
    ...
    public entry fun test_active_validator_unlock_partial_stake(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_EDS);
        stake::assert_validator_state(validator_address, 50 * ONE_EDS, 0, 0, 50 * ONE_EDS, 0);
        ...
        stake::assert_validator_state(validator_address, 55 * ONE_EDS, 55 * ONE_EDS, 0, 0, 0);
    ...
    public entry fun test_active_validator_can_withdraw_all_stake_and_rewards_at_once(
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_EDS, 0, 0, 0, 0);
        ...
        dp::unlock(validator, validator_address, 101 * ONE_EDS);
        stake::assert_validator_state(validator_address, 0, 0, 0, 101 * ONE_EDS, 0);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x10006, location = endless_framework::delegation_pool)]
    public entry fun test_active_validator_unlocking_more_than_available_stake_should_cap(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 200 * ONE_EDS);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 100 * ONE_EDS);
        ...
        dp::withdraw(validator, validator_address, 200 * ONE_EDS);

        // Receive back all coins with an extra 1 for rewards.
        assert!(endless_coin::balance(signer::address_of(validator)) == 100100000000, 2);
    ...
    public entry fun test_active_validator_can_reactivate_pending_inactive_stake(
        ...
        dp::reactivate_stake(validator, validator_address, 50 * ONE_EDS);
        stake::assert_validator_state(validator_address, 100 * ONE_EDS, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_reactivate_more_than_available_pending_inactive_stake_should_cap(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_EDS);
        stake::assert_validator_state(validator_address, 50 * ONE_EDS, 0, 0, 50 * ONE_EDS, 0);
        dp::reactivate_stake(validator, validator_address, 51 * ONE_EDS);
        stake::assert_validator_state(validator_address, 100 * ONE_EDS, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_leaves_staking_but_still_has_a_lockup(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator, 100 * ONE_EDS, true, false);
        ...
        stake::assert_validator_state(validator_address, 100 * ONE_EDS, 0, 0, 0, 1);
        end_epoch();
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_EDS, 0, 0, 0, 1);
    ...
    public entry fun test_pending_inactive_validator_does_not_count_in_increase_limit(
        ...
        mint_and_add_stake(validator_1, 51 * ONE_EDS);
    }
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_EDS, false, false);
        ...
        stake::assert_validator_state(validator_1_address, 100 * ONE_EDS, 0, 0, 0, 0);
        stake::assert_validator_state(validator_2_address, 100 * ONE_EDS, 0, 0, 0, 1);
        ...
        stake::assert_validator_state(validator_1_address, 101 * ONE_EDS, 0, 0, 0, 0);
        ...
        stake::assert_validator_state(validator_2_address, 101 * ONE_EDS, 0, 0, 0, 1);
        assert!(stake::get_validator_state(validator_3_address) == VALIDATOR_STATUS_ACTIVE, 10);
        stake::assert_validator_state(validator_3_address, 100 * ONE_EDS, 0, 0, 0, 1);

        // Validators without enough stake will be removed.
        dp::unlock(validator_1, validator_1_address, 50 * ONE_EDS);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        mint_and_add_stake(validator, 100 * ONE_EDS);
        let pool_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(pool_address, 100 * ONE_EDS, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_EDS);
        stake::assert_validator_state(pool_address, 0, 0, 0, 100 * ONE_EDS, 0);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();

        // Withdraw stake + rewards.
        stake::assert_validator_state(pool_address, 0, 101 * ONE_EDS, 0, 0, 0);
        dp::withdraw(validator, pool_address, 101 * ONE_EDS);
        assert!(endless_coin::balance(signer::address_of(validator)) == 101 * ONE_EDS, 1);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000A, location = endless_framework::stake)]
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000E, location = endless_framework::stake)]
    ...
    public entry fun test_staking_validator_index(
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_4, &pop_4, validator_4, 100 * ONE_EDS, false, false);
        initialize_test_validator(&pk_5, &pop_5, validator_5, 100 * ONE_EDS, false, false);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000B, location = endless_framework::stake)]
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        stake::remove_validators(endless_framework, &vector[validator_to_remove]);
