...
module endless_framework::storage_gas {
    ...
    public(friend) fun set_config(endless_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_endless_framework(endless_framework);
        *borrow_global_mut<StorageGasConfig>(@endless_framework) = config;
    }
    ...
    public fun initialize(endless_framework: &signer) {
        system_addresses::assert_endless_framework(endless_framework);
        assert!(
            !exists<StorageGasConfig>(@endless_framework),
        ...
        move_to(endless_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@endless_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(endless_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@endless_framework);
        let gas = borrow_global_mut<StorageGas>(@endless_framework);
    ...
    //#[test(framework = @endless_framework)]
    #[test_only]
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@endless_framework);
