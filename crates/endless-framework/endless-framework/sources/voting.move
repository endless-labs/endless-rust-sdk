...
module endless_framework::voting {
    ...
    public fun create_proposal_with_empty_execution_hash_should_fail_generic(governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        let governance_address = signer::address_of(governance);
    ...
    #[test_only]
    public entry fun test_voting_passed_generic(endless_framework: &signer, governance: &signer, use_create_multi_step: bool, use_resolve_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
    ...
    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_voting_passed(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(endless_framework, governance, false, false);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_voting_passed_multi_step(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(endless_framework, governance, true, true);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    #[expected_failure(abort_code=0x5000a, location = Self)]
    public entry fun test_voting_passed_multi_step_cannot_use_single_step_resolve_function(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(endless_framework, governance, true, false);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_voting_passed_single_step_can_use_generic_function(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(endless_framework, governance, false, true);
    }

    #[test_only]
    public entry fun test_cannot_resolve_twice_generic(endless_framework: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        timestamp::set_time_has_started_for_testing(endless_framework);
    ...
    public entry fun test_cannot_resolve_twice(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_cannot_resolve_twice_generic(endless_framework, governance, false);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    #[expected_failure(abort_code = 0x30003, location = Self)]
    public entry fun test_cannot_resolve_twice_multi_step(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_cannot_resolve_twice_generic(endless_framework, governance, true);
    }

    #[test_only]
    public entry fun test_voting_passed_early_generic(endless_framework: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        timestamp::set_time_has_started_for_testing(endless_framework);
    ...
    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_voting_passed_early(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_early_generic(endless_framework, governance, false);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_voting_passed_early_multi_step(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_early_generic(endless_framework, governance, true);
    }
    ...
    public entry fun test_voting_passed_early_in_same_tx_should_fail(
        ...
        test_voting_passed_early_in_same_tx_should_fail_generic(endless_framework, governance, false);
    }
    ...
    public entry fun test_voting_passed_early_in_same_tx_should_fail_multi_step(
        ...
        test_voting_passed_early_in_same_tx_should_fail_generic(endless_framework, governance, true);
    }

    #[test_only]
    public entry fun test_voting_failed_generic(endless_framework: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        timestamp::set_time_has_started_for_testing(endless_framework);
    ...
    public entry fun test_voting_failed(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_generic(endless_framework, governance, false);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_multi_step(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_generic(endless_framework, governance, true);
    }
    ...
    #[test_only]
    public entry fun test_voting_failed_early_generic(endless_framework: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        timestamp::set_time_has_started_for_testing(endless_framework);
    ...
    #[test(endless_framework = @endless_framework, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_early(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_early_generic(endless_framework, governance, true);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_early_multi_step(endless_framework: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_early_generic(endless_framework, governance, false);
    }
    ...
    public entry fun test_cannot_set_min_threshold_higher_than_early_resolution(
        ...
        test_cannot_set_min_threshold_higher_than_early_resolution_generic(endless_framework, governance, false);
    }
    ...
    public entry fun test_cannot_set_min_threshold_higher_than_early_resolution_multi_step(
        ...
        test_cannot_set_min_threshold_higher_than_early_resolution_generic(endless_framework, governance, true);
    }

    #[test(endless_framework = @endless_framework, governance = @0x123)]
    public entry fun test_replace_execution_hash(endless_framework: &signer, governance: &signer) acquires VotingForum {
        account::create_account_for_test(@endless_framework);
        timestamp::set_time_has_started_for_testing(endless_framework);
