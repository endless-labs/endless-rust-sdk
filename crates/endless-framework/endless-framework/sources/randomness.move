...
module endless_framework::randomness {
    use std::hash;
    use std::option;
    ...
    const DST: vector<u8> = b"ENDLESS_RANDOMNESS";
    ...
    public(friend) fun on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) acquires PerBlockRandomness {
        ...
        if (exists<PerBlockRandomness>(@endless_framework)) {
            let randomness = borrow_global_mut<PerBlockRandomness>(@endless_framework);
    ...
    fun next_32_bytes(): vector<u8> acquires PerBlockRandomness {
        ...
        let input = DST;
        let randomness = borrow_global<PerBlockRandomness>(@endless_framework);
