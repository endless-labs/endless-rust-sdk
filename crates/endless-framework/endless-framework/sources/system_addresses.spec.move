spec endless_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Endless Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_endless_framework function ensures that the provided signer belongs to the
    /// @endless_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotEndlessFramework).
    ///
    /// No.: 3
    ...
    spec assert_endless_framework(account: &signer) {
        pragma opaque;
        include AbortsIfNotEndlessFramework;
    }
    ...
    spec schema AbortsIfNotEndlessFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @endless_framework with error::PERMISSION_DENIED;
    }
