...
module endless_framework::vesting {
    use std::bcs;
    use std::error;
    use endless_std::fixed_point64::{Self, FixedPoint64};
    use std::signer;
    use std::string::{utf8, String};
    use std::vector;

    use endless_std::pool_u128::{Self, Pool};
    use endless_std::simple_map::{Self, SimpleMap};

    use endless_framework::account::{Self, SignerCapability};
    use endless_framework::endless_account::{Self, assert_account_is_registered_for_eds};
    ...
    use endless_framework::fungible_asset::{Self, FungibleAsset};
    use endless_framework::primary_fungible_store;

    friend endless_framework::genesis;

    const VESTING_POOL_SALT: vector<u8> = b"endless_framework::vesting";
    ...
    ///
    /// This errors out if the vesting contract with the provided address doesn't exist.
    public fun remaining_grant(vesting_contract_address: address): u128 acquires VestingContract {
        assert_vesting_contract_exists(vesting_contract_address);
    ...
    ///
    /// This errors out if the vesting contract with the provided address doesn't exist.
    public fun total_accumulated_rewards(vesting_contract_address: address): u128 acquires VestingContract {
    ...
    public fun accumulated_rewards(
        vesting_contract_address: address, shareholder_or_beneficiary: address): u128 acquires VestingContract {
        assert_active_vesting_contract(vesting_contract_address);
        ...
        let vesting_contract = borrow_global<VestingContract>(vesting_contract_address);
        let shares = pool_u128::shares(&vesting_contract.grant_pool, shareholder);
        pool_u128::shares_to_amount_with_total_coins(&vesting_contract.grant_pool, shares, total_accumulated_rewards)
    }
    ...
    public fun shareholders(vesting_contract_address: address): vector<address> acquires VestingContract {
        ...
        let vesting_contract = borrow_global<VestingContract>(vesting_contract_address);
        pool_u128::shareholders(&vesting_contract.grant_pool)
    }
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, u128>,
        ...
        let grant = endless_coin::zero();
        let grant_amount = 0;
        let grant_pool = pool_u128::create(MAXIMUM_SHAREHOLDERS);
        vector::for_each_ref(shareholders, |shareholder| {
            let shareholder: address = *shareholder;
            let (_, buy_in_amount) = simple_map::remove(&mut buy_ins, &shareholder);
            fungible_asset::merge(&mut grant, fungible_asset::extract(&mut all_buyin_coins, (buy_in_amount as u128)));
            pool_u128::buy_in(
                &mut grant_pool,
                shareholder,
                buy_in_amount,
        ...
        assert!(fungible_asset::amount(&all_buyin_coins) == 0, 0);
        fungible_asset::destroy_zero(all_buyin_coins);
    ...
    public entry fun vest(contract_address: address) acquires VestingContract {
        ...
        let total_grant = pool_u128::total_coins(&vesting_contract.grant_pool);
        let vested_amount = fixed_point64::multiply_u128(total_grant, vesting_fraction);
        ...
        unlock_stake(vesting_contract, (vested_amount as u128));
    ...
    public entry fun distribute(contract_address: address) acquires VestingContract {
        ...
        let total_distribution_amount = fungible_asset::amount(&coins);
        if (total_distribution_amount == 0) {
            fungible_asset::destroy_zero(coins);
            return
        };

        // Distribute coins to all shareholders in the vesting contract.
        let grant_pool = &vesting_contract.grant_pool;
        let shareholders = &pool_u128::shareholders(grant_pool);
        vector::for_each_ref(shareholders, |shareholder| {
            let shareholder = *shareholder;
            let shares = pool_u128::shares(grant_pool, shareholder);
            let amount = pool_u128::shares_to_amount_with_total_coins(grant_pool, shares, total_distribution_amount);
            let share_of_coins = fungible_asset::extract(&mut coins, (amount as u128));
            let recipient_address = get_beneficiary(vesting_contract, shareholder);
            endless_account::deposit_coins(recipient_address, share_of_coins);
        });

        // Send any remaining "dust" (leftover due to rounding error) to the withdrawal address.
        if (fungible_asset::amount(&coins) > 0) {
            endless_account::deposit_coins(vesting_contract.withdrawal_address, coins);
        } else {
            fungible_asset::destroy_zero(coins);
        };
    ...
    public entry fun set_beneficiary(
        ...
        // Verify that the beneficiary account is set up to receive EDS. This is a requirement so distribute() wouldn't
        // fail and block all other accounts from receiving EDS if one beneficiary is not registered.
    ...
    fun create_vesting_contract_account(
        ...
        // Register the vesting contract account to receive EDS as it'll be sent to it when claiming unlocked stake from
        // the underlying staking contract.
        primary_fungible_store::ensure_primary_store_exists(signer::address_of(&account_signer), endless_coin::get_metadata());
    ...
    fun unlock_stake(vesting_contract: &VestingContract, amount: u128) {
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        staking_contract::unlock_stake(contract_signer, vesting_contract.staking.operator, amount);
    }

    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): FungibleAsset {
        ...
        let withdraw_coins = endless_coin::balance(contract_address);
        if (withdraw_coins != 0) {
            let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
            endless_coin::withdraw(contract_signer, withdraw_coins)
        } else {
            endless_coin::zero()
        }
    }
    ...
    #[test_only]
    use endless_framework::stake::with_rewards;
    ...
    #[test_only]
    const MIN_STAKE: u128 = 100000000000000; // 1M EDS coins with 8 decimals.

    #[test_only]
    const GRANT_AMOUNT: u128 = 20000000000000000; // 200M EDS coins with 8 decimals.
    ...
    #[test_only]
    public fun setup(endless_framework: &signer, accounts: &vector<address>) {
        use endless_framework::endless_account::create_account;

        stake::initialize_for_test_custom(endless_framework, MIN_STAKE, GRANT_AMOUNT * 10, 3600, true, 10, 10000, 1000000);
        ...
        std::features::change_feature_flags(endless_framework, vector[MODULE_EVENT, OPERATOR_BENEFICIARY_CHANGE], vector[]);
    }
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let schedule = vector::empty<FixedPoint64>();
        vector::for_each_ref(vesting_numerators, |num| {
            vector::push_back(&mut schedule, fixed_point64::create_from_rational((*num as u128), (vesting_denominator as u128)));
        });
        ...
        let admin_address = signer::address_of(admin);
        let buy_ins = simple_map::create<address, u128>();
        let all_buy_ins_coin = endless_coin::zero();
        vector::enumerate_ref(shares, |i, share| {
            let shareholder = *vector::borrow(shareholders, i);

            if (*share != 0) {
                let fa = stake::mint_coins(*share);
                fungible_asset::merge(&mut all_buy_ins_coin, fa);
            };
            simple_map::add(&mut buy_ins, shareholder, *share);
        });
    ...
    #[test(endless_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            endless_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
        ...
        distribute(contract_address);
        let shareholder_1_bal = endless_coin::balance(shareholder_1_address);
        let shareholder_2_bal = endless_coin::balance(shareholder_2_address);
        // Distribution goes by the shares of the vesting contract.
        assert!(shareholder_1_bal == rewards / 4, (shareholder_1_bal as u64));
        assert!(shareholder_2_bal == rewards * 3 / 4, (shareholder_2_bal as u64));
        ...
        assert!(remaining_grant(contract_address) == remaining_grant, (remaining_grant(contract_address) as u64));
        ...
        assert!(endless_coin::balance(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(endless_coin::balance(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(endless_coin::balance(withdrawal_address) == 1, 0);
        ...
        let previous_bal = endless_coin::balance(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(endless_coin::balance(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }

    #[test(endless_framework = @0x1, admin = @0x123)]
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(endless_framework, &vector[admin_address]);
    ...
    #[test(endless_framework = @0x1, admin = @0x123)]
    #[expected_failure(abort_code = 0x60001, location = endless_framework::endless_account)]
    ...
    #[test(endless_framework = @0x1, admin = @0x123)]
    #[expected_failure(abort_code = 0x60002, location = endless_framework::endless_account)]
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(endless_framework: &signer) {
        setup(endless_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(endless_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(endless_framework: &signer) {
        setup(endless_framework, &vector[]);
        create_vesting_schedule(vector[fixed_point64::create_from_rational(1, 1)], 1, 0);
    }

    #[test(endless_framework = @0x1, admin = @0x123)]
    #[expected_failure(abort_code = 0x10006, location = Self)]
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(endless_framework: &signer) {
        setup(endless_framework, &vector[]);
        timestamp::update_global_time_for_test_secs(1000);
        create_vesting_schedule(
            vector[fixed_point64::create_from_rational(1, 1)],
            900,
            1);
    }

    #[test(endless_framework = @0x1, admin = @0x123, shareholder = @0x234)]
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(endless_framework, &vector[admin_address, shareholder_address]);
    ...
    #[test(endless_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(endless_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(endless_coin::balance(shareholder_address) == staker_rewards + 1, 0);
        assert!(endless_coin::balance(operator_address) == commission - 1, 1);
    }
    ...
    #[test(endless_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(endless_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_commission_percentage_change(
        ...
        assert!(endless_coin::balance(operator_address) == expected_commission, 1);
    }

    #[test(endless_framework = @0x1, admin = @0x123, shareholder = @0x234, operator1 = @0x345, beneficiary = @0x456, operator2 = @0x567)]
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(endless_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        ...
        assert!(endless_coin::balance(operator_address1) == 0, 1);
        assert!(endless_coin::balance(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = endless_coin::balance(beneficiary_address);
        ...
        assert!(endless_coin::balance(operator_address2) >= expected_commission, 1);
        assert!(endless_coin::balance(beneficiary_address) == old_beneficiay_balance, 1);
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(endless_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = endless_coin::balance(@11);
        assert!(balance == vested_amount, (balance as u64));
    }
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(endless_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(endless_coin::balance(@11) == vested_amount, 0);
        assert!(endless_coin::balance(@12) == 0, 1);
    }

    #[test(endless_framework = @0x1, admin = @0x123, resetter = @0x234)]
    ...
    #[test(endless_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
    ...
    #[test_only]
    fun get_accumulated_rewards(contract_address: address): u128 acquires VestingContract {
        let vesting_contract = borrow_global<VestingContract>(contract_address);
    ...
    #[test_only]
    fun fraction(total: u128, numerator: u64, denominator: u64): u128 {
        fixed_point64::multiply_u128(total, fixed_point64::create_from_rational((numerator as u128), (denominator as u128)))
    }
}
