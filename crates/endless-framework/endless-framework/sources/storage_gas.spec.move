spec endless_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@endless_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@endless_framework);
    }
    ...
    spec set_config(endless_framework: &signer, config: StorageGasConfig) {
        include system_addresses::AbortsIfNotEndlessFramework{ account: endless_framework };
        aborts_if !exists<StorageGasConfig>(@endless_framework);
    }
    ...
    spec initialize(endless_framework: &signer) {
        include system_addresses::AbortsIfNotEndlessFramework{ account: endless_framework };
        aborts_if exists<StorageGasConfig>(@endless_framework);
        aborts_if exists<StorageGas>(@endless_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@endless_framework);
        ensures exists<StorageGas>(@endless_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@endless_framework);
        aborts_if !exists<StorageGas>(@endless_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@endless_framework);
    }
