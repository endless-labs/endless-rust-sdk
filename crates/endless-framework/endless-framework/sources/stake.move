...
module endless_framework::stake {
    use std::error;
    use std::features;
    ...
    use endless_std::math128::{Self, min};
    use endless_std::table::{Self, Table};
    use endless_framework::endless_coin::{Self};
    use endless_framework::fungible_asset::{FungibleAsset, FungibleStore, Metadata, Self, MintRef, TransferRef};
    ...
    use endless_framework::staking_config::{Self, StakingConfig};
    use endless_framework::chain_status;
    use endless_std::bls12381::proof_of_possession_from_bytes;
    use endless_framework::reconfiguration_state;
    use endless_framework::validator_consensus_info::{Self, ValidatorConsensusInfo};
    ...
    friend endless_framework::transaction_fee;
    ...
    const EREINVALID_FA_TYPE: u64 = 21;
    /// split shares sum cannot above 100%
    const ESHARES_ABOVE_LIMIT: u64 = 22;
    ...
    const REWARD_DENOMINATOR: u64 = 100;
    ...
    struct EndlessCoinCapabilities has key {
        // mint_cap: MintCapability<EndlessCoin>,
        mint_cap: MintRef,
    ...
    struct AccountShare has store {
        account_address: address,
        percent: u64,
    }

    /// block reward split into [validator, other_accounts]
    struct BlockRewardShareInfo has key {
        accounts: vector<AccountShare>,
    }
    ...
    public(friend) fun initialize_validator_fees(endless_framework: &signer) {
        system_addresses::assert_endless_framework(endless_framework);
        assert!(
            !exists<ValidatorFees>(@endless_framework),
            error::already_exists(EFEES_TABLE_ALREADY_EXISTS)
        );
        move_to(endless_framework, ValidatorFees { fees_table: table::new() });
    }

    /// Stores the transaction fee collected to the specified validator address.
    public(friend) fun add_transaction_fee(validator_addr: address, fee: FungibleAsset) acquires ValidatorFees {
        let fees_table = &mut borrow_global_mut<ValidatorFees>(@endless_framework).fees_table;
        let amount = fungible_asset::amount(&fee);

        // for now, only accept "EDS" FA as gas fee
        assert!(endless_coin::is_true_EDS(&fee), error::invalid_argument(EREINVALID_FA_TYPE));
        if (table::contains(fees_table, validator_addr)) {
            ...
            *collected_fee = *collected_fee + amount;
            ...
            table::add(fees_table, validator_addr, amount);
        };

        deposit_to_pool(fee);
    }

    fun deposit_to_pool(fee: FungibleAsset) {
        let pool_addr = primary_fungible_store::ensure_primary_store_exists(@endless_framework, endless_coin::get_metadata());
        fungible_asset::deposit<FungibleStore>(pool_addr, fee);
    }
    #[view]
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@endless_framework);
    ...
    public fun get_current_epoch_voting_power(pool_address: address): u128 acquires StakePool, ValidatorSet {
        ...
        if (validator_state == VALIDATOR_STATUS_ACTIVE || validator_state == VALIDATOR_STATUS_PENDING_INACTIVE) {
            let active_stake = borrow_global<StakePool>(pool_address).active;
            let pending_inactive_stake = borrow_global<StakePool>(pool_address).pending_inactive;
    ...
    public fun get_current_epoch_proposal_counts(validator_index: u64): (u64, u64) acquires ValidatorPerformance {
        let validator_performances = &borrow_global<ValidatorPerformance>(@endless_framework).validators;
    ...
    public(friend) fun initialize(endless_framework: &signer) {
        ...
        move_to(endless_framework, ValidatorSet {
            consensus_scheme: 0,
        ...
        move_to(endless_framework, ValidatorPerformance {
            validators: vector::empty(),
        });
    }

    /// Initialize validator set to the core resource account.
    public(friend) fun initialize_reward_split(endless_framework: &signer, share_info: vector<AccountShare>) {
        system_addresses::assert_endless_framework(endless_framework);

        // guarantee all account shares sum cannot above 100
        let all_shares = 0;
        vector::for_each_ref(&share_info, |info| {
            let share_info: &AccountShare = info;
            all_shares = all_shares + share_info.percent;
        });
        assert!((all_shares < REWARD_DENOMINATOR), ESHARES_ABOVE_LIMIT);

        move_to(endless_framework, BlockRewardShareInfo {
            accounts: share_info,
        });
    }
    ...
    public(friend) fun store_endless_coin_cap(endless_framework: &signer, mint_cap: MintRef, transfer_cap: TransferRef) {
        system_addresses::assert_endless_framework(endless_framework);
    ...
    public fun remove_validators(
        ...
        system_addresses::assert_endless_framework(endless_framework);
        let validator_set = borrow_global_mut<ValidatorSet>(@endless_framework);
    ...
    public entry fun add_stake(owner: &signer, amount: u128) acquires OwnerCapability, StakePool, ValidatorSet {
        let owner_address = signer::address_of(owner);
        assert_owner_cap_exists(owner_address);
        if (amount != 0) {
            let ownership_cap = borrow_global<OwnerCapability>(owner_address);
            let fa = endless_coin::withdraw(owner, amount);
            add_stake_with_cap(ownership_cap, fa);
        }
    }

    /// Add `coins` into `pool_address`. this requires the corresponding `owner_cap` to be passed in.
    public fun add_stake_with_cap(owner_cap: &OwnerCapability, fa: FungibleAsset) acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        // for now, only accept "EDS" FA as gas fee
        let is_true_EDS = endless_coin::is_true_EDS(&fa);
        assert!(is_true_EDS, error::invalid_argument(EREINVALID_FA_TYPE));
        ...
        // desposit to stake pool(actually under endless_framework)
        deposit_to_pool(fa);
    ...
    public entry fun reactivate_stake(owner: &signer, amount: u128) acquires OwnerCapability, StakePool {
        assert_reconfig_not_in_progress();
        let owner_address = signer::address_of(owner);
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
    ...
    public fun reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u128) acquires StakePool {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        ...
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let total_pending_inactive = stake_pool.pending_inactive;
    ...
    public entry fun unlock(owner: &signer, amount: u128) acquires OwnerCapability, StakePool {
        assert_reconfig_not_in_progress();
        let owner_address = signer::address_of(owner);
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
    ...
    public fun unlock_with_cap(amount: u128, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        let amount = min(amount, stake_pool.active);
        stake_pool.active = stake_pool.active - amount;
        stake_pool.pending_inactive = stake_pool.pending_inactive + amount;
    ...
    ) acquires OwnerCapability, StakePool, ValidatorSet, EndlessCoinCapabilities {
        let owner_address = signer::address_of(owner);
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
        let fa = withdraw_with_cap(ownership_cap, withdraw_amount);
        let store = primary_fungible_store::ensure_primary_store_exists(owner_address, endless_coin::get_metadata());
        fungible_asset::deposit<FungibleStore>(store, fa);
    }
    ...
    public fun withdraw_with_cap(
        ...
        withdraw_amount: u128
    ): FungibleAsset acquires StakePool, ValidatorSet, EndlessCoinCapabilities {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (get_validator_state(pool_address) == VALIDATOR_STATUS_INACTIVE &&
            ...
            let pending_inactive_stake = stake_pool.pending_inactive;
        ...
        let new_withdraw_amount = min(withdraw_amount, stake_pool.inactive);
        let meta = endless_coin::get_metadata();
        if (new_withdraw_amount == 0) return fungible_asset::zero<Metadata>(meta);

        let store = primary_fungible_store::ensure_primary_store_exists(@endless_framework, endless_coin::get_metadata());
        let transfer_cap = &borrow_global<EndlessCoinCapabilities>(@endless_framework).transfer_cap;

        let fa = fungible_asset::withdraw_with_ref(transfer_cap, store, new_withdraw_amount);
    ...
    public(friend) fun update_performance_statistics(proposer_index: Option<u64>, failed_proposer_indices: vector<u64>) acquires ValidatorPerformance {
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@endless_framework);
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, EndlessCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees, BlockRewardShareInfo {
        let validator_set = borrow_global_mut<ValidatorSet>(@endless_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@endless_framework);
        ...
        vector::for_each_ref(&validator_set.active_validators, |validator| {
            ...
            let stake_pool = borrow_global<StakePool>(validator.addr);
        ...
        let eds_total_supply = endless_coin::supply();

        // Process pending stake and distribute transaction fees and rewards for each currently active validator.
        vector::for_each_ref(&validator_set.active_validators, |validator| {
            let validator: &ValidatorInfo = validator;
            update_stake_pool(validator_perf, validator.addr, &config, total_stake_amount, eds_total_supply);
        });
    ...
    public fun next_validator_consensus_infos(): vector<ValidatorConsensusInfo> acquires ValidatorSet, ValidatorPerformance, StakePool, ValidatorFees, ValidatorConfig {
        // Init.
        let cur_validator_set = borrow_global<ValidatorSet>(@endless_framework);
        let staking_config = staking_config::get();
        let validator_perf = borrow_global<ValidatorPerformance>(@endless_framework);
        ...
        let num_cur_pending_inactives = vector::length(&cur_validator_set.pending_inactive);
        let num_candidates = num_cur_actives + num_cur_pending_actives;

        let total_stake_amount = 0;
        let i = 0;
        while (i < num_cur_actives) {
            let candidate = vector::borrow(&cur_validator_set.active_validators, i);
        ...
        let i = 0;
        while (i < num_cur_pending_inactives) {
            let candidate = vector::borrow(&cur_validator_set.pending_inactive, i);
            if (!vector::contains(&cur_validator_set.active_validators, candidate)) {
                let stake_pool = borrow_global<StakePool>(candidate.addr);
                total_stake_amount = total_stake_amount + stake_pool.active + stake_pool.pending_active + stake_pool.pending_inactive;
            };
            i = i + 1;
        ...
        while ({
            ...
            let stake_pool = borrow_global<StakePool>(candidate.addr);
            let cur_active = stake_pool.active;
            let cur_pending_active = stake_pool.pending_active;
            let cur_pending_inactive = stake_pool.pending_inactive;
            ...
            let cur_reward = if (candidate_in_current_validator_set && cur_active > 0) {
                ...
                calculate_rewards_amount(cur_active, total_stake_amount,
            ...
            if (features::collect_and_distribute_gas_fees()) {
                let fees_table = &borrow_global<ValidatorFees>(@endless_framework).fees_table;
    ...
    fun update_stake_pool(
    ...
    ) acquires StakePool, EndlessCoinCapabilities, ValidatorConfig, ValidatorFees, BlockRewardShareInfo {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
        ...
        if (get_reconfig_start_time_secs() >= current_lockup_expiration) {
            let all_pending_inactive = stake_pool.pending_inactive;
            stake_pool.pending_inactive = 0;
    ...
    fun calculate_rewards_amount(
        ...
        if (features::eds_supply_inflation_enabled()) {
            let rewards_numerator = (stake_amount as u128) * (rewards_rate as u128) * (num_successful_proposals as u128);
            let rewards_denominator = (all_stake_amount as u128) * (rewards_rate_denominator as u128) * (num_total_proposals as u128);
            if (rewards_denominator > 0) {
                // The rewards amount is equal to (EDS_supply * rewards rate * (stake amount/all_stake_amount) * performance multiplier).
                // We do multiplication in u128 before division to avoid the overflow and minimize the rounding error.
                let result = std::math128::mul_div(total_supply, (rewards_numerator as u128), (rewards_denominator as u128));
            ...
            if (rewards_denominator > 0) {
                let result = (rewards_numerator / rewards_denominator);
                return result
            };
        };
    ...
    fun distribute_rewards(
        ...
        rewards_rate_denominator: u64,
        eds_total_supply: u128,
    ): u128 acquires EndlessCoinCapabilities, BlockRewardShareInfo {
        let stake_amount = *stake;
        let rewards_amount = if (stake_amount > 0) {
            calculate_rewards_amount(stake_amount, total_stake_amount, num_successful_proposals, num_total_proposals, rewards_rate, rewards_rate_denominator, eds_total_supply)
        } else {
            0
        };

        if (rewards_amount > 0) {
            if (features::reward_split_enabled() && exists<BlockRewardShareInfo>(@endless_framework)) {
                // split total_reward into three parts: [validator, other_accounts]
                let shares_info = &borrow_global<BlockRewardShareInfo>(@endless_framework).accounts;
                let acc_shares_sum = 0;

                let mint_cap = &borrow_global<EndlessCoinCapabilities>(@endless_framework).mint_cap;

                // mint for other accounts
                vector::for_each_ref(shares_info, |account|{
                    let acc: &AccountShare = account;
                    let reward_of_acc = math128::mul_div(rewards_amount, (acc.percent as u128), (REWARD_DENOMINATOR as u128));
                    let fa = fungible_asset::mint(mint_cap, reward_of_acc);
                    let acc_a_store = primary_fungible_store::ensure_primary_store_exists(acc.account_address, endless_coin::get_metadata());
                    fungible_asset::deposit<FungibleStore>(acc_a_store, fa);
                ...
                let validator_nominator = REWARD_DENOMINATOR - acc_shares_sum;
                let validator_reward = math128::mul_div(rewards_amount, (validator_nominator as u128), (REWARD_DENOMINATOR as u128));

                let rewards = fungible_asset::mint(mint_cap, validator_reward);
                deposit_to_pool(rewards);
                *stake = *stake + validator_reward;

                validator_reward
            } else {
                let validator_reward = rewards_amount;
                let mint_cap = &borrow_global<EndlessCoinCapabilities>(@endless_framework).mint_cap;
                let rewards = fungible_asset::mint(mint_cap, validator_reward);
                deposit_to_pool(rewards);
                *stake = *stake + validator_reward;
    ...
    fun get_next_epoch_voting_power(stake_pool: &StakePool): u128 {
        let value_pending_active = stake_pool.pending_active;
        let value_active = stake_pool.active;
        let value_pending_inactive = stake_pool.pending_inactive;
        value_pending_active + value_active + value_pending_inactive
    }

    fun update_voting_power_increase(increase_amount: u128) acquires ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@endless_framework);
    ...
    public fun configure_allowed_validators(endless_framework: &signer, accounts: vector<address>) acquires AllowedValidators {
        let endless_framework_address = signer::address_of(endless_framework);
        system_addresses::assert_endless_framework(endless_framework);
        if (!exists<AllowedValidators>(endless_framework_address)) {
            move_to(endless_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(endless_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@endless_framework)) {
            true
        } else {
            let allowed = borrow_global<AllowedValidators>(@endless_framework);
    ...
    public fun initialize_for_test(endless_framework: &signer) {
        reconfiguration_state::initialize(endless_framework);
        initialize_for_test_custom(endless_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
    }

    #[test_only]
    public fun initialize_for_test_with_million_stake(endless_framework: &signer) {
        reconfiguration_state::initialize(endless_framework);
        initialize_for_test_custom(endless_framework, 1000000 * ONE_EDS, 10000000 * ONE_EDS, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
    }
    ...
    ) acquires EndlessCoinCapabilities, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees, BlockRewardShareInfo {
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration_state::initialize(endless_framework);
        if (!exists<ValidatorSet>(@endless_framework)) {
        ...
        if (!exists<EndlessCoinCapabilities>(@endless_framework)) {
            let (mint_cap, burn_cap, transfer_cap) = endless_coin::initialize_for_test(endless_framework);
            store_endless_coin_cap(endless_framework, mint_cap, transfer_cap);
            fungible_asset::destroy_burn_cap(burn_cap);
        };
    }

    // This function assumes the stake module already the capability to mint endless coins.
    #[test_only]
    public fun mint_coins(amount: u128): FungibleAsset acquires EndlessCoinCapabilities {
        let mint_cap = &borrow_global<EndlessCoinCapabilities>(@endless_framework).mint_cap;
        fungible_asset::mint(mint_cap, amount)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u128) acquires EndlessCoinCapabilities {
        let fa = mint_coins(amount);
        primary_fungible_store::deposit(signer::address_of(account), fa);
    }

    #[test_only]
    public fun mint_and_add_stake(account: &signer, amount: u128)
        acquires EndlessCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, EndlessCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees, BlockRewardShareInfo {
    ...
    public fun create_stake_pool(
        account: &signer,
        active: FungibleAsset,
        pending_inactive: FungibleAsset,
        locked_until_secs: u64,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        assert!(endless_coin::is_true_EDS(&active), 0);
        assert!(endless_coin::is_true_EDS(&pending_inactive), 0);
        ...
        stake_pool.active = stake_pool.active + fungible_asset::amount(&active);
        stake_pool.pending_inactive = stake_pool.pending_inactive + fungible_asset::amount(&pending_inactive);
        stake_pool.locked_until_secs = locked_until_secs;

        let pool_addr = borrow_global<OwnerCapability>(account_address).pool_address;
        let pool_coins_store = primary_fungible_store::ensure_primary_store_exists(pool_addr, endless_coin::get_metadata());
        fungible_asset::deposit(pool_coins_store, active);
        fungible_asset::deposit(pool_coins_store, pending_inactive);
    }
    ...
    #[test(endless_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(endless_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(endless_framework = @endless_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        endless_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, EndlessCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees, BlockRewardShareInfo {
        initialize_for_test(endless_framework);
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100, true, true);
        ...
        assert!(endless_coin::balance(validator_address) == 800, 2);
        ...
        assert!(primary_fungible_store::balance(validator_address, endless_coin::get_metadata()) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(primary_fungible_store::balance(validator_address, endless_coin::get_metadata()) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(endless_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(endless_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(endless_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(endless_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        let (_sk_1, pk_1, pop_1) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@endless_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@endless_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        features::change_feature_flags(endless_framework, vector[features::get_periodical_reward_rate_decrease_feature(), features::get_eds_supply_inflation_feature()], vector[]);

        initialize_for_test_with_million_stake(endless_framework);
        // Initial balance = 9M (idle) + 1M (staked) = 10M.
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 1000000 * ONE_EDS, true, true);
        mint(validator, 9000000 * ONE_EDS);

        // Validator unlocks stake.
        unlock(validator, 1000000 * ONE_EDS);
        // Enough time has passed so the stake is fully unlocked.
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();

        // inactive stake is about 1M + total_EDS(10M) * 0.00002176 = 21,760,000,000
        let validator_address = signer::address_of(validator);
        assert_validator_state(validator_address,0, 100021759978240, 0, 0, 0);

        // Validator can only withdraw a max of 100 unlocked coins even if they request to withdraw more than 100.
        withdraw(validator, 2000000 * ONE_EDS);
        let validator_address = signer::address_of(validator);
        // Receive back all coins with an extra 1 for rewards.
        let acc_balance = primary_fungible_store::balance(validator_address, endless_coin::get_metadata());

        assert!(acc_balance == 1000021759978240, 2);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(endless_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(endless_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@endless_framework).pending_inactive, 0).addr == validator_2_address, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@endless_framework).pending_active, 0).addr == validator_3_address, 0);
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@endless_framework).active_validators, 0).config.consensus_pubkey == pk_1_bytes, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@endless_framework).active_validators, 0).config.consensus_pubkey == pk_1b_bytes, 0);
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        assert!(fungible_asset::amount(&coins) == 101, 1);
        ...
        // Cleanups.
        primary_fungible_store::deposit(pool_address, coins);
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(endless_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    #[test(
        endless_framework = @endless_framework,
        validator_1 = @endless_framework,
        validator_2 = @0x2,
        validator_3 = @0x3,
        validator_4 = @0x4,
        validator_5 = @0x5
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        end_epoch();
        let vci_vec_0 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@endless_framework));
        ...
        assert!(vector[@0x5, @endless_framework, @0x3] == vci_addrs, 1);
        ...
        let vci_vec_1 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@endless_framework));
        assert!(vci_vec_0 == vci_vec_1, 11);
        join_validator_set(validator_2, v2_addr);
        let vci_vec_2 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@endless_framework));
        assert!(vci_vec_0 == vci_vec_2, 12);
        leave_validator_set(validator_1, v1_addr);
        let vci_vec_3 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@endless_framework));
        assert!(vci_vec_0 == vci_vec_3, 13);
        join_validator_set(validator_4, v4_addr);
        let vci_vec_4 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@endless_framework));
    ...
    #[test(endless_framework = @endless_framework, validator_1 = @0x123, validator_2 = @0x234)]
    public entry fun test_mainnet_validator_rewards_rate_decrease_over_time(
        endless_framework: &signer,
        validator_1: &signer,
        validator_2: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, EndlessCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees,BlockRewardShareInfo {
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 1000000 * ONE_EDS, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 10000000 * ONE_EDS, true, true);

        // One epoch passed, Total reward is about 11M EDS * 0.00002176 = 240 EDS
        // Validator 1 and validator 2 should receive rewards with 1/11(21.8) and 10/11(218.2) of total reward.
        end_epoch();
        assert_validator_state(validator_1_address, 100002175997824, 0, 0, 0, 1);
        assert_validator_state(validator_2_address, 1000021759978240, 0, 0, 0, 0);
        ...
        // Reward rate will decrease to previus_rate*0.8 at 2th epoch of the year.
        timestamp::fast_forward_seconds(one_year_in_secs);
        end_epoch();

        // validator1: 100002175997824 + 21.8 EDS = 100004352042997
        // validator2: 1000021759978240 + 218.2 EDS = 1000043520429977
        assert_validator_state(validator_1_address, 100004352042997, 0, 0, 0, 1);
        assert_validator_state(validator_2_address, 1000043520429977, 0, 0, 0, 0);

        end_epoch();

        // now the total EDS supply = 1,100,047,872,472,974
        // the reward_rate decrease to 0.8 * 0.00002176  = 0.00001741
        // rewards is about: 11000478 EDS * 0.00001741 = 191.5 EDS
        // Validator 1 and validator 2 should receive rewards with 1/11(17.4) and 10/11(174) of total reward.
        assert_validator_state(validator_1_address, 100004352042997 + 1740874019, 0, 0, 0, 1);
        assert_validator_state(validator_2_address, 1000043520429977 + 17408740194, 0, 0, 0, 0);
    }
    ...
    public entry fun test_invalid_config(
        endless_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, EndlessCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(endless_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@endless_framework).active_validators) == 2, 0);
        ...
        remove_validators(endless_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@endless_framework).active_validators) == 1, 0);
        assert!(get_validator_state(validator_to_remove) == VALIDATOR_STATUS_PENDING_INACTIVE, 1);
    }

    #[test_only]
    public fun end_epoch() acquires StakePool, EndlessCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees, BlockRewardShareInfo{
    ...
    public fun assert_stake_pool(
        ...
        active_stake: u128,
        inactive_stake: u128,
        pending_active_stake: u128,
        pending_inactive_stake: u128,
    ) acquires StakePool {
        let stake_pool = borrow_global<StakePool>(pool_address);
        let actual_active_stake = stake_pool.active;
        let actual_inactive_stake = stake_pool.inactive;
        let actual_pending_active_stake = stake_pool.pending_active;
        let actual_pending_inactive_stake = stake_pool.pending_inactive;

        assert!(actual_active_stake == active_stake, (actual_active_stake as u64));
        assert!(actual_inactive_stake == inactive_stake, (actual_inactive_stake as u64));
        assert!(actual_pending_active_stake == pending_active_stake, (actual_pending_active_stake as u64));
        assert!(actual_pending_inactive_stake == pending_inactive_stake, (actual_pending_inactive_stake as u64));
    }
    ...
    #[test(endless_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        endless_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        configure_allowed_validators(endless_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(endless_framework, vector[]);
    ...
    #[test_only]
    public fun with_rewards(amount: u128): u128 {
        let (numerator, denominator) = staking_config::get_reward_rate(&staking_config::get());
        amount + amount * (numerator as u128) / (denominator as u128)
    }

    #[test_only]
    public fun get_validator_fee(validator_addr: address): u128 acquires ValidatorFees {
        let fees_table = &borrow_global<ValidatorFees>(@endless_framework).fees_table;
    ...
    fun test_distribute_validator_fees(
        ...
        features::change_feature_flags(endless_framework, vector[COLLECT_AND_DISTRIBUTE_GAS_FEES], vector[]);
        assert!(features::collect_and_distribute_gas_fees(), 0);

        // Initialize staking and validator fees table.
        initialize_for_test(endless_framework);
        initialize_validator_fees(endless_framework);
    ...
    fun test_calc_block_reward(
        ...
        // Make sure that PeriodicalRewardRateReduction is enabled.
        features::change_feature_flags(endless_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);
        ...
        let (_sk_3, pk_3, pop_3) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 1000000 * ONE_EDS, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 1000000 * ONE_EDS, true, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 1000000 * ONE_EDS, true, true);

        // make the test distinct from `get_eds_supply_inflation_feature`
        mint(validator_1, 1000000 * ONE_EDS);

        assert_validator_state(validator_1_address, 1000000 * ONE_EDS, 0, 0, 0, 2);
        assert_validator_state(validator_2_address, 1000000 * ONE_EDS, 0, 0, 0, 1);
        assert_validator_state(validator_3_address, 1000000 * ONE_EDS, 0, 0, 0, 0);

        end_epoch();

        // Epoch ended. Validators must have recieved their rewards
        // without get_eds_supply_inflation_feature feture, each validator got reward according to it's stake share
        // ie. 1M EDS * 0.00002176 = about 21.76 EDS (2175997824)
        assert_validator_state(validator_1_address, 100002175997824, 0, 0, 0, 2);
        assert_validator_state(validator_2_address, 100002175997824, 0, 0, 0, 1);
        assert_validator_state(validator_3_address, 100002175997824, 0, 0, 0, 0);
    }

    #[test_only]
    const EDS_SUPPLY_INFLATION: u64 = 58;

    #[test(endless_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    fun test_block_reward_with_eds_inflation(
        endless_framework: &signer,
        validator_1: &signer,
        validator_2: &signer,
        validator_3: &signer,
        ...
        features::change_feature_flags(endless_framework, vector[features::get_periodical_reward_rate_decrease_feature(), features::get_eds_supply_inflation_feature()], vector[]);
        assert!(features::periodical_reward_rate_decrease_enabled(), 0);
        assert!(features::eds_supply_inflation_enabled(), 0);
        ...
        // Epoch ended. Validators must have recieved their rewards
        // Total rewards is: 4M EDS * 0.00002176
        // each validator got 1/3 of total_rewards, due to same performance and same stake amount
        // ie. 4/3 M EDS * 0.00002176 = about 29.014 EDS (2901330432)
        assert_validator_state(validator_1_address, 100002901330432, 0, 0, 0, 2);
        assert_validator_state(validator_2_address, 100002901330432, 0, 0, 0, 1);
        assert_validator_state(validator_3_address, 100002901330432, 0, 0, 0, 0);
    }
}
