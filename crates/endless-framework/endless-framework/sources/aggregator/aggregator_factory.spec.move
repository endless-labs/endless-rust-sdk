spec endless_framework::aggregator_factory {
    use endless_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the Endless framework is the caller and that the
    /// AggregatorFactory resource will move under the Endless framework account.
    /// Criticality: High
    ...
    /// resource transfers to the Endless framework account.
    ...
    /// under the Endless account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the Endless
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the Endless framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the Endless framework
    /// address.
    ...
    spec initialize_aggregator_factory(endless_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@endless_framework);
    }
