...
module endless_framework::endless_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use endless_std::math128::min;
    use endless_std::simple_map::{Self, SimpleMap};
    use endless_std::smart_table::{Self, SmartTable};
    use endless_std::table::{Self, Table};

    use endless_framework::account::{SignerCapability, create_signer_with_capability};
    use endless_framework::event::Self;
    use endless_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    use endless_framework::reconfiguration_with_dkg;
    use endless_framework::timestamp;
    use endless_framework::voting;
    ...
    struct VotingRecordsV2 has key {
        votes: SmartTable<RecordKey, u128>
    }
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@endless_framework)) {
            move_to(endless_framework, GovernanceResponsbility { signer_caps: simple_map::create<address, SignerCapability>() });
        };

        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@endless_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(endless_framework);
        move_to(endless_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(endless_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(endless_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@endless_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(endless_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@endless_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@endless_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@endless_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@endless_framework);
    ...
    public fun get_remaining_voting_power(stake_pool: address, proposal_id: u64): u128 acquires VotingRecords, VotingRecordsV2 {
        assert_voting_initialization();

        let proposal_expiration = voting::get_proposal_expiration_secs<GovernanceProposal>(@endless_framework, proposal_id);
        ...
        let used_voting_power = 0u128;
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global<VotingRecordsV2>(@endless_framework);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@endless_framework);
        ...
        let total_voting_token_supply = endless_coin::supply();
        let early_resolution_vote_threshold = option::none<u128>();
        if (total_voting_token_supply != 0) {
            // 50% + 1 to avoid rounding errors.
            early_resolution_vote_threshold = option::some(total_voting_token_supply / 2 + 1);
        };
    ...
    public entry fun vote(
        ...
        vote_internal(voter, stake_pool, proposal_id, MAX_U128, should_pass);
    }
    ...
    fun vote_internal(
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global_mut<VotingRecordsV2>(@endless_framework);
            ...
            *used_voting_power = *used_voting_power + voting_power;
            ...
            let voting_records = borrow_global_mut<VotingRecords>(@endless_framework);
            assert!(
                !table::contains(&voting_records.votes, record_key),
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@endless_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@endless_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@endless_framework, proposal_id);
    ...
    public fun resolve(proposal_id: u64, signer_address: address): signer acquires ApprovedExecutionHashes, GovernanceResponsbility {
        voting::resolve<GovernanceProposal>(@endless_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(proposal_id: u64, signer_address: address, next_execution_hash: vector<u8>): signer acquires GovernanceResponsbility, ApprovedExecutionHashes {
        voting::resolve_proposal_v2<GovernanceProposal>(@endless_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@endless_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@endless_framework).hashes;
    ...
    public fun reconfigure(endless_framework: &signer) {
        system_addresses::assert_endless_framework(endless_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            reconfiguration_with_dkg::try_start();
        } else {
            reconfiguration_with_dkg::finish(endless_framework);
        }
    }
    ...
    public fun force_end_epoch(endless_framework: &signer) {
        system_addresses::assert_endless_framework(endless_framework);
        reconfiguration_with_dkg::finish(endless_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(core_resources: &signer) acquires GovernanceResponsbility {
        let endless_framework = get_signer_testnet_only(core_resources, @endless_framework);
        system_addresses::assert_endless_framework(&endless_framework);
        reconfiguration_with_dkg::finish(&endless_framework);
    }

    /// Update feature flags and also trigger reconfiguration.
    public fun toggle_features(endless_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_endless_framework(endless_framework);
        features::change_feature_flags_for_next_epoch(endless_framework, enable, disable);
        reconfigure(endless_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(endless_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }

    #[view]
    /// Return the voting power a stake pool has with respect to governance proposals.
    public fun get_voting_power(pool_address: address): u128 {
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@endless_framework);
    ...
    fun assert_voting_initialization() {
        if (features::partial_governance_voting_enabled()) {
            assert!(exists<VotingRecordsV2>(@endless_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
        };
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_voting(&endless_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(endless_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&endless_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@endless_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @endless_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @endless_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@endless_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@endless_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(endless_framework = @endless_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(endless_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(endless_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(endless_framework = @endless_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code=0x5000a, location = endless_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(endless_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(endless_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<GovernanceProposal>(@endless_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@endless_framework, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<GovernanceProposal>(@endless_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@endless_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@endless_framework).hashes;
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting(
        ...
        test_can_remove_approved_hash_if_executed_directly_via_voting_generic(endless_framework, proposer, yes_voter, no_voter, false);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_multi_step(
        ...
        test_can_remove_approved_hash_if_executed_directly_via_voting_generic(endless_framework, proposer, yes_voter, no_voter, true);
    }

    #[test(endless_framework = @endless_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 0x10004, location = endless_framework::voting)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&endless_framework, &proposer, &voter_1, &voter_2);
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&endless_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        ...
        test_resolving_proposal_generic(endless_framework, true, execution_hash);
    }
    ...
    public entry fun test_stake_pool_can_vote_before_and_after_partial_governance_voting_enabled(
        ...
        initialize_partial_voting(&endless_framework);
        features::change_feature_flags(&endless_framework, vector[features::get_partial_governance_voting()], vector[]);

        endless_coin::mint(&endless_framework, voter_1_addr, 20);
        endless_coin::mint(&endless_framework, voter_2_addr, 10);
    ...
    public entry fun test_no_remaining_voting_power_about_proposal_expiration_time(
        ...
        setup_voting_with_initialized_stake(&endless_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let proposer_addr = signer::address_of(&proposer);
        let voter_1_addr = signer::address_of(&voter_1);
    ...
    public fun setup_voting(
        ...
        staking_config::initialize_for_test(endless_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(endless_framework, 10, 100, 1000);
        store_signer_cap(
            endless_framework,
            @endless_framework,
            account::create_test_signer_cap(@endless_framework),
        );
        ...
        stake::create_validator_set(endless_framework, active_validators, pks);
        ...
        // power.
        primary_fungible_store::deposit(signer::address_of(proposer), fungible_asset::mint(&mint_cap, 100));
        primary_fungible_store::deposit(signer::address_of(yes_voter), fungible_asset::mint(&mint_cap, 20));
        primary_fungible_store::deposit(signer::address_of(no_voter), fungible_asset::mint(&mint_cap, 10));

        stake::create_stake_pool(proposer, fungible_asset::mint(&mint_cap, 50), fungible_asset::mint(&mint_cap, 50), 10000);
        stake::create_stake_pool(yes_voter, fungible_asset::mint(&mint_cap, 10), fungible_asset::mint(&mint_cap, 10), 10000);
        stake::create_stake_pool(no_voter, fungible_asset::mint(&mint_cap, 5), fungible_asset::mint(&mint_cap, 5), 10000);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(endless_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(endless_framework, 10, 100, 1000);
        store_signer_cap(
            endless_framework,
            @endless_framework,
        ...
        // Spread stake among active and pending_inactive because both need to be accounted for when computing voting
        // power.
        primary_fungible_store::deposit(signer::address_of(proposer), stake::mint_coins(100));
        primary_fungible_store::deposit(signer::address_of(yes_voter), stake::mint_coins(20));
        primary_fungible_store::deposit(signer::address_of(no_voter), stake::mint_coins(10));
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&endless_framework));
        initialize(&endless_framework, 1, 2, 3);
        update_governance_config(&endless_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@endless_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = endless_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(endless_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
