...
module endless_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(endless_framework: &signer) {
        system_addresses::assert_endless_framework(endless_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(endless_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by EndlessVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@endless_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@endless_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@endless_framework).microseconds
    }
