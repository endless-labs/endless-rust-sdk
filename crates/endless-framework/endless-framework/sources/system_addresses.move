module endless_framework::system_addresses {
    use std::error;
    use std::signer;
    ...
    const ENOT_ENDLESS_FRAMEWORK_ADDRESS: u64 = 3;
    ...
    public fun assert_endless_framework(account: &signer) {
        assert!(
            is_endless_framework_address(signer::address_of(account)),
            error::permission_denied(ENOT_ENDLESS_FRAMEWORK_ADDRESS),
        )
    }
    ...
    public fun is_framework_reserved_address(addr: address): bool {
        is_endless_framework_address(addr) ||
            addr == @0x2 ||
            addr == @0x3 ||
            addr == @0x4 ||
            addr == @0x5 ||
    ...
    public fun is_endless_framework_address(addr: address): bool {
    ...
    /// Return true if `addr` is either the VM address or an Endless Framework address.
    public fun is_reserved_address(addr: address): bool {
        is_endless_framework_address(addr) || is_vm_address(addr)
    }
}
