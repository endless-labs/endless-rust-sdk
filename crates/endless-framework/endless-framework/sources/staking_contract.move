...
module endless_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::features;
    use std::signer;
    ...
    use endless_framework::stake::{Self, OwnerCapability};
    use endless_framework::fungible_asset::{Self, FungibleAsset};
    use endless_framework::staking_config;

    const SALT: vector<u8> = b"endless_framework::staking_contract";
    ...
    ///
    /// This errors out the staking contract with the provided staker and operator doesn't exist.
    public fun last_recorded_principal(staker: address, operator: address): u128 acquires Store {
        assert_staking_contract_exists(staker, operator);
        let staking_contracts = &borrow_global<Store>(staker).staking_contracts;
    ...
    ///
    /// This errors out the staking contract with the provided staker and operator doesn't exist.
    public fun staking_contract_amounts(staker: address, operator: address): (u128, u128, u128) acquires Store {
        assert_staking_contract_exists(staker, operator);
        let staking_contracts = &borrow_global<Store>(staker).staking_contracts;
    ...
    public fun pending_distribution_counts(staker: address, operator: address): u64 acquires Store {
        assert_staking_contract_exists(staker, operator);
        let staking_contracts = &borrow_global<Store>(staker).staking_contracts;
        pool_u128::shareholders_count(&simple_map::borrow(staking_contracts, &operator).distribution_pool)
    }
    ...
    public entry fun create_staking_contract(
        ...
        let staked_coins = endless_coin::withdraw(staker, amount);
    ...
    public fun create_staking_contract_with_coins(
        ...
        assert!(endless_coin::is_true_EDS(&coins), EINSUFFICIENT_STAKE_AMOUNT);
        let principal = fungible_asset::amount(&coins);
        ...
        simple_map::add(staking_contracts, operator, StakingContract {
            ...
            distribution_pool: pool_u128::create(MAXIMUM_PENDING_DISTRIBUTIONS),
    ...
    public entry fun add_stake(staker: &signer, operator: address, amount: u128) acquires Store {
        let staker_address = signer::address_of(staker);
    ...
    public entry fun unlock_stake(staker: &signer, operator: address, amount: u128) acquires Store, BeneficiaryForOperator {
        ...
        add_distribution(
            operator, staking_contract, staker_address, amount);
    ...
    fun distribute_internal(
        ...
        let distribution_amount = fungible_asset::amount(&coins);
        ...
        while (pool_u128::shareholders_count(distribution_pool) > 0) {
            let recipients = pool_u128::shareholders(distribution_pool);
            let recipient = *vector::borrow(&mut recipients, 0);
            let current_shares = pool_u128::shares(distribution_pool, recipient);
            let amount_to_distribute = pool_u128::redeem_shares(distribution_pool, recipient, current_shares);
            // If the recipient is the operator, send the commission to the beneficiary instead.
            if (recipient == operator) {
                recipient = beneficiary_for_operator(operator);
            };
            endless_account::deposit_coins(recipient, fungible_asset::extract(&mut coins, (amount_to_distribute as u128)));
        ...
        if (fungible_asset::amount(&coins) > 0) {
            endless_account::deposit_coins(staker, coins);
            pool_u128::update_total_coins(distribution_pool, 0);
        } else {
            fungible_asset::destroy_zero(coins);
        }
    }
    ...
    fun add_distribution(
        ...
        pool_u128::buy_in(distribution_pool, recipient, coins_amount);
        let pool_address = staking_contract.pool_address;
        emit(
    ...
    fun get_staking_contract_amounts_internal(staking_contract: &StakingContract): (u128, u128, u128) {
        ...
        let commission_amount = (accumulated_rewards * (staking_contract.commission_percentage as u128)) / 100;
    ...
    fun update_distribution_pool(
        ...
        if (pool_u128::total_coins(distribution_pool) == updated_total_coins) {
            return
        };
        ...
        let shareholders = &pool_u128::shareholders(distribution_pool);
        vector::for_each_ref(shareholders, |shareholder| {
            let shareholder: address = *shareholder;
            if (shareholder != operator) {
                let shares = pool_u128::shares(distribution_pool, shareholder);
                let previous_worth = pool_u128::balance(distribution_pool, shareholder);
                let current_worth = pool_u128::shares_to_amount_with_total_coins(
                    distribution_pool, shares, updated_total_coins);
                let unpaid_commission = (current_worth - previous_worth) * (commission_percentage as u128) / 100;
                ...
                let shares_to_transfer = pool_u128::amount_to_shares_with_total_coins(
                    distribution_pool, unpaid_commission, updated_total_coins);
                pool_u128::transfer_shares(distribution_pool, shareholder, operator, shares_to_transfer);
            };
        });

        pool_u128::update_total_coins(distribution_pool, updated_total_coins);
    }
    ...
    #[test_only]
    const INITIAL_BALANCE: u128 = 100000000000000; // 1M EDS coins with 8 decimals.

    #[test_only]
    const MAXIMUM_STAKE: u128 = 100000000000000000; // 1B EDS coins with 8 decimals.
    ...
    #[test_only]
    public fun setup(endless_framework: &signer, staker: &signer, operator: &signer, initial_balance: u128) {
        // Reward rate of 0.1% per epoch.
        stake::initialize_for_test_custom(endless_framework, INITIAL_BALANCE, MAXIMUM_STAKE, 3600, true, 10, 10000, 1000000);
    ...
    public fun setup_staking_contract(
        ...
        setup(endless_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
    ...
    #[test(endless_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(endless_framework: &signer, staker: &signer, operator: &signer) acquires Store, BeneficiaryForOperator {
        setup_staking_contract(endless_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        ...
        let operator_balance = endless_coin::balance(operator_address);
        let expected_operator_balance = INITIAL_BALANCE + expected_commission_1;
        assert!(operator_balance == expected_operator_balance, (operator_balance as u64));
        stake::assert_stake_pool(pool_address, new_balance, 0, 0, 0);
        ...
        distribute(staker_address, operator_address);
        operator_balance = endless_coin::balance(operator_address);
        ...
        assert_no_pending_distributions(staker_address, operator_address);
        operator_balance = endless_coin::balance(operator_address);
        assert!(operator_balance == expected_operator_balance + unpaid_commission, (operator_balance as u64));
        let staker_balance = endless_coin::balance(staker_address);
        // Staker receives the extra dust due to rounding error.
        assert!(staker_balance == withdrawn_amount + 1, (staker_balance as u64));
        assert_no_pending_distributions(staker_address, operator_address);
    }

    #[test(endless_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_operator_cannot_request_same_commission_multiple_times(
        endless_framework: &signer, staker: &signer, operator: &signer) acquires Store, BeneficiaryForOperator {
        setup_staking_contract(endless_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        let pool_address = stake_pool_address(staker_address, operator_address);
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_invalid_commission(
        ...
        setup_staking_contract(endless_framework, staker, operator, INITIAL_BALANCE, 101);
    }
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(endless_framework, staker, operator, 50, 100);
    }
    ...
    #[test(endless_framework = @0x1, staker = @0x123, operator_1 = @0x234, operator_2 = @0x345)]
    public entry fun test_staker_can_switch_operator(
        ...
        setup_staking_contract(endless_framework, staker, operator_1, INITIAL_BALANCE, 10);
        ...
        let operator_1_balance = endless_coin::balance(operator_1_address);
        assert!(operator_1_balance == INITIAL_BALANCE + commission_for_operator_1, (operator_1_balance as u64));
        ...
        let operator_2_balance = endless_coin::balance(operator_2_address);
        new_balance = with_rewards(new_balance);
    ...
    #[test(endless_framework = @0x1, staker = @0x123, operator1 = @0x234, beneficiary = @0x345, operator2 = @0x456)]
    public entry fun test_operator_can_set_beneficiary(
        ...
        setup_staking_contract(endless_framework, staker, operator1, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        ...
        endless_framework::endless_account::create_account(beneficiary_address);
        ...
        let operator_balance = endless_coin::balance(operator1_address);
        let beneficiary_balance = endless_coin::balance(beneficiary_address);
        let expected_operator_balance = INITIAL_BALANCE;
        let expected_beneficiary_balance = expected_commission_1;
        assert!(operator_balance == expected_operator_balance, (operator_balance as u64));
        assert!(beneficiary_balance == expected_beneficiary_balance, (beneficiary_balance as u64));
        stake::assert_stake_pool(pool_address, new_balance, 0, 0, 0);
        ...
        assert!(endless_coin::balance(operator2_address) >= expected_commission, 1);
    ...
    public entry fun test_staker_can_withdraw_partial_stake(
        ...
        assert_no_pending_distributions(staker_address, operator_address);
        let operator_balance = endless_coin::balance(operator_address);
        assert!(operator_balance == initial_balance + unpaid_commission, (operator_balance as u64));
        let staker_balance = endless_coin::balance(staker_address);
        assert!(staker_balance == withdrawn_stake, (staker_balance as u64));
    }
    ...
    public entry fun test_staker_can_withdraw_partial_stake_if_operator_never_joined_validator_set(
        ...
        let operator_balance = endless_coin::balance(operator_address);
        assert!(operator_balance == initial_balance, (operator_balance as u64));
    ...
    public entry fun test_multiple_distributions_added_before_distribute(
        ...
        // std::debug::print(&std::string::utf8(b"MY TEST!"));
        ...
        // std::debug::print(&std::string::utf8(b"11111111111"));
        ...
        // std::debug::print(&std::string::utf8(b"2222222222"));
        // std::debug::print(&std::string::utf8(b"EDS Supply:"));
        // std::debug::print(&endless_coin::supply());
        ...
        // std::debug::print(&std::string::utf8(b"333333333"));
        ...
        // std::debug::print(&std::string::utf8(b"4444444444"));
    ...
    #[test(endless_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_update_commission(endless_framework: &signer, staker: &signer, operator: &signer) acquires Store, BeneficiaryForOperator {
        let initial_balance = INITIAL_BALANCE * 2;
        setup_staking_contract(endless_framework, staker, operator, initial_balance, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
    ...
    public fun assert_staking_contract(
        staker: address, operator: address, principal: u128, commission_percentage: u64) acquires Store {
        let staking_contract = simple_map::borrow(&borrow_global<Store>(staker).staking_contracts, &operator);
        assert!(staking_contract.principal == principal, (staking_contract.principal as u64));
    ...
    public fun assert_no_pending_distributions(staker: address, operator: address) acquires Store {
        ...
        let shareholders_count = pool_u128::shareholders_count(distribution_pool);
        assert!(shareholders_count == 0, shareholders_count);
        let total_coins_remaining = pool_u128::total_coins(distribution_pool);
        assert!(total_coins_remaining == 0, (total_coins_remaining as u64));
    }

    #[test_only]
    public fun assert_distribution(
        staker: address, operator: address, recipient: address, coins_amount: u128) acquires Store {
        let staking_contract = simple_map::borrow(&borrow_global<Store>(staker).staking_contracts, &operator);
        let distribution_balance = pool_u128::balance(&staking_contract.distribution_pool, recipient);
        assert!(distribution_balance == coins_amount, (distribution_balance as u64));
    }
}
