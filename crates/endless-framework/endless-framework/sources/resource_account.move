...
/// }
/// ```
module endless_framework::resource_account {
    use std::error;
    use std::signer;
    use std::vector;
    ...
    const ZERO_AUTH_KEY: vector<vector<u8>> = vector[];
    ...
    public entry fun create_resource_account(
        origin: &signer,
        seed: vector<u8>,
        optional_auth_key: vector<vector<u8>>,
    ) acquires Container {
    ...
    public entry fun create_resource_account_and_fund(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        primary_fungible_store::ensure_primary_store_exists(signer::address_of(&resource), endless_coin::get_metadata());
        endless_coin::transfer(origin, signer::address_of(&resource), fund_amount);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun create_resource_account_and_publish_package(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        endless_framework::code::publish_package_txn(&resource, metadata_serialized, code);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun test_create_account_and_retrieve_cap(user: signer) acquires Container {
        ...
        let resource_addr = endless_framework::account::create_resource_address(&user_addr, seed);
    ...
    #[test(user = @0x1111)]
    #[expected_failure(abort_code = 0x10002, location = endless_std::simple_map)]
    ...
    public entry fun with_coin(framework: signer, user: signer) acquires Container {
        ...
        let user_addr = signer::address_of(&user);
        account::create_account(copy user_addr);

        let (mint_cap, burn_cap, transfer_cap) = endless_framework::endless_coin::initialize_for_test(&framework);
        primary_fungible_store::mint(&mint_cap, user_addr, 100);

        let seed = x"01";
        create_resource_account_and_fund(&user, copy seed, vector::empty(), 10);

        let resource_addr = endless_framework::account::create_resource_address(&user_addr, seed);
        endless_coin::transfer(&user, resource_addr, 10);
