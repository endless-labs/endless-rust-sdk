...
module endless_framework::reconfiguration_state {
    use std::error;
    use std::string;
    ...
    public fun is_initialized(): bool {
        exists<State>(@endless_framework)
    }
    ...
    public(friend) fun is_in_progress(): bool acquires State {
        ...
        let state = borrow_global<State>(@endless_framework);
    ...
    public(friend) fun on_reconfig_start() acquires State {
        if (exists<State>(@endless_framework)) {
            let state = borrow_global_mut<State>(@endless_framework);
            let variant_type_name = *string::bytes(copyable_any::type_name(&state.variant));
