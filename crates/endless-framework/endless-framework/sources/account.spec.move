spec endless_framework::account {
    /// <high-level-req>
    /// No.: 1
    ...
    /// to the OriginatingAddress under the endless_framework account.
    ...
    /// Only the address `@endless_framework` can call.
    /// OriginatingAddress does not exist under `@endless_framework` before the call.
    spec initialize(endless_framework: &signer) {
        let endless_addr = signer::address_of(endless_framework);
        aborts_if !system_addresses::is_endless_framework_address(endless_addr);
        aborts_if exists<OriginatingAddress>(endless_addr);
        ensures exists<OriginatingAddress>(endless_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @endless_framework || new_address == @endless_token;
    ...
    spec get_authentication_key(addr: address): vector<vector<u8>> {
        aborts_if !exists<Account>(addr);
    ...
    spec rotate_authentication_key_internal(account: &signer, new_auth_key: vector<vector<u8>>) {
    ...
    //     let address_map = global<OriginatingAddress>(@endless_framework).address_map;
    //     let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

    //     aborts_if !exists<OriginatingAddress>(@endless_framework);
    //     aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    //         chain_id: global<chain_id::ChainId>(@endless_framework).id,
    //         sequence_number: account_resource.sequence_number,
    //         source_address,
    //         recipient_address,
    //     };

    //     aborts_if !exists<chain_id::ChainId>(@endless_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@endless_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        // ensures table::spec_contains(global<OriginatingAddress>(@endless_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
    ...
    spec verify_signed_message<T: drop>(
        ...
        aborts_if account_scheme == ED25519_SCHEME && ({
            ...
            !vector::spec_contains(account_resource.authentication_key, expected_auth_key)
        });
    ...
    spec fun spec_check_authentication_key(addr: address, auth_key: vector<vector<u8>>): bool {
        let authentication_key = global<Account>(addr).authentication_key;
        let num_signatures_required = global<Account>(addr).num_signatures_required;
        vector::length(auth_key) >= num_signatures_required
            && !(exists k in auth_key: !vector::spec_contains(authentication_key, k))
    }
}
