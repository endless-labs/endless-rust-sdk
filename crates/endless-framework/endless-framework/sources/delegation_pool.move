 ...
 */
module endless_framework::delegation_pool {
    use std::error;
    use std::features;
    use std::signer;
    use std::vector;
    ...
    use endless_std::pool_u128_unbound::{Self as pool_u128, total_coins};
    ...
    use endless_framework::event::{Self, emit};
    use endless_framework::stake;
    use endless_framework::stake::get_operator;
    use endless_framework::staking_config;
    use endless_framework::timestamp;
    use endless_framework::primary_fungible_store;

    const MODULE_SALT: vector<u8> = b"endless_framework::delegation_pool";
    ...
    const MIN_COINS_ON_SHARES_POOL: u128 = 1000000000;
    ...
    struct DelegationPool has key {
        ...
        inactive_shares: Table<ObservedLockupCycle, pool_u128::Pool>,
    ...
    struct DistributeCommission has drop, store {
        ...
        operator: address,
        beneficiary: address,
        commission_active: u128,
        commission_pending_inactive: u128,
    }
    ...
    public fun shareholders_count_active_pool(pool_address: address): u64 acquires DelegationPool {
        assert_delegation_pool_exists(pool_address);
        pool_u128::shareholders_count(&borrow_global<DelegationPool>(pool_address).active_shares)
    }

    #[view]
    /// Return the stake amounts on `pool_address` in the different states:
    /// (`active`,`inactive`,`pending_active`,`pending_inactive`)
    public fun get_delegation_pool_stake(pool_address: address): (u128, u128, u128, u128) {
        assert_delegation_pool_exists(pool_address);
    ...
    public fun get_pending_withdrawal(
        pool_address: address,
        delegator_address: address
    ): (bool, u128) acquires DelegationPool {
        assert_delegation_pool_exists(pool_address);
        ...
        if (!withdrawal_exists) {
            ...
            if (withdrawal_olc.index < pool.observed_lockup_cycle.index) {
                ...
                (true, pool_u128::balance(inactive_shares, delegator_address))
            } else {
                pending_inactive = pool_u128::shares_to_amount_with_total_coins(
                    inactive_shares,
                    pool_u128::shares(inactive_shares, delegator_address),
    ...
    public fun get_stake(pool_address: address, delegator_address: address): (u128, u128, u128) acquires DelegationPool, BeneficiaryForOperator {
        assert_delegation_pool_exists(pool_address);
        let pool = borrow_global<DelegationPool>(pool_address);
        let (
            lockup_cycle_ended,
        ...
        let total_active_shares = pool_u128::total_shares(&pool.active_shares);
        let delegator_active_shares = pool_u128::shares(&pool.active_shares, delegator_address);
        ...
        if (pending_active == 0) {
            ...
            total_active_shares = total_active_shares - pool_u128::shares(&pool.active_shares, NULL_SHAREHOLDER);
    ...
    public fun get_add_stake_fee(pool_address: address, amount: u128): u128 acquires DelegationPool, NextCommissionPercentage {
        if (stake::is_current_epoch_validator(pool_address)) {
            ...
            if (rewards_rate_denominator > 0) {
                ...
                math128::mul_div(amount, (rewards_rate as u128), (rewards_rate + rewards_rate_denominator as u128))
            } else { 0 }
        } else { 0 }
    }
    ...
    /// latest state.
    public fun calculate_and_update_voter_total_voting_power(pool_address: address, voter: address): u128 acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
    ...
    /// latest state.
    public fun calculate_and_update_remaining_voting_power(pool_address: address, voter_address: address, proposal_id: u64): u128 acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        assert_partial_governance_voting_enabled(pool_address);
        ...
        if (endless_governance::get_remaining_voting_power(pool_address, proposal_id) == 0) {
            return 0
        };
    ...
    public entry fun initialize_delegation_pool(
        ...
        primary_fungible_store::ensure_primary_store_exists(signer::address_of(&stake_pool_signer), endless_coin::get_metadata());
        ...
        let inactive_shares = table::new<ObservedLockupCycle, pool_u128::Pool>();
        table::add(
            &mut inactive_shares,
            olc_with_index(0),
            pool_u128::create_with_scaling_factor(SHARES_SCALING_FACTOR)
        );

        move_to(&stake_pool_signer, DelegationPool {
            active_shares: pool_u128::create_with_scaling_factor(SHARES_SCALING_FACTOR),
    ...
    public entry fun vote(voter: &signer, pool_address: address, proposal_id: u64, voting_power: u128, should_pass: bool) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        assert_partial_governance_voting_enabled(pool_address);
        // synchronize delegation and stake pools before any user operation.
        synchronize_delegation_pool(pool_address);
        ...
        *used_voting_power = *used_voting_power + voting_power;
        ...
        endless_governance::partial_vote(&pool_signer, pool_address, proposal_id, (voting_power as u128), should_pass);
    ...
    public entry fun create_proposal(
        ...
        let pool = borrow_global<DelegationPool>(pool_address);
        let governance_records = borrow_global_mut<GovernanceRecords>(pool_address);
        let total_voting_power = (calculate_and_update_delegated_votes(pool, governance_records, voter_addr) as u128);
        assert!(
            total_voting_power >= endless_governance::get_required_proposer_stake(),
            error::invalid_argument(EINSUFFICIENT_PROPOSER_STAKE));
        let pool_signer = retrieve_stake_pool_owner(borrow_global<DelegationPool>(pool_address));
        let proposal_id = endless_governance::create_proposal_v2_impl(
            &pool_signer,
            pool_address,
            execution_hash,
            metadata_location,
    ...
    fun assert_min_active_balance(pool: &DelegationPool, delegator_address: address) {
        let balance = pool_u128::balance(&pool.active_shares, delegator_address);
    ...
    fun assert_min_pending_inactive_balance(pool: &DelegationPool, delegator_address: address) {
        let balance = pool_u128::balance(pending_inactive_shares_pool(pool), delegator_address);
        assert!(
    ...
    fun coins_to_transfer_to_ensure_min_stake(
        src_shares_pool: &pool_u128::Pool,
        dst_shares_pool: &pool_u128::Pool,
        shareholder: address,
        amount: u128,
    ): u128 {
        ...
        let dst_balance = pool_u128::balance(dst_shares_pool, shareholder);
    ...
    fun get_delegator_active_shares(pool: &DelegationPool, delegator: address): u128 {
        pool_u128::shares(&pool.active_shares, delegator)
    }
    ...
    fun get_delegator_pending_inactive_shares(pool: &DelegationPool, delegator: address): u128 {
        pool_u128::shares(pending_inactive_shares_pool(pool), delegator)
    }

    /// Get the used voting power of a voter on a proposal.
    fun get_used_voting_power(governance_records: &GovernanceRecords, voter: address, proposal_id: u64): u128 {
    ...
    inline fun borrow_mut_used_voting_power(governance_records: &mut GovernanceRecords, voter: address, proposal_id: u64): &mut u128 {
    ...
    fun calculate_total_voting_power(delegation_pool: &DelegationPool, latest_delegated_votes: &DelegatedVotes): u128 {
        let active_amount = pool_u128::shares_to_amount(
            &delegation_pool.active_shares,
            latest_delegated_votes.active_shares);
        let pending_inactive_amount = pool_u128::shares_to_amount(
    ...
    fun calculate_and_update_delegated_votes(pool: &DelegationPool, governance_records: &mut GovernanceRecords, voter: address): u128 {
    ...
    public entry fun add_stake(delegator: &signer, pool_address: address, amount: u128) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        ...
        endless_account::transfer(delegator, pool_address, (amount as u128));
        stake::add_stake(&retrieve_stake_pool_owner(pool), (amount as u128));
    ...
    public entry fun unlock(delegator: &signer, pool_address: address, amount: u128) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        ...
        assert!((amount as u128) <= active, error::invalid_argument(ENOT_ENOUGH_ACTIVE_STAKE_TO_UNLOCK));
        ...
        stake::unlock(&retrieve_stake_pool_owner(pool), (amount as u128));
    ...
    public entry fun reactivate_stake(delegator: &signer, pool_address: address, amount: u128) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        ...
        stake::reactivate_stake(&retrieve_stake_pool_owner(pool), (amount as u128));
    ...
    public entry fun withdraw(delegator: &signer, pool_address: address, amount: u128) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
    ...
    fun withdraw_internal(pool: &mut DelegationPool, delegator_address: address, amount: u128) acquires GovernanceRecords {
        ...
        if (can_withdraw_pending_inactive(pool_address)) {
            ...
            if (withdrawal_olc.index == pool.observed_lockup_cycle.index) {
                ...
                pending_inactive = pending_inactive - (amount as u128)
            };
            // escape excess stake from inactivation
            stake::reactivate_stake(stake_pool_owner, pending_inactive);
            stake::withdraw(stake_pool_owner, (amount as u128));
        ...
        endless_account::transfer(stake_pool_owner, delegator_address, (amount as u128));
    ...
    fun pending_inactive_shares_pool_mut(pool: &mut DelegationPool): &mut pool_u128::Pool {
        let observed_lockup_cycle = pool.observed_lockup_cycle;
        table::borrow_mut(&mut pool.inactive_shares, observed_lockup_cycle)
    }

    fun pending_inactive_shares_pool(pool: &DelegationPool): &pool_u128::Pool {
    ...
    fun execute_pending_withdrawal(pool: &mut DelegationPool, delegator_address: address) acquires GovernanceRecords {
        ...
        if (withdrawal_exists && withdrawal_olc.index < pool.observed_lockup_cycle.index) {
            withdraw_internal(pool, delegator_address, MAX_U128);
        }
    }
    ...
    fun buy_in_active_shares(
        ...
        let new_shares = pool_u128::amount_to_shares(&pool.active_shares, coins_amount);
        ...
        pool_u128::buy_in(&mut pool.active_shares, shareholder, coins_amount);
        new_shares
    }
    ...
    fun buy_in_pending_inactive_shares(
        ...
        let new_shares = pool_u128::amount_to_shares(pending_inactive_shares_pool(pool), coins_amount);
        ...
        pool_u128::buy_in(pending_inactive_shares_pool_mut(pool), shareholder, coins_amount);
    ...
    fun amount_to_shares_to_redeem(
        shares_pool: &pool_u128::Pool,
        shareholder: address,
        coins_amount: u128,
    ): u128 {
        if (coins_amount >= pool_u128::balance(shares_pool, shareholder)) {
            // cap result at total shares of shareholder to pass `EINSUFFICIENT_SHARES` on subsequent redeem
            pool_u128::shares(shares_pool, shareholder)
        } else {
            pool_u128::amount_to_shares(shares_pool, coins_amount)
        }
    }
    ...
    fun redeem_active_shares(
        ...
        pool_u128::redeem_shares(&mut pool.active_shares, shareholder, shares_to_redeem)
    }
    ...
    fun redeem_inactive_shares(
        ...
        let redeemed_coins = pool_u128::redeem_shares(inactive_shares, shareholder, shares_to_redeem);
        ...
        if (pool_u128::shares(inactive_shares, shareholder) == 0) {
        ...
        if (lockup_cycle.index < pool.observed_lockup_cycle.index && total_coins(inactive_shares) == 0) {
            pool_u128::destroy_empty(table::remove(&mut pool.inactive_shares, lockup_cycle));
        };
    ...
    fun calculate_stake_pool_drift(pool: &DelegationPool): (bool, u128, u128, u128, u128) {
        let (active, inactive, pending_active, pending_inactive) = stake::get_stake(get_pool_address(pool));
        let total_coins_inactive = pool.total_coins_inactive;
        assert!(
        ...
        let lockup_cycle_ended = inactive > total_coins_inactive;
        ...
        let commission_active = if (active > pool_active) {
            math128::mul_div(active - pool_active, (pool.operator_commission_percentage as u128), (MAX_FEE as u128))
        } else {
        ...
        let commission_pending_inactive = if (pending_inactive > pool_pending_inactive) {
            math128::mul_div(
                ...
                (pool.operator_commission_percentage as u128),
                (MAX_FEE as u128),
            )
        } else {
    ...
    public entry fun synchronize_delegation_pool(pool_address: address) acquires DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        ...
        if (pending_active == 0) {
            ...
            redeem_active_shares(pool, NULL_SHAREHOLDER, MAX_U128);
        };
        ...
        pool_u128::update_total_coins(&mut pool.active_shares, active - commission_active);
        // update total coins accumulated by `pending_inactive` shares at current observed lockup cycle
        pool_u128::update_total_coins(
    ...
    inline fun assert_and_update_proposal_used_voting_power(
        governance_records: &mut GovernanceRecords, pool_address : address, proposal_id : u64, voting_power: u128
    ) {
        let stake_pool_remaining_voting_power = endless_governance::get_remaining_voting_power(pool_address, proposal_id);
        let stake_pool_used_voting_power = endless_governance::get_voting_power(pool_address) - stake_pool_remaining_voting_power;
    ...
    #[test_only]
    use endless_framework::stake::fast_forward_to_unlock;
    #[test_only]
    use endless_framework::timestamp::fast_forward_seconds;
    ...
    #[test_only]
    public fun end_endless_epoch() {
        stake::end_epoch(); // additionally forwards EPOCH_DURATION seconds
        reconfiguration::reconfigure_for_test_custom();
    }

    #[test_only]
    public fun initialize_for_test(endless_framework: &signer) {
    ...
    #[test_only]
    public fun initialize_for_test_no_reward(endless_framework: &signer) {
        initialize_for_test_custom(
            endless_framework,
            100 * ONE_EDS,
            10000000 * ONE_EDS,
    ...
    public fun initialize_for_test_custom(
        ...
        account::create_account_for_test(signer::address_of(endless_framework));
        ...
        features::change_feature_flags(endless_framework, vector[DELEGATION_POOLS, MODULE_EVENT, OPERATOR_BENEFICIARY_CHANGE, COMMISSION_CHANGE_DELEGATION_POOL], vector[]);
    }
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    ...
    public entry fun test_delegation_pools_disabled(
        ...
        features::change_feature_flags(endless_framework, vector[], vector[DELEGATION_POOLS]);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020)]
    public entry fun test_add_stake_fee(
        ...
        assert!(get_add_stake_fee(pool_address, 1000000 * ONE_EDS) == 0, 0);

        // add 1M EDS, join the validator set and activate this stake
        stake::mint(validator, 1000000 * ONE_EDS);
        add_stake(validator, pool_address, 1000000 * ONE_EDS);
        ...
        assert!(get_add_stake_fee(pool_address, 100000 * ONE_EDS) == 62259941466, 0);

        // add pending_active stake from multiple delegators
        stake::mint(delegator1, 100000 * ONE_EDS);
        add_stake(delegator1, pool_address, 100000 * ONE_EDS);
        stake::mint(delegator2, 10000 * ONE_EDS);
        add_stake(delegator2, pool_address, 10000 * ONE_EDS);
        ...
        stake::mint(delegator1, 10000 * ONE_EDS);
        let (delegator1_active, _, _) = get_stake(pool_address, delegator1_address);
        add_stake(delegator1, pool_address, 10000 * ONE_EDS);

        let fee = get_add_stake_fee(pool_address, 10000 * ONE_EDS);
        assert_delegation(delegator1_address, pool_address, delegator1_active + 10000 * ONE_EDS - fee, 0, 0);

        // delegator 2 should not benefit in any way from this new stake
        assert_delegation(delegator2_address, pool_address, 1000000000000, 0, 0);

        // add more stake from delegator 2
        stake::mint(delegator2, 100000 * ONE_EDS);
        add_stake(delegator2, pool_address, 100000 * ONE_EDS);
        ...
        // add more stake from delegator 1
        stake::mint(delegator1, 20000 * ONE_EDS);
        (delegator1_active, _, _) = get_stake(pool_address, delegator1_address);
        add_stake(delegator1, pool_address, 20000 * ONE_EDS);

        fee = get_add_stake_fee(pool_address, 20000 * ONE_EDS);
        assert_delegation(delegator1_address, pool_address, delegator1_active + 20000 * ONE_EDS - fee, 0, 0);

        // delegator 1 unlocks his entire newly added stake
        unlock(delegator1, pool_address, 20000 * ONE_EDS - fee);
        end_endless_epoch();
        ...
        staking_config::override_rewards_for_test(
            endless_framework,
        ...
        features::change_feature_flags(endless_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);

        // add more stake from delegator 1
        stake::mint(delegator1, 20000 * ONE_EDS);
        let delegator1_pending_inactive: u128;
        (delegator1_active, _, delegator1_pending_inactive) = get_stake(pool_address, delegator1_address);
        fee = get_add_stake_fee(pool_address, 20000 * ONE_EDS);
        add_stake(delegator1, pool_address, 20000 * ONE_EDS);

        assert_delegation(delegator1_address, pool_address, delegator1_active + 20000 * ONE_EDS - fee, 0, delegator1_pending_inactive);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123, delegator = @0x010)]
    public entry fun test_never_create_pending_withdrawal_if_no_shares_bought(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 1000 * ONE_EDS, true, false);
        ...
        stake::mint(delegator, 10 * ONE_EDS);
        add_stake(delegator, pool_address, 10 * ONE_EDS);
        end_endless_epoch();

        unlock(validator, pool_address, 100 * ONE_EDS);
    ...
    public entry fun test_add_stake_single(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 1000 * ONE_EDS, false, false);

        let validator_address = signer::address_of(validator);
        let pool_address = get_owned_pool_address(validator_address);

        // validator is inactive => added stake is `active` by default
        stake::assert_stake_pool(pool_address, 1000 * ONE_EDS, 0, 0, 0);
        assert_delegation(validator_address, pool_address, 1000 * ONE_EDS, 0, 0);

        // zero `add_stake` fee as validator is not producing rewards this epoch
        assert!(get_add_stake_fee(pool_address, 250 * ONE_EDS) == 0, 0);

        // check `add_stake` increases `active` stakes of delegator and stake pool
        stake::mint(validator, 300 * ONE_EDS);
        let balance = endless_coin::balance(validator_address);
        add_stake(validator, pool_address, 250 * ONE_EDS);

        // check added stake have been transferred out of delegator account
        assert!(endless_coin::balance(validator_address) == balance - 250 * ONE_EDS, 0);
        // zero `add_stake` fee charged from added stake
        assert_delegation(validator_address, pool_address, 1250 * ONE_EDS, 0, 0);
        ...
        stake::assert_stake_pool(pool_address, 1250 * ONE_EDS, 0, 0, 0);
        ...
        stake::mint(validator, 250 * ONE_EDS);
        add_stake(validator, pool_address, 250 * ONE_EDS);

        let fee1 = get_add_stake_fee(pool_address, 250 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 1500 * ONE_EDS - fee1, 0, 0);
        // check `add_stake` fee has been transferred to the null shareholder
        assert_delegation(NULL_SHAREHOLDER, pool_address, fee1, 0, 0);
        stake::assert_stake_pool(pool_address, 1250 * ONE_EDS, 0, 250 * ONE_EDS, 0);

        // add 100 additional coins being pending_active until next epoch
        stake::mint(validator, 100 * ONE_EDS);
        add_stake(validator, pool_address, 100 * ONE_EDS);

        let fee2 = get_add_stake_fee(pool_address, 100 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 1600 * ONE_EDS - fee1 - fee2, 0, 0);
        // check `add_stake` fee has been transferred to the null shareholder
        assert_delegation(NULL_SHAREHOLDER, pool_address, fee1 + fee2, 0, 0);
        stake::assert_stake_pool(pool_address, 1250 * ONE_EDS, 0, 350 * ONE_EDS, 0);
        ...
        assert_delegation(NULL_SHAREHOLDER, pool_address, 0, 0, 0);
        synchronize_delegation_pool(pool_address);
        assert!(pool_u128::shares(&borrow_global<DelegationPool>(pool_address).active_shares, NULL_SHAREHOLDER) == 0, 0);
        assert_delegation(NULL_SHAREHOLDER, pool_address, 0, 0, 0);

        // add 200 coins being pending_active until next epoch
        stake::mint(validator, 200 * ONE_EDS);
        add_stake(validator, pool_address, 200 * ONE_EDS);

        fee1 = get_add_stake_fee(pool_address, 200 * ONE_EDS);
    ...
    public entry fun test_add_stake_many(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 1000 * ONE_EDS, true, true);
        ...
        stake::mint(delegator, 250 * ONE_EDS);
        add_stake(delegator, pool_address, 250 * ONE_EDS);

        let fee1 = get_add_stake_fee(pool_address, 250 * ONE_EDS);
        assert_delegation(delegator_address, pool_address, 250 * ONE_EDS - fee1, 0, 0);
        assert_delegation(validator_address, pool_address, 1000 * ONE_EDS, 0, 0);
        stake::assert_stake_pool(pool_address, 1000 * ONE_EDS, 0, 250 * ONE_EDS, 0);

        end_endless_epoch();
        // 1000 * 1.01 active stake + 250 pending_active stake
        stake::assert_stake_pool(pool_address, 1260 * ONE_EDS, 0, 0, 0);
        // delegator got its `add_stake` fee back
        assert_delegation(delegator_address, pool_address, 250 * ONE_EDS, 0, 0);
        ...
        fee1 = get_add_stake_fee(pool_address, 250 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 125999999999 - fee1, 0, 0);
        assert_delegation(delegator_address, pool_address, 250 * ONE_EDS, 0, 0);
        stake::assert_stake_pool(pool_address, 1260 * ONE_EDS, 0, 250 * ONE_EDS, 0);

        // add another 100 coins from second account
        stake::mint(delegator, 100 * ONE_EDS);
        add_stake(delegator, pool_address, 100 * ONE_EDS);

        let fee2 = get_add_stake_fee(pool_address, 100 * ONE_EDS);
        assert_delegation(delegator_address, pool_address, 350 * ONE_EDS - fee2, 0, 0);
        assert_delegation(validator_address, pool_address, 125999999999 - fee1, 0, 0);
        stake::assert_stake_pool(pool_address, 1260 * ONE_EDS, 0, 350 * ONE_EDS, 0);
    ...
    public entry fun test_unlock_single(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 100 * ONE_EDS, true, true);
        ...
        let fee = get_add_stake_fee(pool_address, 200 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 300 * ONE_EDS - fee, 0, 0);
        stake::assert_stake_pool(pool_address, 100 * ONE_EDS, 0, 200 * ONE_EDS, 0);

        // cannot unlock pending_active stake (only 100/300 stake can be displaced)
        unlock(validator, pool_address, 100 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 200 * ONE_EDS - fee, 0, 100 * ONE_EDS);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 100 * ONE_EDS);
        stake::assert_stake_pool(pool_address, 0, 0, 200 * ONE_EDS, 100 * ONE_EDS);
        assert_inactive_shares_pool(pool_address, 0, true, 100 * ONE_EDS);

        // reactivate entire pending_inactive stake progressively
        reactivate_stake(validator, pool_address, 50 * ONE_EDS);

        assert_delegation(validator_address, pool_address, 250 * ONE_EDS - fee, 0, 50 * ONE_EDS);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 50 * ONE_EDS);
        stake::assert_stake_pool(pool_address, 50 * ONE_EDS, 0, 200 * ONE_EDS, 50 * ONE_EDS);
        ...
        assert_delegation(validator_address, pool_address, 301 * ONE_EDS, 0, 0);
        stake::assert_stake_pool(pool_address, 301 * ONE_EDS, 0, 0, 0);

        // can unlock more than at previous epoch as the pending_active stake became active
        unlock(validator, pool_address, 150 * ONE_EDS);
        ...
        end_endless_epoch(); // additionally forwards EPOCH_DURATION seconds
        ...
        end_endless_epoch(); // additionally forwards EPOCH_DURATION seconds and expires lockup cycle
        ...
        stake::mint(delegator, 50 * ONE_EDS);
        add_stake(delegator, pool_address, 50 * ONE_EDS);

        // observed lockup cycle should have advanced at `add_stake`(on synchronization)
        assert!(observed_lockup_cycle(pool_address) == 1, 0);

        fee = get_add_stake_fee(pool_address, 50 * ONE_EDS);
        assert_delegation(delegator_address, pool_address, 4999999999 - fee, 0, 0);
        assert_delegation(validator_address, pool_address, 15403510001, 15301499997, 0);
        stake::assert_stake_pool(pool_address, 15403510001, 15301499997, 50 * ONE_EDS, 0);

        // cannot withdraw stake unlocked by others
        withdraw(delegator, pool_address, 50 * ONE_EDS);
        assert!(endless_coin::balance(delegator_address) == 0, 0);

        // withdraw own unlocked stake
        withdraw(validator, pool_address, 15301499997);
        assert!(endless_coin::balance(validator_address) == 15301499997, 0);
        ...
        assert!(endless_coin::balance(validator_address) == balance + 5457545100, 0);
        ...
        initialize_test_validator(delegator, 100 * ONE_EDS, true, true);
        ...
        balance = endless_coin::balance(validator_address);
        withdraw(validator, pool_address, 10303010000);

        assert!(endless_coin::balance(validator_address) == balance + 10303010000, 0);
        ...
        stake::mint(validator, 30 * ONE_EDS);
        add_stake(validator, pool_address, 30 * ONE_EDS);
        unlock(validator, pool_address, 10 * ONE_EDS);
        ...
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert!(endless_coin::balance(validator_address) == balance + 1000000000, 0);
    }
    ...
    public entry fun test_total_coins_inactive(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 200 * ONE_EDS, true, true);
        ...
        stake::mint(delegator1, 100 * ONE_EDS);
        stake::mint(delegator2, 200 * ONE_EDS);
        add_stake(delegator1, pool_address, 100 * ONE_EDS);
        add_stake(delegator2, pool_address, 200 * ONE_EDS);
        end_endless_epoch();

        assert_delegation(delegator1_address, pool_address, 100 * ONE_EDS, 0, 0);
        assert_delegation(delegator2_address, pool_address, 200 * ONE_EDS, 0, 0);

        // unlock some stake from delegator 1
        unlock(delegator1, pool_address, 50 * ONE_EDS);
        ...
        assert_delegation(delegator2_address, pool_address, 202 * ONE_EDS, 0, 0);

        synchronize_delegation_pool(pool_address);
        assert!(total_coins_inactive(pool_address) == 5049999998, 0);

        // unlock some stake from delegator 2
        unlock(delegator2, pool_address, 50 * ONE_EDS);
        ...
        unlock(validator, pool_address, 50 * ONE_EDS);

        // create dummy validator to ensure the existing validator can leave the set
        initialize_test_validator(delegator1, 100 * ONE_EDS, true, true);
        // inactivate validator
        stake::leave_validator_set(validator, pool_address);
        end_endless_epoch();
        ...
        assert!(inactive_now == inactive, (inactive_now as u64));
        assert!(pending_inactive_now == pending_inactive, (pending_inactive_now as u64));
        ...
        withdraw(validator, pool_address, MAX_U128);
        assert!(total_coins_inactive(pool_address) == inactive, 0);
        (_, _, _, pending_inactive) = stake::get_stake(pool_address);
        assert!(pending_inactive == 0, (pending_inactive as u64));

        // withdraw entire inactive stake
        withdraw(delegator1, pool_address, MAX_U128);
        withdraw(delegator2, pool_address, MAX_U128);
        assert!(total_coins_inactive(pool_address) == 0, 0);
        (_, inactive, _, _) = stake::get_stake(pool_address);
        assert!(inactive == 0, (inactive as u64));
    }
    ...
    public entry fun test_reactivate_stake_single(
        ...
        unlock(validator, pool_address, 100 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 100 * ONE_EDS, 0, 100 * ONE_EDS);
        stake::assert_stake_pool(pool_address, 100 * ONE_EDS, 0, 0, 100 * ONE_EDS);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 100 * ONE_EDS);

        // add some stake to pending_active state
        stake::mint(validator, 150 * ONE_EDS);
        add_stake(validator, pool_address, 150 * ONE_EDS);

        let fee = get_add_stake_fee(pool_address, 150 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 250 * ONE_EDS - fee, 0, 100 * ONE_EDS);
        stake::assert_stake_pool(pool_address, 100 * ONE_EDS, 0, 150 * ONE_EDS, 100 * ONE_EDS);

        // can reactivate only pending_inactive stake
        reactivate_stake(validator, pool_address, 150 * ONE_EDS);

        assert_delegation(validator_address, pool_address, 350 * ONE_EDS - fee, 0, 0);
        stake::assert_stake_pool(pool_address, 200 * ONE_EDS, 0, 150 * ONE_EDS, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        ...
        unlock(validator, pool_address, 100 * ONE_EDS);
        assert!(endless_coin::balance(validator_address) == 15149999998, 0);
    ...
    public entry fun test_withdraw_many(
        ...
        stake::mint(delegator, 200 * ONE_EDS);
        add_stake(delegator, pool_address, 200 * ONE_EDS);
        ...
        assert_delegation(delegator_address, pool_address, 200 * ONE_EDS, 0, 0);
        ...
        withdraw(delegator, pool_address, MAX_U128);
        assert_delegation(delegator_address, pool_address, 200 * ONE_EDS, 0, 0);
        assert_delegation(validator_address, pool_address, 90899999999, 10100000000, 0);

        unlock(delegator, pool_address, 100 * ONE_EDS);
        ...
        let balance = endless_coin::balance(validator_address);
        withdraw(validator, pool_address, MAX_U128);
        assert!(endless_coin::balance(validator_address) == balance + 10100000000, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert_inactive_shares_pool(pool_address, 0, false, 0);
    ...
    public entry fun test_inactivate_no_excess_stake(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 1200 * ONE_EDS, true, true);
        ...
        unlock(validator, pool_address, 200 * ONE_EDS);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123)]
    public entry fun test_active_stake_rewards_single(
        endless_framework: &signer,
        validator: &signer,
    ) acquires DelegationPoolOwnership, DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        initialize_for_test(endless_framework);
        ...
        assert_delegation(validator_address, pool_address, 1010 * ONE_EDS, 0, 0);
        ...
        let fee = get_add_stake_fee(pool_address, 200 * ONE_EDS);
        assert_delegation(validator_address, pool_address, 1210 * ONE_EDS - fee, 0, 0);
        ...
        // add stake in pending_active state
        stake::mint(validator, 1000 * ONE_EDS);
        add_stake(validator, pool_address, 1000 * ONE_EDS);

        fee = get_add_stake_fee(pool_address, 1000 * ONE_EDS);
        let active_stake = 211717346653 - fee;
        assert_delegation(validator_address, pool_address, active_stake, 20199999998, 0);
        ...
        end_endless_epoch();
        // (212834520119  +  233345220835 * 1.01) active stake  + 20199999998 inactive stake
    ...
    public entry fun test_active_stake_rewards_multiple(
        ...
        // add stake in pending_active state
        stake::mint(delegator, 300 * ONE_EDS);
        add_stake(delegator, pool_address, 300 * ONE_EDS);

        let fee = get_add_stake_fee(pool_address, 300 * ONE_EDS);
        assert_delegation(delegator_address, pool_address, 300 * ONE_EDS - fee, 0, 0);
        assert_delegation(validator_address, pool_address, 200 * ONE_EDS, 0, 0);
        stake::assert_stake_pool(pool_address, 200 * ONE_EDS, 0, 300 * ONE_EDS, 0);

        end_endless_epoch();
        // `delegator` got its `add_stake` fee back and `validator` its active stake rewards
        assert_delegation(delegator_address, pool_address, 300 * ONE_EDS, 0, 0);
        assert_delegation(validator_address, pool_address, 20199999999, 0, 0);
        stake::assert_stake_pool(pool_address, 502 * ONE_EDS, 0, 0, 0);

        // delegators earn their own rewards from now on
        end_endless_epoch();
        assert_delegation(delegator_address, pool_address, 303 * ONE_EDS, 0, 0);
        ...
        stake::mint(delegator, 1000 * ONE_EDS);
        add_stake(delegator, pool_address, 1000 * ONE_EDS);
    ...
    public entry fun test_pending_inactive_stake_rewards(
        ...
        end_endless_epoch(); // 81000000001 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_endless_epoch(); // 81810000001 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_endless_epoch(); // 82628100001 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_endless_epoch(); // 83454381001 active stake * 1.01 + 20606019996 pending_inactive stake(now inactive)
        ...
        unlock(validator, pool_address, 200 * ONE_EDS);
        assert!(endless_coin::balance(validator_address) == 20606019996, 0);
        ...
        end_endless_epoch(); // 64288924812 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_endless_epoch(); // 64931814060 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01
        end_endless_epoch(); // 65581132200 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_endless_epoch(); // 66236943522 active stake * 1.01 + 20606019996 pending_inactive stake * 1.01
        assert_delegation(validator_address, pool_address, 66899312957, 0, 20812080195);

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_endless_epoch(); // 66899312957 active stake * 1.01 + 20812080195 pending_inactive stake * 1.01
        end_endless_epoch(); // 67568306086 active stake * 1.01 + 21020200996 pending_inactive stake(now inactive)
        end_endless_epoch(); // 68243989147 active stake * 1.01 + 21020200996 inactive stake
    ...
    public entry fun test_out_of_order_redeem(
        ...
        stake::mint(delegator1, 300 * ONE_EDS);
        add_stake(delegator1, pool_address, 300 * ONE_EDS);

        stake::mint(delegator2, 300 * ONE_EDS);
        add_stake(delegator2, pool_address, 300 * ONE_EDS);

        end_endless_epoch();

        // create the pending withdrawal of delegator 1 in lockup cycle 0
        unlock(delegator1, pool_address, 150 * ONE_EDS);
        ...
        unlock(delegator2, pool_address, 150 * ONE_EDS);
        ...
        assert!(endless_coin::balance(delegator1_address) == 15149999998, 0);
        assert!(endless_coin::balance(delegator2_address) == 5149999997, 0);

        // recreate the pending withdrawal of delegator 1 in lockup cycle 2
        unlock(delegator1, pool_address, 100 * ONE_EDS);
        ...
        assert!(endless_coin::balance(delegator2_address) == 15149999998, 0);
        ...
        assert!(endless_coin::balance(delegator1_address) == 15149999998 + 10099999998, 0);
        assert_pending_withdrawal(delegator1_address, pool_address, false, 0, false, 0);
    }
    ...
    public entry fun test_operator_fee(
        ...
        stake::assert_stake_pool(pool_address, 300 * ONE_EDS, 0, 0, 0);
        ...
        unlock(delegator2, pool_address, 100 * ONE_EDS);
        ...
        stake::mint(delegator1, 100 * ONE_EDS);
        assert!(get_add_stake_fee(pool_address, 100 * ONE_EDS) > 0, 0);
        add_stake(delegator1, pool_address, 100 * ONE_EDS);
        ...
        unlock(delegator2, pool_address, 100 * ONE_EDS);
        // 10807241561 - 100 EDS < `MIN_COINS_ON_SHARES_POOL` thus active stake is entirely unlocked
        ...
        primary_fungible_store::ensure_primary_store_exists(validator_address, endless_coin::get_metadata());
        synchronize_delegation_pool(pool_address);
        assert!(endless_coin::balance(validator_address) == 25536996, 0);
    ...
    #[test(endless_framework = @endless_framework, old_operator = @0x123, delegator = @0x010, new_operator = @0x020)]
    ...
    #[test(endless_framework = @endless_framework, operator1 = @0x123, delegator = @0x010, beneficiary = @0x020, operator2 = @0x030)]
    public entry fun test_set_beneficiary_for_operator(
        ...
        endless_account::create_account(operator1_address);

        let operator2_address = signer::address_of(operator2);
        endless_account::create_account(operator2_address);

        let beneficiary_address = signer::address_of(beneficiary);
        endless_account::create_account(beneficiary_address);
        ...
        stake::mint(delegator, 2000000 * ONE_EDS);
        add_stake(delegator, pool_address, 2000000 * ONE_EDS);
        unlock(delegator, pool_address, 1000000 * ONE_EDS);
        ...
        withdraw(operator1, pool_address, ONE_EDS);
        assert!(endless_coin::balance(operator1_address) == ONE_EDS - 1, 0);
        ...
        unlock(beneficiary, pool_address, ONE_EDS);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_endless_epoch();

        withdraw(beneficiary, pool_address, ONE_EDS);
        assert!(endless_coin::balance(beneficiary_address) == ONE_EDS - 1, 0);
        assert!(endless_coin::balance(operator1_address) == ONE_EDS - 1, 0);
        ...
        end_endless_epoch();
        unlock(operator2, pool_address, ONE_EDS);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_endless_epoch();

        withdraw(operator2, pool_address, ONE_EDS);
        assert!(endless_coin::balance(beneficiary_address) == ONE_EDS - 1, 0);
        assert!(endless_coin::balance(operator2_address) == ONE_EDS - 1, 0);
    }

    #[test(endless_framework = @endless_framework, operator = @0x123, delegator = @0x010)]
    public entry fun test_update_commission_percentage(
        ...
        assert!(get_add_stake_fee(pool_address, 100 * ONE_EDS) == 76756290, 0);
    ...
    public entry fun test_min_stake_is_preserved(
        ...
        initialize_for_test(endless_framework);
        initialize_test_validator(validator, 100 * ONE_EDS, true, false);
        ...
        // add stake without fees as validator is not active yet
        stake::mint(delegator1, 50 * ONE_EDS);
        add_stake(delegator1, pool_address, 50 * ONE_EDS);
        stake::mint(delegator2, 16 * ONE_EDS);
        add_stake(delegator2, pool_address, 16 * ONE_EDS);
        ...
        assert_delegation(delegator2_address, pool_address, 16 * ONE_EDS, 0, 0);
        unlock(delegator2, pool_address, 1);
        assert_delegation(delegator2_address, pool_address, 0, 0, 16 * ONE_EDS);
        reactivate_stake(delegator2, pool_address, 1);
        assert_delegation(delegator2_address, pool_address, 16 * ONE_EDS, 0, 0);

        unlock(delegator2, pool_address, ONE_EDS);
        assert_delegation(delegator2_address, pool_address, 0, 0, 16 * ONE_EDS);
        reactivate_stake(delegator2, pool_address, 2 * ONE_EDS);
        assert_delegation(delegator2_address, pool_address, 16 * ONE_EDS, 0, 0);
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123, delegator1 = @0x010)]
    ...
    public entry fun test_create_proposal_abort_if_inefficient_stake(
        ...
        initialize_for_test(endless_framework);
        endless_governance::initialize_for_test(
            endless_framework,
            (10 * ONE_EDS as u128),
            100 * ONE_EDS,
            1000,
        );
        endless_governance::initialize_partial_voting(endless_framework);
        ...
        account::create_account_for_test(delegator1_address);
        stake::mint(delegator1, 100 * ONE_EDS);
        add_stake(delegator1, pool_address, 10 * ONE_EDS);
        end_endless_epoch();
    ...
    #[test(endless_framework = @endless_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020, voter1 = @0x030, voter2 = @0x040)]
    public entry fun test_voting_power_change(
        ...
        initialize_for_test_no_reward(endless_framework);
        endless_governance::initialize_for_test(
            endless_framework,
            (10 * ONE_EDS as u128),
            100 * ONE_EDS,
        ...
        stake::mint(delegator1, 110 * ONE_EDS);
        add_stake(delegator1, pool_address, 10 * ONE_EDS);
        stake::mint(delegator2, 110 * ONE_EDS);
        add_stake(delegator2, pool_address, 90 * ONE_EDS);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 10 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 90 * ONE_EDS, 1);
        ...
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_endless_epoch();
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 10 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 10 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);

        // delegator1 adds stake to the pool. Voting power changes immediately.
        add_stake(delegator1, pool_address, 90 * ONE_EDS);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
        ...
        unlock(delegator1, pool_address, 90 * ONE_EDS);
        delegate_voting_power(delegator1, pool_address, voter2_address);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        ...
        withdraw(delegator1, pool_address, 45 * ONE_EDS);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);

        // voter2 adds stake for itself. Voting power changes immediately.
        stake::mint(voter2, 110 * ONE_EDS);
        add_stake(voter2, pool_address, 10 * ONE_EDS);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 110 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
    }

    #[test(endless_framework = @endless_framework, validator = @0x123, delegator1 = @0x010, voter1 = @0x030)]
    ...
    public entry fun test_voting_power_change_for_rewards(
        ...
        // 50% commission rate
        initialize_test_validator_custom(validator, 100 * ONE_EDS, true, false, 5000);
        ...
        add_stake(delegator2, pool_address, 90 * ONE_EDS);
        // By default, the voter of a delegator is itself.
        assert!(calculate_and_update_voter_total_voting_power(pool_address, validator_address) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 10 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 90 * ONE_EDS, 1);
        ...
        end_endless_epoch();
        end_endless_epoch();
        assert!(calculate_and_update_voter_total_voting_power(pool_address, validator_address) == 550 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 25 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 225 * ONE_EDS, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 375 * ONE_EDS, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
    }
    ...
    public entry fun test_voting_power_change_already_voted_before_partial(
        ...
        let proposal1_id = setup_vote(endless_framework, validator, false);
        ...
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let proposal2_id = endless_governance::create_proposal_v2_impl(
            validator,
            pool_address,
            execution_hash,
            b"",
        ...
        endless_governance::vote(validator, pool_address, proposal1_id, true);
        ...
        assert!(calculate_and_update_remaining_voting_power(pool_address, validator_address, proposal2_id) == 100 * ONE_EDS, 1);
        assert!(calculate_and_update_remaining_voting_power(pool_address, delegator1_address, proposal2_id) == 10 * ONE_EDS, 1);
        assert!(calculate_and_update_remaining_voting_power(pool_address, delegator2_address, proposal2_id) == 90 * ONE_EDS, 1);

        // Delegator1 tries to use 50 EDS to vote on proposal2, but it only has 10 EDS. So only 10 EDS voting power is used.
        vote(delegator1, pool_address, proposal2_id, 50 * ONE_EDS, true);
        assert!(calculate_and_update_remaining_voting_power(pool_address, delegator1_address, proposal2_id) == 0, 1);

        add_stake(delegator1, pool_address, 60 * ONE_EDS);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 70 * ONE_EDS, 1);
        vote(delegator1, pool_address, proposal2_id, 25 * ONE_EDS, true);
        assert!(calculate_and_update_remaining_voting_power(pool_address, delegator1_address, proposal2_id) == 35 * ONE_EDS, 1);
        vote(delegator1, pool_address, proposal2_id, 30 * ONE_EDS, false);
        assert!(calculate_and_update_remaining_voting_power(pool_address, delegator1_address, proposal2_id) == 5 * ONE_EDS, 1);
    }
    ...
    public entry fun test_vote_should_failed_if_already_voted_before_enable_partial_voting_flag(
        ...
        vote(delegator1, pool_address, proposal1_id, 10 * ONE_EDS, true);
    }
    ...
    public entry fun test_vote_should_failed_if_no_stake(
        ...
        // partial voing hasn't been enabled yet. A proposal has been created by the validator.
        let proposal1_id = setup_vote(endless_framework, validator, true);
    ...
    public entry fun test_delegate_voting_power_should_pass_even_if_no_stake(
        ...
        // partial voing hasn't been enabled yet. A proposal has been created by the validator.
        setup_vote(endless_framework, validator, true);
    ...
    public fun assert_delegation(
        ...
        // std::debug::print(&std::string::utf8(b"actual_active, actual_inactive, actual_pending_inactive"));
        // std::debug::print(&actual_active);
        // std::debug::print(&actual_inactive);
        // std::debug::print(&actual_pending_inactive);

        // if (actual_active != active_stake) {
        //     std::debug::print(&std::string::utf8(b"mistach active"));
        //     std::debug::print(&active_stake);
        // };

        // if (actual_inactive != inactive_stake) {
        //     std::debug::print(&std::string::utf8(b"mistach inactive"));
        //     std::debug::print(&inactive_stake);
        // };

        // if (actual_pending_inactive != pending_inactive_stake) {
        //     std::debug::print(&std::string::utf8(b"mistach pending_inactive"));
        //     std::debug::print(&pending_inactive_stake);
        // };

        assert!(actual_active == active_stake, (actual_active as u64));
        assert!(actual_inactive == inactive_stake, (actual_inactive as u64));
        assert!(actual_pending_inactive == pending_inactive_stake, (actual_pending_inactive as u64));
    }
    ...
    public fun assert_pending_withdrawal(
        ...
        assert!(withdrawal_stake == stake, (withdrawal_stake as u64));
    }
    ...
    public fun assert_inactive_shares_pool(
        ...
        if (exists) {
            ...
            assert!(actual_stake == stake, (actual_stake as u64));
        } else {
            assert!(0 == stake, 0);
        }
    }
    ...
    #[test_only]
    public fun total_coins_inactive(pool_address: address): u128 acquires DelegationPool {
