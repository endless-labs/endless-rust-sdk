...
module endless_framework::randomness_config {
    use std::string;
    use endless_std::copyable_any;
    use endless_std::copyable_any::Any;
    use endless_std::fixed_point64::FixedPoint64;
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfig {
        ...
        if (config_buffer::does_exist<RandomnessConfig>()) {
            ...
            if (exists<RandomnessConfig>(@endless_framework)) {
                *borrow_global_mut<RandomnessConfig>(@endless_framework) = new_config;
            } else {
    ...
    public fun enabled(): bool acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@endless_framework)) {
            let config = borrow_global<RandomnessConfig>(@endless_framework);
    ...
    public fun current(): RandomnessConfig acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@endless_framework)) {
            *borrow_global<RandomnessConfig>(@endless_framework)
        } else {
            new_off()
        }
    }
