...
module endless_framework::staking_config {
    use std::error;
    use std::features;
    ...
    use endless_std::fixed_point64::{Self, FixedPoint64, less_or_equal};
    use endless_std::math_fixed64;

    friend endless_framework::genesis;
    friend endless_framework::stake;
    ...
    const UN_INITIALIZATION_LAST_REWARDS_START_IN_SECS: u64 = 0;
    ...
    public(friend) fun initialize(
        ...
        move_to(endless_framework, StakingConfig {
            minimum_stake,
            maximum_stake,
        ...
        // Based on our economic model, the rates are as follows:
        // Initial inflation is 8%, decreasing by 20% annually thereafter until it stabilizes at 1%.
        let rewards_rate = fixed_point64::create_from_raw_value(324109293375076);
        let min_rewards_rate = fixed_point64::create_from_raw_value(41874109047320);
        let rewards_rate_period_in_secs = 31536000;
        let last_rewards_rate_period_start_in_secs = UN_INITIALIZATION_LAST_REWARDS_START_IN_SECS;
        let rewards_rate_decrease_rate = fixed_point64::create_from_raw_value(3689348814741910528);

        move_to(endless_framework, StakingRewardsConfig  {
            rewards_rate,
            min_rewards_rate,
    ...
    public fun get(): StakingConfig acquires StakingConfig {
        *borrow_global<StakingConfig>(@endless_framework)
    }
    ...
    public fun get_reward_rate(config: &StakingConfig): (u64, u64) acquires StakingRewardsConfig {
        if (features::periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = borrow_global<StakingRewardsConfig>(@endless_framework).rewards_rate;
    ...
    fun calculate_and_save_latest_rewards_config(): StakingRewardsConfig acquires StakingRewardsConfig {
        let staking_rewards_config = borrow_global_mut<StakingRewardsConfig>(@endless_framework);
        ...
        // if staking_reward_config is initialized and one epoch passed
        // set last_rewards_rate_period_start_in_secs from un_init_value to now(due to testcase limit, we cannot set to (now -7200))
        if ((staking_rewards_config.last_rewards_rate_period_start_in_secs == UN_INITIALIZATION_LAST_REWARDS_START_IN_SECS)
    ...
    public fun update_required_stake(
        ...
        let staking_config = borrow_global_mut<StakingConfig>(@endless_framework);
    ...
    #[test_only]
    use endless_std::fixed_point64::{equal, create_from_rational, create_from_raw_value};

    #[test(endless_framework = @endless_framework)]
    public entry fun test_change_staking_configs(endless_framework: signer) acquires StakingConfig {
        initialize(&endless_framework, 0, 1, 1, false, 1, 1, 1);

        update_required_stake(&endless_framework, 100, 1000);
        update_recurring_lockup_duration_secs(&endless_framework, 10000);
        update_rewards_rate(&endless_framework, 10, 100);
        update_voting_power_increase_limit(&endless_framework, 10);

        let config = borrow_global<StakingConfig>(@endless_framework);
    ...
    #[test(endless_framework = @endless_framework)]
    public entry fun test_staking_rewards_rate_decrease_over_time(endless_framework: signer) acquires StakingRewardsConfig {
    ...
    #[test(endless_framework = @endless_framework)]
    public entry fun test_change_staking_rewards_configs(endless_framework: signer) acquires StakingRewardsConfig {
        let start_time_in_secs: u64 = 100001000000;
        initialize_rewards_for_test(
            &endless_framework,
            create_from_rational(1, 100),
        ...
        let config = borrow_global<StakingRewardsConfig>(@endless_framework);
    ...
    #[test(account = @0x123, endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x50003, location = endless_framework::system_addresses)]
    public entry fun test_update_rewards_config_unauthorized_should_fail(account: signer, endless_framework: signer) acquires StakingRewardsConfig {
    ...
    #[test(endless_framework = @endless_framework)]
    public entry fun test_mainnet_staking_rewards_rate_decrease_over_time(endless_framework: signer) acquires StakingRewardsConfig {
        features::change_feature_flags(&endless_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);

        // Initial inflation is 10%, decreasing by 20% annually thereafter until it stabilizes at 1%.
        // reward rate is: 0.00002176
        let origin_rewards_rate = fixed_point64::create_from_raw_value(401401151043919);
        let min_rewards_rate = fixed_point64::create_from_raw_value(41874109047320);
        let last_rewards_rate_period_start_in_secs: u64 = 100001000000;
        let rewards_rate_decrease_rate = fixed_point64::create_from_raw_value(3689348814741910528);
        ...
        let epoch_reward_rate = calculate_and_save_latest_epoch_rewards_rate();
        assert!(equal(epoch_reward_rate, origin_rewards_rate), 0);

        // Rewards rate should not change after 6 months pass
        // until the current reward rate period ends.
        timestamp::fast_forward_seconds(ONE_YEAR_IN_SECS / 2);
        epoch_reward_rate = calculate_and_save_latest_epoch_rewards_rate();
        assert!(equal(epoch_reward_rate, origin_rewards_rate), 1);

        // Rewards rate should change till the current reward rate period ends.
        // Rewards rate decreases to  80% of previous reward rate.
        timestamp::fast_forward_seconds(ONE_YEAR_IN_SECS / 2);
        epoch_reward_rate = calculate_and_save_latest_epoch_rewards_rate();
        assert!(equal(epoch_reward_rate, create_from_raw_value(321120920835135)), 2);

        timestamp::fast_forward_seconds(ONE_YEAR_IN_SECS);
        epoch_reward_rate = calculate_and_save_latest_epoch_rewards_rate();
        assert!(equal(epoch_reward_rate, create_from_raw_value(256896736668107)), 2);

        let match_rates = vector[205517389334485, 164413911467587, 131531129174069, 105224903339255, 84179922671403,
            67343938137122, 53875150509697, 43100120407757, 41874109047320, 41874109047320];

        // decrease until rate hit min_rewards_rate at 12th year
        for (i in 3..13) {
            timestamp::fast_forward_seconds(ONE_YEAR_IN_SECS);
            epoch_reward_rate = calculate_and_save_latest_epoch_rewards_rate();
            // std::debug::print(&std::string::utf8(b"epoch_reward_rate"));
            // std::debug::print(&epoch_reward_rate);
            let matched_rate = std::vector::borrow<u128>(&match_rates, i-3);
            assert!(equal(epoch_reward_rate, create_from_raw_value(*matched_rate)), i);
        };
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_update_required_stake_invalid_range_should_fail(endless_framework: signer) acquires StakingConfig {
        update_required_stake(&endless_framework, 10, 5);
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_update_required_stake_zero_max_stake_should_fail(endless_framework: signer) acquires StakingConfig {
        update_required_stake(&endless_framework, 0, 0);
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10001, location = Self)]
    public entry fun test_update_required_lockup_to_zero_should_fail(endless_framework: signer) acquires StakingConfig {
        update_recurring_lockup_duration_secs(&endless_framework, 0);
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10002, location = Self)]
    public entry fun test_update_rewards_invalid_denominator_should_fail(endless_framework: signer) acquires StakingConfig {
        update_rewards_rate(&endless_framework, 1, 0);
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10005, location = Self)]
    public entry fun test_update_rewards_config_rewards_rate_greater_than_1_should_fail(endless_framework: signer) acquires StakingRewardsConfig {
        let start_time_in_secs: u64 = 100001000000;
        initialize_rewards_for_test(
            &endless_framework,
    ...
    public entry fun test_update_rewards_config_invalid_rewards_rate_decrease_rate_should_fail(endless_framework: signer) acquires StakingRewardsConfig {
        let start_time_in_secs: u64 = 100001000000;
        initialize_rewards_for_test(
            &endless_framework,
            create_from_rational(15981, 1000000000),
    ...
    public entry fun test_update_rewards_config_cannot_change_rewards_rate_period(endless_framework: signer) acquires StakingRewardsConfig {
        let start_time_in_secs: u64 = 100001000000;
        initialize_rewards_for_test(
            &endless_framework,
            create_from_rational(15981, 1000000000),
    ...
    public entry fun test_feature_flag_disabled_get_epoch_rewards_rate_should_fail(endless_framework: signer) acquires StakingRewardsConfig {
        features::change_feature_flags(&endless_framework, vector[], vector[features::get_periodical_reward_rate_decrease_feature()]);
    ...
    public entry fun test_update_voting_power_increase_limit_to_zero_should_fail(
        ...
        update_voting_power_increase_limit(&endless_framework, 0);
    }

    #[test(endless_framework = @endless_framework)]
    #[expected_failure(abort_code = 0x10004, location = endless_framework::staking_config)]
    public entry fun test_update_voting_power_increase_limit_to_more_than_upper_bound_should_fail(
        endless_framework: signer
    ) acquires StakingConfig {
        update_voting_power_increase_limit(&endless_framework, 51);
    }

    // For tests to bypass all validations.
    // StakingRewardsConfig as Endless Ecnomic Model Setting
    #[test_only]
    public fun initialize_for_test(
        ...
        if (!exists<StakingConfig>(@endless_framework)) {
            move_to(endless_framework, StakingConfig {
                minimum_stake,
                maximum_stake,
                recurring_lockup_duration_secs,
        ...
        if (!exists<StakingRewardsConfig>(@endless_framework)) {
            move_to(endless_framework, StakingRewardsConfig {
                rewards_rate: fixed_point64::create_from_raw_value(401401151043919),
                min_rewards_rate: fixed_point64::create_from_raw_value(41874109047320),
                rewards_rate_period_in_secs: 31536000,
                last_rewards_rate_period_start_in_secs: UN_INITIALIZATION_LAST_REWARDS_START_IN_SECS,
                rewards_rate_decrease_rate: fixed_point64::create_from_raw_value(3689348814741910528),
            });
        };
    }

    #[test_only]
    public fun override_rewards_for_test(
        _endless_framework: &signer,
        rewards_rate: FixedPoint64,
        min_rewards_rate: FixedPoint64,
        ...
        let reward_config = borrow_global_mut<StakingRewardsConfig>(@endless_framework);
        reward_config.rewards_rate = rewards_rate;
        reward_config.min_rewards_rate = min_rewards_rate;
        reward_config.rewards_rate_period_in_secs = rewards_rate_period_in_micros;
        reward_config.last_rewards_rate_period_start_in_secs = last_rewards_rate_period_start_in_secs;
        reward_config.rewards_rate_decrease_rate = rewards_rate_decrease_rate;
    }
