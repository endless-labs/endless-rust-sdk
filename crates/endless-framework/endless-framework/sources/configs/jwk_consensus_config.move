module endless_framework::jwk_consensus_config {
    use std::error;
    use std::option;
    ...
    public fun initialize(framework: &signer, config: JWKConsensusConfig) {
        ...
        if (!exists<JWKConsensusConfig>(@endless_framework)) {
    ...
    public(friend) fun on_new_epoch() acquires JWKConsensusConfig {
        if (config_buffer::does_exist<JWKConsensusConfig>()) {
            ...
            borrow_global_mut<JWKConsensusConfig>(@endless_framework).variant = new_config.variant;
        }
    }
    ...
    fun enabled(): bool acquires JWKConsensusConfig {
        let variant= borrow_global<JWKConsensusConfig>(@endless_framework).variant;
