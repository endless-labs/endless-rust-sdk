...
module endless_framework::config_buffer {
    use std::string::String;
    use endless_std::any;
    use endless_std::any::Any;
    ...
    friend endless_framework::jwks;
    friend endless_framework::jwk_consensus_config;
    friend endless_framework::randomness_config;
    ...
    public fun initialize(endless_framework: &signer) {
        if (!exists<PendingConfigs>(@endless_framework)) {
            move_to(endless_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@endless_framework)) {
            let config = borrow_global<PendingConfigs>(@endless_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@endless_framework);
