spec endless_framework::randomness {

    spec module {
        use endless_framework::chain_status;
        pragma verify = true;
        invariant [suspendable] chain_status::is_operating() ==> exists<PerBlockRandomness>(@endless_framework);
    ...
    spec initialize(framework: &signer) {
        ...
        aborts_if framework_addr != @endless_framework;
    ...
    spec on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) {
        ...
        ensures exists<PerBlockRandomness>(@endless_framework) ==> global<PerBlockRandomness>(@endless_framework).seed == seed_for_new_block;
        ensures exists<PerBlockRandomness>(@endless_framework) ==> global<PerBlockRandomness>(@endless_framework).epoch == epoch;
        ensures exists<PerBlockRandomness>(@endless_framework) ==> global<PerBlockRandomness>(@endless_framework).round == round;
    }

    spec next_32_bytes(): vector<u8> {
        use std::hash;
        include NextBlobAbortsIf;
        let input = b"ENDLESS_RANDOMNESS";
        let randomness = global<PerBlockRandomness>(@endless_framework);
    ...
    spec schema NextBlobAbortsIf {
        ...
        aborts_if !exists<PerBlockRandomness>(@endless_framework);
    }
    ...
    spec permutation(n: u64): vector<u64> {
        ...
        // aborts_if n > 1 && !exists<PerBlockRandomness>(@endless_framework);
    }
