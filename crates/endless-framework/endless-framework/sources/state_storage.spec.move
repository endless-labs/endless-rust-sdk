spec endless_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, the resources for tracking state storage usage and gas
    /// parameters must exist for the Endless framework address.
    /// Criticality: Critical
    /// Implementation: The initialize function ensures only the Endless framework address can call it.
    ...
    /// storage usage will be moved under the Endless framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the endless_framework account.
    ...
    /// usage will be moved under the Endless framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the endless_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@endless_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@endless_framework);
    }
    ...
    spec initialize(endless_framework: &signer) {
        ...
        /// [high-level-req-4]
        aborts_if !system_addresses::is_endless_framework_address(addr);
        /// [high-level-req-3]
        aborts_if exists<StateStorageUsage>(@endless_framework);
        ensures exists<StateStorageUsage>(@endless_framework);
        let post state_usage = global<StateStorageUsage>(@endless_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@endless_framework).epoch;
    }
