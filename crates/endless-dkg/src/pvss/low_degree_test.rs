...
impl<'a> LowDegreeTest<'a> {
    ...
    pub fn low_degree_test(self, evals: &[Scalar]) -> anyhow::Result<()> {
    ...
    pub fn low_degree_test_on_g1(self, evals: &[G1Projective]) -> anyhow::Result<()> {
        if evals.len() != self.n {
            bail!("Expected {} evaluations; got {}", self.n, evals.len())
        }
        ...
        debug_assert_eq!(evals.len(), v_times_f.len());
        let zero = g1_multi_exp(evals, v_times_f.as_slice());
    ...
    pub fn low_degree_test_on_g2(self, evals: &[G2Projective]) -> anyhow::Result<()> {
        if evals.len() != self.n {
            bail!("Expected {} evaluations; got {}", self.n, evals.len())
        }
        ...
        debug_assert_eq!(evals.len(), v_times_f.len());
        let zero = g2_multi_exp(evals, v_times_f.as_slice());
