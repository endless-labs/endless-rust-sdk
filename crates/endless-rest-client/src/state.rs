...
use endless_api_types::{
    X_ENDLESS_BLOCK_HEIGHT, X_ENDLESS_CHAIN_ID, X_ENDLESS_CURSOR, X_ENDLESS_EPOCH,
    X_ENDLESS_LEDGER_OLDEST_VERSION, X_ENDLESS_LEDGER_TIMESTAMP, X_ENDLESS_LEDGER_VERSION,
    X_ENDLESS_OLDEST_BLOCK_HEIGHT,
};
...
impl State {
    pub fn from_headers(headers: &reqwest::header::HeaderMap) -> anyhow::Result<Self> {
        let maybe_chain_id = headers
            .get(X_ENDLESS_CHAIN_ID)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_version = headers
            .get(X_ENDLESS_LEDGER_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_timestamp = headers
            .get(X_ENDLESS_LEDGER_TIMESTAMP)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        ...
        let maybe_oldest_ledger_version = headers
            .get(X_ENDLESS_LEDGER_OLDEST_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_block_height = headers
            .get(X_ENDLESS_BLOCK_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_oldest_block_height = headers
            .get(X_ENDLESS_OLDEST_BLOCK_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let cursor = headers
            .get(X_ENDLESS_CURSOR)
            .and_then(|h| h.to_str().ok())
            .map(|s| s.to_string());
