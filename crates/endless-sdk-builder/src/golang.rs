...
pub fn output(
    ...
    endless_module_path: Option<String>,
    package_name: String,
    abis: &[EntryABI],
) -> Result<()> {
        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "endless_account"
...
struct GoEmitter<T> {
    ...
    serde_module_path: Option<String>,
    /// Go module path for Endless types.
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let endless_types_package = match &self.endless_module_path {
            Some(path) => format!("{}/endlesstypes", path),
            None => "endlesstypes".into(),
        };
        let mut external_definitions =
            crate::common::get_external_definitions(&endless_types_package);
        // We need BCS for argument encoding and decoding
        external_definitions.insert(
            "github.com/endless-labs/serde-reflection/serde-generate/runtime/golang/bcs"
                .to_string(),
            Vec::new(),
        );
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Endless `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) endlesstypes.Script {{"#
            )?;
            self.out.indent();
...
func EncodeEntryFunction(call EntryFunctionCall) endlesstypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Endless `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *endlesstypes.Script) (ScriptCall, error) {{
...
func DecodeEntryFunctionPayload(script endlesstypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *endlesstypes.TransactionPayload__EntryFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}({}) endlesstypes.Script {{",
        ...
        writeln!(
            self.out,
            r#"return endlesstypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []endlesstypes.TypeTag{{{}}},
	Args: []endlesstypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) endlesstypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_upper_camel_case(),
            abi.name().to_upper_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &endlesstypes.TransactionPayload__EntryFunction {{
            endlesstypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []endlesstypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *endlesstypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script endlesstypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*endlesstypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(endlesstypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        let (constructor, stmt) = match type_tag {
            ...
            Struct(_) | Signer | Auth => common::type_not_allowed(type_tag),
        };
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg endlesstypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*endlesstypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_module_id(module_id: &ModuleId) -> String {
        format!(
            "endlesstypes.ModuleId {{ Address: {}, Name: {} }}",
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} endlesstypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            U256 => unimplemented!(),
            Address => "endlesstypes.AccountAddress".into(),
            Vector(type_tag) => {
            ...
            Signer | Auth => common::type_not_allowed(type_tag),
        }
    }
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*endlesstypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*endlesstypes.TransactionArgument__U8)(&{})", name),
            U16 => format!("(*endlesstypes.TransactionArgument__U16)(&{})", name),
            U32 => format!("(*endlesstypes.TransactionArgument__U32)(&{})", name),
            U64 => format!("(*endlesstypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*endlesstypes.TransactionArgument__U128)(&{})", name),
            U256 => format!("(*endlesstypes.TransactionArgument__U256)(&{})", name),
            Address => format!("&endlesstypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*endlesstypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            Struct(_) | Signer | Auth => common::type_not_allowed(type_tag),
        }
...
impl crate::SourceInstaller for Installer {
    ...
    fn install_transaction_builders(
        ...
        output(
            ...
            self.endless_module_path.clone(),
            name.to_string(),
            abis,
        )?;
        Ok(())
