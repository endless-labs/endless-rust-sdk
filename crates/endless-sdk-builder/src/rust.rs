...
/// If `local_types` is true, we generate a file suitable for the Endless codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targetting the Endless repository itself (as opposed to generated Endless types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("endless_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("endless_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("endless_types", vec![
                "AccountAddress",
                "TypeTag",
                "Script",
                "EntryFunction",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Endless `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    endless_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, endless_types_version: String) -> Self {
        Installer {
            install_dir,
            endless_types_version,
        }
    }
}
...
serde_bytes = "0.11.6"
endless-types = {{ path = "../endless-types", version = "{}" }}
"#,
            name, version, self.endless_types_version,
        )?;
