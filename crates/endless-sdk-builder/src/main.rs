...
//!
//! '''bash
//! cargo run -p endless-sdk-builder -- --help
//! '''
...
#[derive(Debug, Parser)]
#[clap(
    name = "Endless SDK Builder",
    about = "Generate boilerplate Endless SDKs"
)]
struct Options {
    ...
    /// Also install the endless types described by the given YAML file, along with the BCS runtime.
    #[clap(long)]
    with_endless_types: Option<PathBuf>,
    ...
    ///     and `endless_types` is assumed to be in "go_module/path/endless_types".
    #[clap(long)]
    ...
    /// Optional version number for the `endless_types` module (useful in Rust).
    /// If `--with-endless-types` is passed, this will be the version of the generated `endless_types` module.
    #[clap(long, default_value = "0.1.0")]
    endless_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `endless_types` dependency.
    #[clap(long)]
...
fn main() {
    ...
    let abis = endless_sdk_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    endless_sdk_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    endless_sdk_builder::golang::output(
                        &mut out,
    ...
    // Endless types
    if let Some(registry_file) = options.with_endless_types {
        ...
        if let Language::Rust = options.language {
            endless_sdk_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.endless_version_number == "0.1.0" {
                    "endless-types".to_string()
                } else {
                    format!("endless-types:{}", options.endless_version_number)
                },
                vec!["endless-types"],
            ),
            Language::Go => ("endlesstypes".to_string(), vec!["endlesstypes"]),
        };
    ...
    let installer: Box<
        dyn endless_sdk_builder::SourceInstaller<Error = Box<dyn std::error::Error>>,
    > = match options.language {
        Language::Rust => Box::new(endless_sdk_builder::rust::Installer::new(
            install_dir,
            options.endless_version_number,
        )),
        Language::Go => Box::new(endless_sdk_builder::golang::Installer::new(
            install_dir,
