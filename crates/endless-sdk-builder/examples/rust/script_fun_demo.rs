...
use endless_framework::{endless_coin_transfer, EntryFunctionCall};
use endless_types::AccountAddress;
...
fn demo_p2p_entry_function() {
    ...
    let payload = endless_coin_transfer(payee.clone(), amount);
    let function_call = EntryFunctionCall::decode(&payload);
    match function_call {
        Some(EntryFunctionCall::EndlessCoinTransfer { amount: a, to: p }) => {
            assert_eq!(a, amount);
