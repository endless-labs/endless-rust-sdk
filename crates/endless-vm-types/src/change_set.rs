...
use endless_types::{
    account_config::FungibleAssetWithdrawEvent,
...
impl VMChangeSet {
    ...
    pub fn get_payment_checksum(&self, sender: AccountAddress) -> HashValue {
        let mut data = Vec::new();
        let withdraw_event_type_tag = FungibleAssetWithdrawEvent::type_tag();
        for (event, _) in self.events() {
            if event.type_tag() == &withdraw_event_type_tag {
                let Ok(evt) = bcs::from_bytes::<FungibleAssetWithdrawEvent>(event.event_data())
                else {
                    continue;
                };
                if evt.owner == sender {
                    data.push((evt.store, evt.amount));
                }
            }
        }
        // TODO: Need to sort?
        // data.sort_by_key(|x| x.store);
        let data = bcs::to_bytes(&data).unwrap();
        HashValue::sha3_256_of(&data)
    }
}
