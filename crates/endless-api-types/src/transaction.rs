...
use crate::{
    Address, EndlessError, EntryFunctionId, EventGuid, HashValue, HexEncodedBytes,
...
use endless_types::{
    ...
    contract_event::{ContractEvent, EventWithVersion, EventWithVersionIdx},
    oidb,
    transaction::{
        authenticator::{
...
pub struct TransactionOnChainData {
    ...
    pub transaction: endless_types::transaction::Transaction,
    /// Information about the transaction
    pub info: endless_types::transaction::TransactionInfo,
    /// Events emitted by the transaction
    pub events: Vec<ContractEvent>,
    /// The accumulator root hash at this version
    pub accumulator_root_hash: endless_crypto::HashValue,
    /// Final state of resources changed by the transaction
    pub changes: endless_types::write_set::WriteSet,
}

impl From<(TransactionWithProof, endless_crypto::HashValue)> for TransactionOnChainData {
    fn from(
        (txn, accumulator_root_hash): (TransactionWithProof, endless_crypto::HashValue),
    ) -> Self {
        Self {
            version: txn.version,
...
impl
    From<(
        u64,
        endless_types::transaction::Transaction,
        endless_types::transaction::TransactionInfo,
        Vec<ContractEvent>,
        endless_crypto::HashValue,
        endless_types::write_set::WriteSet,
    )> for TransactionOnChainData
{
    fn from(
...
/// Enum of the different types of transactions in Endless
...
impl Transaction {
    ...
    pub fn events(&self) -> anyhow::Result<&Vec<Event>> {
        Ok(match self {
            ...
            Transaction::PendingTransaction(_txn) => {
                bail!("pending transaction does not have Event")
            },
            Transaction::GenesisTransaction(txn) => &txn.events,
            Transaction::StateCheckpointTransaction(_txn) => {
                bail!("state checkpoint transaction does not have Event")
            },
            Transaction::ValidatorTransaction(txn) => &txn.events,
        })
    }
}
...
impl From<(&EventWithVersionIdx, serde_json::Value)> for VersionedEventV2 {
    fn from((event, data): (&EventWithVersionIdx, serde_json::Value)) -> Self {
        let type_tag = match &event.event {
            ContractEvent::V1(v1) => v1.type_tag().clone(),
            ContractEvent::V2(v2) => v2.type_tag().clone(),
        };
        Self {
            version: event.transaction_version.into(),
...
pub enum TransactionPayload {
    ...
    SafeEntryFunctionPayload(SafeEntryFunctionPayload),
    SafeScriptPayload(SafeScriptPayload),
}
...
impl VerifyInput for TransactionPayload {
    fn verify(&self) -> anyhow::Result<()> {
        match self {
            ...
            TransactionPayload::SafeEntryFunctionPayload(inner) => inner.verify(),
            TransactionPayload::SafeScriptPayload(inner) => inner.verify(),
...
/// Similar to `EntryFunctionPayload` but with a payment checksum
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
pub struct SafeEntryFunctionPayload {
    pub function: EntryFunctionId,
    /// Type arguments of the function
    pub type_arguments: Vec<MoveType>,
    /// Arguments of the function
    pub arguments: Vec<serde_json::Value>,
    /// The payment checksum
    pub payment_checksum: HashValue,
}

impl SafeEntryFunctionPayload {
    pub fn to_entry_function_payload(&self) -> EntryFunctionPayload {
    ...
    pub fn set_payment_checksum(&mut self, payment_checksum: HashValue) {
        self.payment_checksum = payment_checksum
    }
}

impl VerifyInput for SafeEntryFunctionPayload {
    fn verify(&self) -> anyhow::Result<()> {
        self.function.verify()?;
        for type_arg in self.type_arguments.iter() {
            type_arg.verify(0)?;
...
/// Similar to ScriptPayload but with a payment checksum
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
pub struct SafeScriptPayload {
    pub code: MoveScriptBytecode,
    /// Type arguments of the function
    pub type_arguments: Vec<MoveType>,
    /// Arguments of the function
...
impl SafeScriptPayload {
    pub fn to_script_payload(&self) -> ScriptPayload {
        ScriptPayload {
...
impl VerifyInput for SafeScriptPayload {
    fn verify(&self) -> anyhow::Result<()> {
        for type_arg in self.type_arguments.iter() {
            type_arg.verify(0)?;
        }
...
impl TryFrom<(Script, HashValue)> for SafeScriptPayload {
    type Error = anyhow::Error;

    fn try_from((script, payment_checksum): (Script, HashValue)) -> anyhow::Result<Self> {
        let (code, ty_args, args) = script.into_inner();
        Ok(Self {
            code: MoveScriptBytecode::new(code).try_parse_abi(),
            type_arguments: ty_args.into_iter().map(|arg| arg.into()).collect(),
...
pub enum TransactionSignature {
    ...
    SingleSender(SingleSenderSignature),
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
pub struct SingleSenderSignature {
...
impl VerifyInput for TransactionSignature {
    fn verify(&self) -> anyhow::Result<()> {
        match self {
            ...
            TransactionSignature::SingleSender(inner) => inner.sender.verify(),
        }
    }
}
    ...
    fn try_from(ts: TransactionSignature) -> anyhow::Result<Self> {
        Ok(match ts {
            ...
            TransactionSignature::SingleSender(sig) => {
                TransactionAuthenticator::single_sender(sig.sender.try_into()?)
            },
        })
    }
}
...
impl TryFrom<MultiEd25519Signature> for TransactionAuthenticator {
    ...
    fn try_from(value: MultiEd25519Signature) -> Result<Self, Self::Error> {
        ...
        Ok(TransactionAuthenticator::multi_ed25519(
            ...
            endless_crypto::multi_ed25519::MultiEd25519Signature::new_with_signatures_and_bitmap(
                ed25519_signatures,
...
#[oai(one_of, discriminator_name = "type", rename_all = "snake_case")]
pub enum Signature {
    Ed25519(SignatureHexEncoded),
    Secp256k1Ecdsa(SignatureHexEncoded),
    WebAuthn(SignatureHexEncoded),
    Oidb(SignatureHexEncoded),
    Bls12381(SignatureHexEncoded),
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
pub struct SignatureHexEncoded {
    pub signature: HexEncodedBytes,
}

impl<T: Into<HexEncodedBytes>> From<T> for SignatureHexEncoded {
    fn from(signature: T) -> Self {
        Self {
...
impl TryFrom<Signature> for AnySignature {
    ...
    fn try_from(signature: Signature) -> Result<Self, Self::Error> {
        Ok(match signature {
            Signature::Ed25519(s) => AnySignature::ed25519(s.signature.inner().try_into()?),
            Signature::Secp256k1Ecdsa(s) => {
                AnySignature::secp256k1_ecdsa(s.signature.inner().try_into()?)
            },
            Signature::WebAuthn(s) => AnySignature::webauthn(s.signature.inner().try_into()?),
            Signature::Oidb(s) => AnySignature::oidb(s.signature.inner().try_into()?),
            Signature::Bls12381(s) => AnySignature::bls12381(s.signature.inner().try_into()?),
        })
    }
}
...
impl From<AnySignature> for Signature {
    fn from(signature: AnySignature) -> Self {
        match signature {
            ...
            AnySignature::Bls12381 { signature } => {
                Signature::Bls12381(signature.to_bytes().to_vec().into())
            },
        }
    }
}
...
#[oai(one_of, discriminator_name = "type", rename_all = "snake_case")]
pub enum PublicKey {
    Ed25519(PublicKeyHexEncoded),
    Secp256k1Ecdsa(PublicKeyHexEncoded),
    Secp256r1Ecdsa(PublicKeyHexEncoded),
    Oidb(PublicKeyHexEncoded),
    Bls12381(PublicKeyHexEncoded),
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
pub struct PublicKeyHexEncoded {
    pub public_key: HexEncodedBytes,
}

impl<T: Into<HexEncodedBytes>> From<T> for PublicKeyHexEncoded {
...
impl TryFrom<PublicKey> for AnyPublicKey {
    ...
    fn try_from(public_key: PublicKey) -> Result<Self, Self::Error> {
        Ok(match public_key {
            PublicKey::Ed25519(p) => AnyPublicKey::ed25519(p.public_key.inner().try_into()?),
            PublicKey::Secp256k1Ecdsa(p) => {
                AnyPublicKey::secp256k1_ecdsa(p.public_key.inner().try_into()?)
            },
            PublicKey::Secp256r1Ecdsa(p) => {
                AnyPublicKey::secp256r1_ecdsa(p.public_key.inner().try_into()?)
            },
            PublicKey::Oidb(p) => AnyPublicKey::oidb(p.public_key.inner().try_into()?),
            PublicKey::Bls12381(p) => AnyPublicKey::bls12381(bcs::from_bytes(&p.public_key.0)?),
        })
    }
}
...
impl From<AnyPublicKey> for PublicKey {
    fn from(key: AnyPublicKey) -> Self {
        match key {
            ...
            AnyPublicKey::Bls12381 { public_keys } => {
                PublicKey::Bls12381(bcs::to_bytes(&public_keys).unwrap().into())
            },
        }
    }
}
    ...
    fn try_from(value: SingleKeySignature) -> Result<Self, Self::Error> {
        let key =
            match value.public_key {
                ...
                public_key @ PublicKey::Bls12381(_) => public_key
                    .try_into()
                    .context("Failed to parse given public key bytes as Bls12381PublicKey")?,
            };
        ...
        let signature = match value.signature {
            ...
            Signature::Bls12381(s) => {
                let signature = s
                    .inner()
                    .try_into()
                    .context("Failed to parse given signature bytes as Bls12381Signature")?;
                AnySignature::bls12381(signature)
            },
        };
    ...
    fn try_from(value: MultiKeySignature) -> Result<Self, Self::Error> {
        ...
        for public_key in value.public_keys {
            let key = match public_key {
                ...
                public_key @ PublicKey::Bls12381(_) => public_key
                    .try_into()
                    .context("Failed to parse given public_key bytes as Bls12381PublicKey")?,
            };
                ...
                match indexed_signature.signature {
                    ...
                    Signature::Bls12381(s) => {
                        let signature = s
                            .inner()
                            .try_into()
                            .context("Failed to parse given signature as Bls12381Signature")?;
                        AnySignature::bls12381(signature)
                    },
                };
    ...
    fn from(auth: &AccountAuthenticator) -> Self {
        ...
        match auth {
            ...
            MultiAuthKey {
                ...
                public_keys: public_keys.iter().map(|pk| pk.clone().into()).collect(),
                signatures: signatures
                    .iter()
                    .enumerate()
                    .map(|(index, signature)| IndexedSignature {
                    ...
                    .collect(),
                signatures_required: public_keys.len() as u8,
            }),
            Bls12381 {
                public_key,
                signature,
            } => Self::SingleKeySignature(SingleKeySignature {
...
impl From<TransactionAuthenticator> for TransactionSignature {
    fn from(auth: TransactionAuthenticator) -> Self {
        ...
        match &auth {
            ...
            SingleSender { sender } => Self::SingleSender(SingleSenderSignature {
                sender: sender.into(),
            }),
        }
    }
