...
pub struct HashValue(pub endless_crypto::hash::HashValue);

impl From<endless_crypto::hash::HashValue> for HashValue {
    fn from(val: endless_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for endless_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<endless_crypto::hash::HashValue>()?.into())
        } else {
            match bs58::decode(s).into_vec() {
                Ok(bytes) if bytes.len() == 32 => {
                    Ok(endless_crypto::hash::HashValue::from_slice(&bytes)?.into())
                },
                _ => Ok(s.parse::<endless_crypto::hash::HashValue>()?.into()),
            }
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(endless_crypto::hash::HashValue::zero())
    }
}
