...
use crate::{
    ...
    types::{
        code_invariant_error, expect_ok, DelayedFieldCompare, DelayedFieldValue,
...
impl DelayedFieldData {
    pub fn compare(
        ...
        match (method, other) {
            // u128 always >= 0
            (DelayedFieldCompare::GreaterThanOrEqual, 0) => return Ok(true),
            (DelayedFieldCompare::LessThan, 0) => return Ok(false),
            // aggregator always <= max_value
            (DelayedFieldCompare::GreaterThanOrEqual, v) if v > max_value => return Ok(false),
            (DelayedFieldCompare::GreaterThan, v) if v >= max_value => return Ok(false),
            (DelayedFieldCompare::LessThanOrEqual, v) if v >= max_value => return Ok(true),
            (DelayedFieldCompare::LessThan, v) if v > max_value => return Ok(true),
            _ => (),
        }
        match self.delayed_fields.get(&id) {
            Some(DelayedChange::Create(DelayedFieldValue::Aggregator(value))) => {
                Ok(method.compare(*value, other))
            },
            Some(DelayedChange::Apply(DelayedApplyChange::AggregatorDelta {
                delta: previous_delta,
            })) => Ok(resolver.delayed_field_compare(
                &id,
            ...
            None => Ok(resolver.delayed_field_compare(
                &id,
            ...
            _ => Err(
                code_invariant_error("Tried to compare to a non-aggregator delayed field").into(),
            ),
        }
    }
