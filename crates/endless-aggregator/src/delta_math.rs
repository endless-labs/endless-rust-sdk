...
use crate::{
    ...
    types::{
        expect_ok, DelayedFieldCompare, DelayedFieldsSpeculativeError,
        DeltaApplicationFailureReason, DeltaHistoryMergeOffsetFailureReason, PanicOr,
    },
};
...
impl DeltaHistory {
    ...
    pub fn record_compare(
        ...
        match (method, value) {
            (DelayedFieldCompare::LessThan, true) => {
                self.record_success(SignedU128::Positive(max_value - other + 1))
            },
            (DelayedFieldCompare::LessThan, false) => self.record_overflow(max_value - other + 1),
            (DelayedFieldCompare::LessThanOrEqual, true) => {
                self.record_success(SignedU128::Positive(max_value - other))
            },
            (DelayedFieldCompare::LessThanOrEqual, false) => {
                self.record_overflow(max_value - other)
            },
            (DelayedFieldCompare::GreaterThan, true) => {
                self.record_success(SignedU128::Negative(other + 1))
            },
            (DelayedFieldCompare::GreaterThan, false) => self.record_underflow(other + 1),
            (DelayedFieldCompare::GreaterThanOrEqual, true) => {
                self.record_success(SignedU128::Negative(other))
            },
            (DelayedFieldCompare::GreaterThanOrEqual, false) => self.record_underflow(other),
        }
    }
