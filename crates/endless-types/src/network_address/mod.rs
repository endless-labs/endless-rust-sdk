...
/// ## Overview
///
/// Endless `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a EndlessNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the EndlessNet concept of a
...
///
/// use endless_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use endless_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use endless_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical EndlessNet address with
    /// associated protocols.
    ///
    /// "typical" EndlessNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_endlessnet_addr());
    /// ```
    pub fn is_endlessnet_addr(&self) -> bool {
        parse_endlessnet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_endlessnet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_endlessnet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_endlessnet_protos).prop_map(
        |(mut transport_protos, mut endlessnet_protos)| {
            transport_protos.append(&mut endlessnet_protos);
...
/// parse canonical endlessnet protocols
///
/// See: [`NetworkAddress::is_endlessnet_addr`]
fn parse_endlessnet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_endlessnet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_endlessnet_addr()) {
        ...
        #[test]
        fn test_is_endlessnet_addr(addr in arb_endlessnet_addr()) {
            assert!(addr.is_endlessnet_addr(), "addr.is_endlessnet_addr() = false; addr: '{}'", addr);
        }
    }
}
