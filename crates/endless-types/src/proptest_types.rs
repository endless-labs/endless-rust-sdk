...
fn new_raw_transaction(
    ...
    match payload {
        ...
        TransactionPayload::Script(script) | TransactionPayload::SafeScript(script, _) => {
        ...
        | TransactionPayload::SafeEntryFunction(script_fn, _) => {
...
impl AccountResourceGen {
    pub fn materialize(
        ...
        let auth_key = AuthenticationKey::from_preimage(
            account_info.public_key.to_bytes().to_vec(),
            Scheme::Ed25519,
        );
        AccountResource::new(account_info.sequence_number, vec![auth_key])
    }
}
...
impl AccountStateGen {
    pub fn materialize(self, account_index: Index, universe: &AccountInfoUniverse) -> AccountState {
        ...
        AccountState::try_from((address, &account_resource)).unwrap()
    }
}
