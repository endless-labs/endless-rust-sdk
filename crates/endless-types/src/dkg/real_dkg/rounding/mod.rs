...
use endless_dkg::pvss::WeightedConfig;
...
pub fn total_weight_upper_bound(
    ...
    let stake_sum = validator_stakes.iter().sum::<u128>();
...
impl DKGRoundingProfile {
    pub fn new(
        ...
        // todo: remove once endless-dkg supports 0 weights
        if !is_valid_profile(&best_profile, reconstruct_threshold_in_stake_ratio) {
...
fn compute_profile_fixed_point(
    ...
    let stake_sum: u128 = validator_stakes.iter().sum();
    let stake_sum_fixed = U64F64::from_num(stake_sum);
    let stake_per_weight: u64 = max(1, stake_sum as u64 / weights_sum);
