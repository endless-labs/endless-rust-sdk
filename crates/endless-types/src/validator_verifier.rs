...
impl Arbitrary for ValidatorConsensusInfo {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        (any::<AccountAddress>(), any::<PublicKey>(), any::<u64>())
            .prop_map(
                |(address, public_key, voting_power)| ValidatorConsensusInfo {
                    address,
                    public_key,
                    voting_power: voting_power as u128,
                },
            )
            .boxed()
    }
...
impl ValidatorConsensusInfo {
    pub fn new(address: AccountAddress, public_key: PublicKey, voting_power: u128) -> Self {
...
impl ValidatorVerifier {
    ...
    pub fn get_voting_power(&self, author: &AccountAddress) -> Option<u128> {
        self.address_to_validator_index
            .get(author)
...
fn sum_voting_power(address_to_validator_info: &[ValidatorConsensusInfo]) -> u128 {
    address_to_validator_info.iter().fold(0, |sum, x| {
        sum.checked_add(x.voting_power)
            .expect("sum of all voting power is greater than u128::max")
    })
}
...
mod tests {
    use super::*;
    use crate::validator_signer::ValidatorSigner;
    use endless_crypto::test_utils::{TestEndlessCrypto, TEST_SEED};
    ...
    fn test_check_voting_power() {
        ...
        let dummy_struct = TestEndlessCrypto("Hello, World".to_string());
