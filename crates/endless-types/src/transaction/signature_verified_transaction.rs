...
use endless_crypto::{hash::CryptoHash, HashValue};
...
impl BlockExecutableTransaction for SignatureVerifiedTransaction {
    ...
    fn is_block_metadata(&self) -> bool {
        matches!(
            self,
            SignatureVerifiedTransaction::Valid(Transaction::BlockMetadata(_))
                | SignatureVerifiedTransaction::Valid(Transaction::BlockMetadataExt(_))
        )
    }
}
