...
use crate::{
    ...
    fee_statement::FeeStatementEvent, proof::accumulator::InMemoryEventAccumulator,
...
impl RawTransaction {
    ...
    /// Create a new `RawTransaction` of safe script type.
    pub fn new_safe_script(
        sender: AccountAddress,
        sequence_number: u64,
        script: Script,
        payment_checksum: HashValue,
        max_gas_amount: u64,
        gas_unit_price: u64,
        expiration_timestamp_secs: u64,
        chain_id: ChainId,
        ...
        RawTransaction {
            sender,
            sequence_number,
            payload: TransactionPayload::SafeScript(script, payment_checksum),
            max_gas_amount,
            gas_unit_price,
            expiration_timestamp_secs,
            chain_id,
    ...
    /// Create a new `RawTransaction` of safe entry function type.
    pub fn new_safe_entry_function(
        sender: AccountAddress,
        sequence_number: u64,
        entry_function: EntryFunction,
        payment_checksum: HashValue,
        ...
        RawTransaction {
            sender,
            sequence_number,
            payload: TransactionPayload::SafeEntryFunction(entry_function, payment_checksum),
            max_gas_amount,
            gas_unit_price,
            expiration_timestamp_secs,
            chain_id,
...
pub enum TransactionPayload {
    ...
    Multisig(Multisig),
    /// Similar to Script, but with a payment checksum.
    /// Payment checksum is a hash of all coin withdraw happens in this transaction.
    /// Payment checksum only focus on `sender` owned fungible asset stores, and
    /// its data source is events, so gas fee is not included in the checksum.
    SafeScript(Script, HashValue),
    /// Similar to EntryFunction, but with a payment checksum.
    /// Read above the comment of `SafeScript`
    SafeEntryFunction(EntryFunction, HashValue),
}
...
impl TransactionWithProof {
    ...
    pub fn verify(&self, ledger_info: &LedgerInfo) -> Result<()> {
        let bs58_encode = bs58::encode(self.transaction.hash().as_slice()).into_string();
        ensure!(
            bs58_encode == self.proof.transaction_info().transaction_hash(),
...
impl From<KeptVMStatus> for ExecutionStatus {
    fn from(kept_status: KeptVMStatus) -> Self {
        match kept_status {
            ...
            KeptVMStatus::PaymentChecksumMismatch => ExecutionStatus::PaymentChecksumMismatch,
        }
    }
}
...
impl VMValidatorResult {
    pub fn new(vm_status: Option<DiscardedVMStatus>, score: u64) -> Self {
        // In sponsored transaction, the discard status will be execution failure.
        // So we should not assert the status here.
...
impl TransactionOutput {
    ...
    pub fn try_extract_fee_statement(&self) -> Result<Option<FeeStatementEvent>> {
        ...
        for event in self.events.iter().rev() {
            if let Some(fee_statement) = event.try_v2_typed(&FEE_STATEMENT_EVENT_TYPE)? {
                println!("Found fee statement: {:?}", fee_statement);
...
impl TransactionInfoV0 {
    pub fn new(
        ...
        Self {
            transaction_hash: bs58::encode(transaction_hash.as_slice()).into_string(),
            gas_used,
            status,
            event_root_hash,
            state_change_hash,
    ...
    pub fn verify(
        ...
        self.transactions
            ...
            .map(|(txn, txn_info)| {
                ...
                let bs58_encode = bs58::encode(txn_hash.as_slice()).into_string();
                ensure!(
                    bs58_encode == txn_info.transaction_hash(),
...
/// `Transaction` will be the transaction type used internally in the endless node to represent the
...
pub enum Transaction {
    ...
    /// Transaction that applies a WriteSet to the current storage, it's applied manually via endless-db-bootstrapper.
...
pub trait BlockExecutableTransaction: Sync + Send + Clone + 'static {
    ...
    fn is_block_metadata(&self) -> bool {
        false
    }
}
