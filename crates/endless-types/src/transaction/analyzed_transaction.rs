...
use crate::{
    ...
    account_config::{
        eds_metadata, primary_store, AccountResource, MetadataResource, ObjectGroupResource,
    },
    chain_id::ChainId,
...
pub fn endless_coin_info_location() -> StorageLocation {
    StorageLocation::Specific(StateKey::access_path(AccessPath::new(
        AccountAddress::ONE,
        MetadataResource::struct_tag().access_vector(),
    )))
}

pub fn coin_store_location(address: AccountAddress) -> StorageLocation {
    let store = primary_store(&address, &eds_metadata());
    StorageLocation::Specific(StateKey::access_path(
        AccessPath::resource_group_access_path(store, ObjectGroupResource::struct_tag()),
    ))
}
                ...
                TransactionPayload::EntryFunction(func) => {
                    match (
                        *func.module().address(),
                        ...
                        (AccountAddress::ONE, "endless_coin", "transfer") => {
                        ...
                        (AccountAddress::ONE, "endless_account", "transfer") => {
                            let sender_address = signed_txn.sender();
                            let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
                            rw_set_for_coin_transfer(
                                sender_address,
                        ...
                        (AccountAddress::ONE, "endless_account", "create_account") => {
                            let sender_address = signed_txn.sender();
                            let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
