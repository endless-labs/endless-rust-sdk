...
use endless_ledger::EndlessLedgerError;
use endless_types::event::EventKey;
...
/// LocalAccount represents an account on the Endless blockchain. Internally it
...
impl LocalAccount {
    ...
    /// does not actually create an account on the Endless blockchain, just a
    ...
    /// create an account on the Endless blockchain, it just generates a new
    /// account locally.
...
impl HardwareWalletAccount {
    ...
    /// This requires the Ledger device to be connected, unlocked and the Endless app to be opened
    pub fn from_ledger(
        derivation_path: String,
        sequence_number: u64,
    ) -> Result<Self, EndlessLedgerError> {
        let public_key = endless_ledger::get_public_key(&derivation_path, false)?;
    ...
    pub fn sign_arbitrary_message(
        ...
        endless_ledger::sign_message(&self.derivation_path, message)
    }
}
...
mod tests {
    ...
    fn test_recover_account_from_derive_path() {
        ...
        // https://github.com/endless-labs/endless-core/blob/main/ecosystem/typescript/sdk/src/endless_account.test.ts
