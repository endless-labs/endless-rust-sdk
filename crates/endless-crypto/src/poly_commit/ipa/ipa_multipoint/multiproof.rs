...
use crate::poly_commit::ipa::{
    banderwagon::{trait_defs::*, Element, Fr},
    ipa_multipoint::{
        crs::CRS,
        ipa::{slow_vartime_multiscalar_mul, IPAProof},
        lagrange_basis::{LagrangeBasis, PrecomputedWeights},
        math_utils::powers_of,
...
impl MultiPointProof {
    pub fn from_bytes(
        bytes: &[u8],
        poly_degree: usize,
    ) -> crate::poly_commit::ipa::ipa_multipoint::IOResult<MultiPointProof> {
        use crate::poly_commit::ipa::ipa_multipoint::{IOError, IOErrorKind};
    ...
    pub fn to_bytes(&self) -> crate::poly_commit::ipa::ipa_multipoint::IOResult<Vec<u8>> {
    ...
    pub fn to_bytes_uncompressed(
        &self,
    ) -> crate::poly_commit::ipa::ipa_multipoint::IOResult<Vec<u8>> {
...
pub fn open_point_outside_of_domain(
    crs: &CRS,
    precomp: &PrecomputedWeights,
    transcript: &mut Transcript,
    ...
    crate::poly_commit::ipa::ipa_multipoint::ipa::create(
        transcript,
        crs.clone(),
        a,
        commitment,
...
fn open_multiproof_lagrange_2_polys() {
    ...
    let mut transcript = Transcript::new(b"foo");
    let multiproof = MultiPoint::open(&crs, &precomp, &mut transcript, vec![
        prover_query_i.clone(),
        prover_query_j.clone(),
    ]);
...
fn test_ipa_consistency() {
    use crate::poly_commit::ipa::{
        banderwagon::trait_defs::*, ipa_multipoint::math_utils::inner_product,
    };
    let n = 256;
...
fn multiproof_consistency() {
    use crate::poly_commit::ipa::banderwagon::trait_defs::*;
    let n = 256;
    let crs = CRS::new(n, b"eth_verkle_oct_2021");
    let precomp = PrecomputedWeights::new(n);
    ...
    let mut prover_transcript = Transcript::new(b"test");
    let multiproof = MultiPoint::open(&crs, &precomp, &mut prover_transcript, vec![
        prover_query_a.clone(),
        prover_query_b.clone(),
    ]);
