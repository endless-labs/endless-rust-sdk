use crate::poly_commit::ipa::{
    banderwagon::{try_reduce_to_element, Element},
    ipa_multipoint::{
        default_crs, ipa::slow_vartime_multiscalar_mul, lagrange_basis::LagrangeBasis,
    },
};
...
impl CRS {
    ...
    pub fn get_first_n_elements(&self, n: usize) -> CRS {
        assert!(n <= self.n);
        CRS {
            n,
            G: self.G.split_at(n).0.to_vec(),
            Q: self.Q,
        }
    }
}
