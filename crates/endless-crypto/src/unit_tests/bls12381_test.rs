...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestEndlessCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestEndlessCrypto {
    TestEndlessCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestEndlessCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestEndlessCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestEndlessCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestEndlessCrypto>>();
...
/// endless-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    let pk = PublicKey::try_from(
        hex::decode(
            "adcb95ffa73c2727e3106d27bc2a5d37826538096b736742eb69f4e9a5aa7ec4eaf9d9c277ba12d8660ebaed1261ef77"
        ).unwrap().as_slice()
    ).unwrap();

    let sig = bls12381::Signature::try_from(
        hex::decode(
            "867ca36140ca9d6d961fb5b8a353cd97882c7a57a51fa8fba572cd2168beaa755b50e2e6746213a1af64d908f3adb5f0056248072818f32d1b4721b9bae8be2e65c1e50499fdcf487df7c6017d8cc5905820aa9c42e98972f1c5370cfc84b7fc"
        ).unwrap().as_slice()
    ).unwrap();

    assert!(sig.verify_arbitrary_msg(b"Hello Endless!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    let message = TestEndlessCrypto("test".to_owned());
    ...
    println!("// signature on TestEndlessCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    for i in 1..=num {
        let msg = format!("Hello, Endless {}!", i);
        let keypair = KeyPair::<PrivateKey, PublicKey>::generate(&mut rng);
    ...
    println!(
        "// The signed messages are \"Hello, Endless <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
