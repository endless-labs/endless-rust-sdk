...
use crate::{
    ...
    test_utils::{TestEndlessCrypto, TEST_SEED},
    traits::*,
...
static MESSAGE: Lazy<TestEndlessCrypto> =
    Lazy::new(|| TestEndlessCrypto("Test Message".to_string()));
fn message() -> &'static TestEndlessCrypto {
    &MESSAGE
}
...
fn test_sample_multisig() {
    ...
    println!("let msg = b\"Hello Endless!\";");
    print!("//let ks = vector[");
    for k in ks {
        print!("{k}, ")
    }
