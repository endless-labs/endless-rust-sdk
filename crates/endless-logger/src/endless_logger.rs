...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        let peer_id = endless_node_identity::peer_id_as_str();
        let chain_id = endless_node_identity::chain_id().map(|chain_id| chain_id.id());
...
/// A builder for a `EndlessData`, configures what, where, and how to write logs.
pub struct EndlessDataBuilder {
    channel_size: usize,
...
impl EndlessDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<EndlessData> {
        ...
        if self.is_async {
            ...
            let logger = Arc::new(EndlessData {
    ...
    pub fn build(&mut self) -> Arc<EndlessData> {
...
impl EndlessData {
    pub fn builder() -> EndlessDataBuilder {
        EndlessDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> EndlessDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Endless Data Builder
    ...
    fn send_entry(&self, entry: LogEntry) {
        ...
        if let Some(sender) = &self.sender {
            let _ = sender.try_send(LoggerServiceEvent::LogEntry(entry));
        }
    }
}

impl Logger for EndlessData {
    fn enabled(&self, metadata: &Metadata) -> bool {
...
/// or to a `EndlessData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<EndlessData>,
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/endless-logger/src/lib.rs:261 Hello { "world": true }
...
fn json_format(entry: &LogEntry) -> Result<String, fmt::Error> {
    serde_json::to_string(&entry).map_err(|_| fmt::Error)
}
...
pub struct LoggerFilterUpdater {
    logger: Arc<EndlessData>,
    logger_builder: EndlessDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<EndlessData>, logger_builder: EndlessDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{EndlessData, LogEntry};
    use crate::{
        debug,
        endless_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        error, info,
        logger::Logger,
        trace, warn, EndlessDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater,
        Metadata, Schema, Value, Visitor,
    };
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"endless_logger\",\"file\":\"crates/endless-logger/src/endless_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (EndlessDataBuilder, Arc<EndlessData>) {
        let mut logger_builder = EndlessDataBuilder::new();
