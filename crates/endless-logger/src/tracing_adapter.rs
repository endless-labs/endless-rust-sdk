...
/// A layer that translates tracing events into endless-logger events.
pub struct TracingToEndlessDataLayer;
...
impl<S> Layer<S> for TracingToEndlessDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-endless-logger] failed to translate event due to unknown level {:?}",
